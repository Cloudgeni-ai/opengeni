...
- **Sandbox**: pluggable execution environment behind the OpenAI Agents SDK sandbox interface. OpenGeni should describe the contract and selected backend, not pretend the backend is hard-coded. The shipped `SandboxBackend` enum is broad (currently twelve members, including the first-party Kubernetes Pod backend), so never claim it is only Docker/Modal/local/none.
