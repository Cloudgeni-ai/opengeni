...
- **Sandbox rotation wait**: a recovering turn fenced by an active managed-sandbox rotation parks on its exact sandbox group and lease epoch. The reaper's durable draining-to-cold commit wakes that waiter; the workflow does not repeatedly reserve turn-worker slots while the same transition remains pending.
