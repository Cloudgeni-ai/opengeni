...
- External host identities and credential authority: `packages/core/src/access/`,
  ...
  `captureDirectHostMcpAuthority` persists that snapshot in the 0439 append-only
  ...
  work and exact same-session causal resumptions. Migration 0440 separately
  proves delivered goal/child-result lineage before copying authority. Revoked
  selections are omitted, not broadened. Migration 0441 freezes host selections
  ...
  Migration 0442 admits only the spawning turn's selected `always` grants to a
