...
4. Approval-required operations show a parent-owned one-shot approval dialog.
   Site code cannot confirm its own approval or read the viewer's credential.
5. Handle missing tools, declined approvals, revoked connections, and stale
   catalogs as normal user-visible error states.
...
- The requested tool list is exact and minimal, and approval/error states are
  understandable.
