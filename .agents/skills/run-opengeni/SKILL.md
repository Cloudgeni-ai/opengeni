...
Identify the actual execution host first: an isolated agent sandbox is not the user's physical computer. The full checkout launcher supports Linux and macOS (Docker infrastructure on macOS); on Windows use WSL2, with the checkout and tools inside Linux, not a mixture of Windows and WSL executables. Read `.bun-version` for the exact Bun version; an arbitrary recent Bun is not sufficient. Check available disk space, Docker daemon access (a Docker CLI alone is not enough), and run `bun run dev:check` for the selected backend's prerequisites. Report missing prerequisites together. `bun run dev:tools` prints a no-write plan; `bun run dev:tools -- --install` installs verified project-local service binaries, and the launcher finds them automatically. PostgreSQL 17 with matching pgcrypto/pgvector still needs host packages on the native path; use the checker's OS-specific guidance. Do not improvise unverified binary mirrors or install Go merely to recover a failed MinIO download. Fresh native storage uses Garage; preserve recorded MinIO state and never clean data to change the default.
