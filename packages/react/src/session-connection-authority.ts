...
function normalizeProviderDomain(domain: string): string {
  return domain
    .trim()
    .toLowerCase()
    .replace(/^www\./, "");
...
export async function sessionConnectionAuthorities(
  ...
  return personal.flatMap((item) => {
    ...
    // conversation has authorized them or an existing workspace/context grant
    // covers it. Restore that exact account, never a provider-domain default.
    const matches = personalConnections(item, connections).flatMap((connection) => {
      ...
      // Preserve a conversation-specific selection when a broader grant also
      // exists for this same account; grant listing order must not widen it.
