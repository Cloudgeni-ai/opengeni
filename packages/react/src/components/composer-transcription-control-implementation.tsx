...
export type ComposerTranscriptionControlImplementationProps = ComposerTranscriptionControlProps & {
  composer: {
    value: string;
  ...
  onStartOnMountConsumed?: (() => void) | undefined;
};
...
export function ComposerTranscriptionControlImplementation({
  client = null,
  workspaceId = "",
  capability = null,
  ...
  const startOnMountConsumedRef = useRef(false);
  const startTranscription = transcription.start;
  useEffect(() => {
    if (!startOnMount || startOnMountConsumedRef.current) return;
    startOnMountConsumedRef.current = true;
    onStartOnMountConsumed?.();
    if (!suppressed) void startTranscription();
  }, [onStartOnMountConsumed, startOnMount, startTranscription, suppressed]);
