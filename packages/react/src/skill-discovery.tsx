...
export type SkillDiscoveryItem = { id: string; name: string; source: string; installs: number; url: string };
...
export function SkillDiscovery(props: SkillDiscoveryProps) {
  ...
  return <section className="og-skill-discovery" aria-label="Discover skills">
    <header><h3>Discover skills</h3><a href="https://skills.sh/" target="_blank" rel="noopener noreferrer" className="og-skill-discovery-browse">Browse skills.sh ↗</a></header>
    {!query && props.onSearch ? <button type="button" className="og-skill-discovery-search" onClick={props.onSearch}>Search skills</button> : null}
  ...
  return <div aria-busy={loading}>
    <div className="og-skill-discovery-grid">
      {result?.items.map((skill) => {
        const installed = installedSkills.some((entry) =>
          entry.sourceUrl.replace(/\/$/, "").toLowerCase() === skill.url.toLowerCase() ||
          (entry.repositoryUrl.replace(/\/$/, "").toLowerCase() === `https://github.com/${skill.source}`.toLowerCase() && entry.name.toLowerCase() === skill.name.toLowerCase()));
        return <button key={skill.id} type="button" className="og-skill-discovery-item"
        disabled={!canManage} onClick={() => onImport(skill.url)}>
        <span className="og-skill-discovery-copy"><strong>{skill.name}</strong><span>{skill.source}</span><small>{skill.installs.toLocaleString()} installs</small></span>
        <span className="og-skill-discovery-action">{installed ? <><span aria-hidden="true">✓ </span>Installed</> : !canManage ? "Admin required" : "Preview →"}</span>
      </button>; })}
    </div>
    {loading ? <p role="status">Loading skills…</p> : null}
    {error ? <div role="alert" className="og-skill-discovery-error">Could not load skills.<button type="button" onClick={() => setRetry((value) => value + 1)}>Retry</button></div> : null}
    {result && !result.items.length && !loading && !error ? <p role="status">No skills found. Try a different search.</p> : null}
  </div>;
}
