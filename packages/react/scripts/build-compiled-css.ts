...
export async function buildEffectiveTokensCss(): Promise<string> {
  ...
  const declarations = [...defaults]
    .filter(([, value]) => isDerivedToken(value))
    .map(([token]) => `  ${effectiveToken(token)}: var(${token});`)
    .join("\n");
  return `/* Generated by scripts/build-compiled-css.ts; do not edit directly. */\n.og-root,\n[data-og-density="compact"],\n.og-density-compact,\n[data-og-theme="light"],\n.og-light {\n${declarations}\n}\n`;
}

function hydrateEffectiveTokenFallbacks(
  root: ReturnType<typeof parse>,
  defaults: ReadonlyMap<string, string>,
): void {
  root.walkDecls(/^--_og-/u, (declaration) => {
    const token = declaration.prop.replace(/^--_og-/u, "--og-");
    if (declaration.value.trim() !== `var(${token})`) return;
    const fallback = defaults.get(token);
    if (!fallback || !isDerivedToken(fallback)) {
      throw new Error(`Effective token alias has no derived fallback: ${token}`);
    }
    declaration.value = `var(${token}, ${effectiveDerivedDefault(fallback, defaults)})`;
  });
}
...
function rewriteDerivedTokenConsumers(
  ...
  root.walkDecls((declaration) => {
    if (declaration.prop.startsWith("--_og-")) return;
