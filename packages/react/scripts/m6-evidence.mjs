...
for (const theme of ["dark", "light"]) {
  ...
  if (theme === "dark") {
    results.push({
      proof: "E1 renderer attached (idle)",
      renderer: info?.renderer,
      PASS: info?.renderer === "webgl" || info?.renderer === "dom",
    });
    results.push({
      proof: "E2 fontFamily concrete (no var()",
      fontFamily: info?.fontFamily,
      hasVar: info?.hasVarInFont,
      PASS: info?.hasVarInFont === false && !!info?.fontFamily,
    });
  }
  results.push({
    proof: `E3 full ANSI theme (${theme})`,
    ansiPresent,
    hasSelection: !!theme3.selectionBackground,
    hasCursor: !!theme3.cursor,
    PASS: ansiPresent && !!theme3.selectionBackground && !!theme3.cursor,
  });
  await page.close();
}
...
for (const fail of ["webgl", "webgl,canvas"]) {
  ...
  const renderer = await page.evaluate(() =>
    document.querySelector("[data-opengeni-terminal]")?.getAttribute("data-og-term-renderer"),
  );
...
// ── E4: no 80x24 flash (hidden-until-fit ordering) ───────────────────────────
{
  ...
  const sawHiddenBeforeReady = frames
    .slice(0, firstReady < 0 ? frames.length : firstReady)
    .some((f) => f.vis === "hidden");
...
// ── E5: projection→PTY preserves the visible buffer ──────────────────────────
{
  ...
  const wasReadOnly = await page.evaluate(() =>
    document.body.innerText.toLowerCase().includes("read-only"),
  );
  ...
  const nowInteractive = await page.evaluate(
    () => !document.body.innerText.toLowerCase().includes("read-only"),
  );
  results.push({
...
// ── E6: bursty output responsiveness (frame-time sample) ─────────────────────
{
  ...
  const sample = await page.evaluate(async () => {
    ...
    for (let i = 0; i < 4000; i++)
      chunk.push(`\x1b[3${i % 8}mline ${i} \x1b[1mburst\x1b[0m payload token ${i * 7}\r\n`);
