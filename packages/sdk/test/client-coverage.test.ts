...
describe("OpenGeniClient connections", () => {
  ...
  test("Google Drive preview uses dedicated OAuth, browse, and source routes", async () => {
    const connection = fakeConnection({
    ...
    const started = await client.startGoogleDriveConnection(WORKSPACE_ID);
    ...
    expect(
      ...
      `POST /v1/workspaces/${WORKSPACE_ID}/connections/google-drive/install`,
      `GET /v1/workspaces/${WORKSPACE_ID}/connections/google-drive/${connection.id}/browse?parentId=root`,
