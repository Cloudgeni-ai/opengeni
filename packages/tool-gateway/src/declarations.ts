...
export function generateToolDeclarations(
  ...
  options: {
    ...
    /** Optional runtime wrapper for typed results carrying out-of-band image blocks. */
    typedResultWrapperType?: string;
    generatedBy: string;
  ...
  return [
    ...
    ...(options.typedResultWrapperType
      ? [
          "// Typed calls normally return their structured output. If image blocks are present,",
          "// they return CodemodeMultimodalValue<T> with local image paths and original content.",
        ]
      : []),
    `import type { ${[
      options.callOptionsType,
      options.fallbackResultType,
      ...(options.typedResultWrapperType ? [options.typedResultWrapperType] : []),
    ].join(", ")} } from ${JSON.stringify(options.moduleSpecifier)};`,
    "",
...
function renderChildren(
  ...
  for (const [name, child] of [...node.children].sort(([left], [right]) =>
    ...
    if (child.entry) {
      ...
      const structuredOutput = entry.outputSchema
        ? jsonSchemaToTypeScript(entry.outputSchema)
        : null;
      const output = structuredOutput
        ? options.typedResultWrapperType
          ? `${structuredOutput} | ${options.typedResultWrapperType}<${structuredOutput}>`
          : structuredOutput
        : options.fallbackResultType;
