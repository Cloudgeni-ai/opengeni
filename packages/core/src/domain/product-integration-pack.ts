...
- For packaged React chat, use SessionConversation or compose MessageTimeline and ChatComposer with the normal SDK. The separate React chat wrapper is removed. The backend chat handler remains optional and requires a custom or compatible frontend; it is not the transport for the native session components.
