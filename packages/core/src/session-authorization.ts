...
/**
 * The agent-to-agent reach rule (migration 0425) for one caller/target pair
...
export async function requireSessionAuthorization(
  ...
  // Agent-access scope (migration 0425): an attempt always keeps its own root
...
export async function requireSessionAuthorizationListScope(
  ...
  // declared agent-access reach (migration 0425). The viewer is derived from
