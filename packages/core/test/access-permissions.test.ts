...
describe("literal high-trust permissions", () => {
  ...
  test("an explicit secrets:read grant passes the literal boundary", () => {
    ...
    expect(hasLiteralPermission(explicit.permissions, "secrets:read")).toBe(
      true,
    );
    expect(() =>
      requireLiteralPermission(explicit, "secrets:read"),
    ).not.toThrow();
  });

  test("legacy scopes imply granular metadata and write scopes but never plaintext", () => {
    expect(hasPermission(["variable-sets:use"], "variable-sets:list")).toBe(
      true,
    );
    expect(hasPermission(["variable-sets:use"], "variable-sets:read")).toBe(
      true,
    );
    expect(hasPermission(["variable-sets:use"], "secrets:list")).toBe(true);
    expect(hasPermission(["variable-sets:use"], "variable-sets:write")).toBe(
      false,
    );
    expect(hasPermission(["variable-sets:use"], "secrets:write")).toBe(false);

    expect(hasPermission(["variable-sets:manage"], "variable-sets:write")).toBe(
      true,
    );
