...
async function fixture(mode: "always" | "session") {
  ...
  const connection = await createConnection(db, {
    ...
    providerDomain: "slack.com",
    kind: "oauth2",
    credentialEncrypted: "test-never-sent",
  });
  ...
  const delegations = await freezePersonalConnectionDelegations({
    ...
    settings,
    tools,
    source: { kind: "subject", subjectId, accountId: grant.accountId },
    authoritySelections: [
      { serverId: "slack", connectionId: connection.id, userDelegation: issued.delegation },
    ],
  });
  await withWorkspaceSubjectSessionActivityRls(db, grant.workspaceId, subjectId, (tx) =>
  ...
  const claimed = await withWorkspaceSessionActivityRls(db, grant.workspaceId, (tx) =>
    claimSessionWorkForAttempt(tx, grant.workspaceId, {
      sessionId: session.id,
  ...
  const createTask = (targetSessionId: string | null = mode === "session" ? session.id : null) =>
    createValidatedScheduledTask({
      ...
      payload: CreateScheduledTaskRequest.parse({
        name: "My Slack schedule",
        schedule: { type: "manual" },
        runMode: targetSessionId ? "existing_session" : "new_session_per_run",
        targetSessionId,
        agentConfig: {
          prompt: "Read my latest Slack message",
          tools,
          resources: [],
          model: "scripted-model",
          sandboxBackend: "none",
...
async function createOccurrence(task: ScheduledTask, subjectId: string) {
  ...
  const personalConnectionDelegations = await getScheduledTaskPersonalConnectionDelegations(
    db,
    task.workspaceId,
    task.id,
  );
  ...
  return await createScheduledTaskRun(db, {
    runId,
    workspaceId: task.workspaceId,
    taskId: task.id,
    taskAuthorityRevision: task.authorityRevision,
    ...
    producerKey: `scheduled-slack:${runId}`,
    acceptedExecutionSnapshot: {
      ...
      generatedSessionBinding: targetSessionExecution
        ? null
        : {
            createIdempotencyKey: `scheduled-slack:${runId}`,
...
test("a live agent carries its human's standing Slack consent into a generated scheduled occurrence", async () => {
  ...
  expect(
    await getScheduledTaskPersonalConnectionDelegations(client.db, task.workspaceId, task.id),
  ).toEqual(f.delegations);
  const run = await createOccurrence(task, f.subjectId);
  expect(run.status).toBe("queued");
  const snapshots =
    await shared.admin`select owner_subject_id, grant_id, grant_mode from scheduled_task_run_connection_authority_snapshots where run_id = ${run.id}`;
  expect(snapshots).toHaveLength(1);
  expect(snapshots[0]).toMatchObject({
    owner_subject_id: f.subjectId,
    grant_id: f.issued.grantId,
    grant_mode: "always",
  });
  await expect(createOccurrence(task, "user:unrelated-member")).rejects.toThrow();
...
test("a live agent retains exact-session Slack consent only for that same scheduled destination", async () => {
  ...
  await expect(f.createTask(null)).rejects.toThrow("ongoing");
}, 180_000);

test("once-only Slack consent is rejected for schedules even targeting its original chat", () => {
