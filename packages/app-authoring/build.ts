...
if (exitCode !== 0) throw new Error(`app-authoring build failed with exit code ${exitCode}`);
...
if (!binaryBuild.success) {
  ...
  throw new Error("og-app Bun bundle failed");
}

const executable = resolve(binaryDirectory, "og-app.cjs");
