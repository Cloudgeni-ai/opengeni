...
const SettingsSchema = z.object({
  ...
  // Organization governance is a separately activated v2 authority plane.
  // Keeping it default-off lets legacy API/worker fleets continue their normal
  // database path during a rolling rollout; the v2 handle is verified against
  // its own runtime role before any governance route is served.
  organizationGovernanceEnabled: EnvBoolean.default(false),
  organizationGovernanceDatabaseUrl: z.string().optional(),
  organizationGovernanceDatabaseRole: z.string().min(1).default("opengeni_governance_app"),
...
export function getSettings(): Settings {
  const raw = {
    ...
    organizationGovernanceEnabled: optional("OPENGENI_ORGANIZATION_GOVERNANCE_ENABLED"),
    organizationGovernanceDatabaseUrl: optional("OPENGENI_ORGANIZATION_GOVERNANCE_DATABASE_URL"),
    organizationGovernanceDatabaseRole: optional("OPENGENI_ORGANIZATION_GOVERNANCE_DATABASE_ROLE"),
...
function validateSettings(settings: Settings): void {
  ...
  if (settings.organizationGovernanceEnabled) {
    if (settings.organizationGovernanceDatabaseRole === settings.runtimeDatabaseRole) {
      throw new Error(
