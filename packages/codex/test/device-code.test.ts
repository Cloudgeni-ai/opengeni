...
describe("pollDeviceCode", () => {
  test("403 and 404 -> pending", async () => {
    for (const status of [403, 404]) {
      ...
      expect(await pollDeviceCode({ deviceAuthId: "d", userCode: "u" }, fetchImpl)).toEqual({
        status: "pending",
      });
    }
  });
  ...
  test("200 -> authorized with PKCE pair", async () => {
    const { fetchImpl } = recorder(() =>
      json({ authorization_code: "ac_1", code_verifier: "ver_1" }),
    );
    expect(await pollDeviceCode({ deviceAuthId: "d", userCode: "u" }, fetchImpl)).toEqual({
      status: "authorized",
  ...
  test("other status -> throws", async () => {
    ...
    await expect(
      pollDeviceCode({ deviceAuthId: "d", userCode: "u" }, fetchImpl),
    ).rejects.toBeInstanceOf(CodexDeviceError);
  });
});
...
describe("exchangeDeviceCode", () => {
  test("form-encodes grant_type=authorization_code with redirect_uri and returns tokens", async () => {
    const { fetchImpl, calls } = recorder(() =>
      json({ id_token: "id", access_token: "ac", refresh_token: "rf" }),
    );
    const tokens = await exchangeDeviceCode(
      { authorizationCode: "ac_1", codeVerifier: "ver_1" },
      fetchImpl,
    );
