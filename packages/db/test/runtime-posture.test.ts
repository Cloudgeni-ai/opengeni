...
describe("runtime database posture evaluator", () => {
  test("freezes the unique, sorted current-ledger table privilege classes", () => {
    const contracts = [
      ...
      [RUNTIME_FULL_DML_TABLES, 86],
      [RUNTIME_READ_ONLY_TABLES, 1],
      [RUNTIME_READ_INSERT_TABLES, 3],
      [PROTECTED_NO_DIRECT_DML_TABLES, 5],
      [RUNTIME_DML_TABLES, 90],
    ] as const;
    ...
    expect(new Set([...RUNTIME_DML_TABLES, ...PROTECTED_NO_DIRECT_DML_TABLES]).size).toBe(95);
    expect(new Set([...FORCE_RLS_TABLES, ...NON_RLS_RUNTIME_TABLES]).size).toBe(95);
    expect(
