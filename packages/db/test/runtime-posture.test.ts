...
describe("runtime database posture evaluator", () => {
  test("freezes the unique, sorted current-ledger table privilege classes", () => {
    const contracts = [
      ...
      [RUNTIME_READ_INSERT_TABLES, 40],
      [RUNTIME_READ_INSERT_UPDATE_TABLES, 16],
      [PROTECTED_NO_DIRECT_DML_TABLES, 7],
      [RUNTIME_DML_TABLES, 190],
    ] as const;
    ...
    expect(new Set([...RUNTIME_DML_TABLES, ...PROTECTED_NO_DIRECT_DML_TABLES]).size).toBe(197);
    expect(new Set([...FORCE_RLS_TABLES, ...NON_RLS_RUNTIME_TABLES]).size).toBe(197);
