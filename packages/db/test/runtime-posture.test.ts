...
describe("runtime database posture evaluator", () => {
  test("freezes the unique, sorted current-ledger table privilege classes", () => {
    const contracts = [
      [FORCE_RLS_TABLES, 115],
      [NON_RLS_RUNTIME_TABLES, 11],
      [RUNTIME_FULL_DML_TABLES, 93],
      [RUNTIME_READ_ONLY_TABLES, 6],
      [RUNTIME_READ_INSERT_TABLES, 22],
      [PROTECTED_NO_DIRECT_DML_TABLES, 5],
      [RUNTIME_DML_TABLES, 121],
    ] as const;
    ...
    expect(new Set([...RUNTIME_DML_TABLES, ...PROTECTED_NO_DIRECT_DML_TABLES]).size).toBe(126);
    expect(new Set([...FORCE_RLS_TABLES, ...NON_RLS_RUNTIME_TABLES]).size).toBe(126);
    expect(
