...
const migrationUrl = new URL("../drizzle/0212_slack_installation_bindings.sql", import.meta.url);
const requireRealDatabase = process.env.OPENGENI_REQUIRE_REAL_DB === "1";
let shared: SharedTestDatabase | null = null;

beforeAll(async () => {
  shared = await acquireSharedTestDatabase("migration-0212-slack-bindings");
  if (!shared && requireRealDatabase) {
    throw new Error(
      "[migration-0212-slack-bindings] OPENGENI_REQUIRE_REAL_DB=1 but PostgreSQL is unavailable",
    );
  }
}, 180_000);
...
describe("migration 0212 Slack installation bindings", () => {
