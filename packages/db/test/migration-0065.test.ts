...
const migrationFile = "0065_sandbox_recovery_protocol_fence.sql";
...
describe("migration 0065 (sandbox recovery protocol maintenance activation)", () => {
  test("refuses live old writers, rolls back atomically, then fences legacy acquisition", async () => {
    ...
    try {
      ...
      for (const file of pre0065) await applyFile(admin, file);
