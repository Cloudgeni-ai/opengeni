...
const migrationName = "0096_enrollment_credential_generation.sql";
...
beforeAll(async () => {
  blank = await acquireBlankTestDatabase("migration-0096-enrollment-generation");
  if (!blank) {
    throw new Error(
      "[migration-0096-enrollment-generation] real PostgreSQL is required; migration proof cannot skip",
    );
  }
}, 180_000);
...
describe("0096 enrollment credential generation migration (real PostgreSQL)", () => {
  test("follows the current migration chain and defaults every legacy enrollment to generation 1", async () => {
    if (!blank) {
    ...
    try {
      ...
      const accountId = (
        await admin<
          { id: string }[]
        >`insert into managed_accounts (name) values ('0096 legacy account') returning id`
      )[0]!.id;
      const workspaceId = (
        await admin<
          { id: string }[]
        >`insert into workspaces (account_id, name) values (${accountId}, '0096 legacy workspace') returning id`
      )[0]!.id;
      const legacyEnrollmentId = (
        await admin<{ id: string }[]>`
          insert into enrollments (account_id, workspace_id, pubkey)
          values (${accountId}, ${workspaceId}, 'ed25519:LEGACY-0096') returning id`
      )[0]!.id;
      ...
      const [fresh] = await admin<{ credential_generation: number }[]>`
        insert into enrollments (account_id, workspace_id, pubkey)
        values (${accountId}, ${workspaceId}, 'ed25519:FRESH-0096')
