...
beforeAll(async () => {
  owned = await acquireOwnerMigratedTestDatabase("migration-0478-connection-consent");
  if (!owned) {
...
test("preactivation consent retains owner, private-session, workspace, epoch and resource-kind fences under FORCE RLS", async () => {
  ...
  const standing = () => issue({ mode: "always", sessionId: null, expectedAuthorityEpoch: null });
  const standingGrant = await standing();
  expect(standingGrant).toMatchObject({
    mode: "always",
    context: "workspace_shared",
    authorityEpoch: null,
  });
  expect((await standing()).grantId).toBe(standingGrant.grantId);
  await expect(
    issue({ mode: "always", sessionId: session.id, expectedAuthorityEpoch: 1 }),
  ).rejects.toThrow();
  await expect(
    issue({
      mode: "always",
  ...
  await expect(
    issue({ mode: "always", sessionId: null, expectedAuthorityEpoch: null, subjectId: stranger }),
  ).rejects.toThrow();
  await expect(
    issue({
      mode: "always",
  ...
  await admin`update connections set status = 'revoked' where id = ${connection.id}`;
  await expect(standing()).rejects.toThrow();
  // Revocation stays possible after disconnect and increments generation once.
  const revokedStanding = await revokeSelfUserResourceGrant(db, {
    accountId,
    workspaceId,
    subjectId,
    grantId: standingGrant.grantId,
  });
  expect(revokedStanding).toMatchObject({ status: "revoked", generation: 2 });
  expect(
    (
  ...
  await admin`update connections set status = 'active' where id = ${connection.id}`;
