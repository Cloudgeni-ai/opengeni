...
const migration = "0172_retire_model_visible_github_token.sql";
...
beforeAll(async () => {
  blank = await acquireBlankTestDatabase("migration-0172-retire-model-visible-github-token");
  if (!blank) {
    if (requireRealDatabase) {
      throw new Error(
        "[migration-0172-retire-model-visible-github-token] OPENGENI_REQUIRE_REAL_DB=1 but the real PostgreSQL harness is unavailable",
      );
    }
    available = false;
  }
