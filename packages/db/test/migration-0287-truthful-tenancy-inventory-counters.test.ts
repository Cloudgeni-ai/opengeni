// Migration 0287: the tenancy inventory seam's `unclassified` counters for
...
const migrationUrl = new URL(
  "../drizzle/0287_truthful_tenancy_inventory_counters.sql",
  import.meta.url,
);
...
beforeAll(async () => {
  shared = await acquireSharedTestDatabase("migration-0287-tenancy-inventory-counters");
  if (!shared) {
    available = false;
...
describe("migration 0287 truthful tenancy inventory counters", () => {
  ...
  test("byScope distinguishes a correctly classified workspace row from a user-scoped one, and no `unclassified` counter is reported", async () => {
    if (!available) return;
    const { accountId } = await seedOrganization("inventory-0287-counters");
  ...
  test("the removed predicate was structurally `total - userScoped`, never an unmigrated population", async () => {
    if (!available) return;
    const { accountId } = await seedOrganization("inventory-0287-predicate");
  ...
  test("CREATE OR REPLACE preserves the seam's EXECUTE grant to the application role", async () => {
    ...
    const [account] = await admin<{ id: string }[]>`
        insert into managed_accounts (name) values ('inventory-0287-grant') returning id`;
    const inventory = (await inventoryOrganizationTenancy(db, {
  ...
  test("a null organization id still raises 22004 and a cross-organization request still raises 42501", async () => {
    if (!available) return;
    const [account] = await admin<{ id: string }[]>`
        insert into managed_accounts (name) values ('inventory-0287-scope-a') returning id`;
    const [victim] = await admin<{ id: string }[]>`
        insert into managed_accounts (name) values ('inventory-0287-scope-b') returning id`;
