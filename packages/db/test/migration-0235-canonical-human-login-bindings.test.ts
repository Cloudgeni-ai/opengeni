...
const migrationPath = join(
  ...
  "../drizzle/0235_canonical_human_login_bindings.sql",
);
...
beforeAll(async () => {
  ...
  if (externalAdminUrl && externalAppUrl) {
    ...
    shared = await acquireSharedTestDatabase("migration-0235-canonical-human-identities");
  }
  if (!shared && requireRealDatabase) {
    throw new Error(
      "[migration-0235-canonical-human-identities] OPENGENI_REQUIRE_REAL_DB=1 but PostgreSQL is unavailable",
    );
  }
...
describe("migration 0235 canonical human identities and login bindings", () => {
