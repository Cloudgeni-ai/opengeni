...
const migrationUrl = new URL(
  "../drizzle/0257_scoped_connected_machines_and_rigs.sql",
  import.meta.url,
);
...
describe("migration 0257 scoped Connected Machines and Rigs", () => {
  ...
  test("enforces organization, workspace, and owner visibility through PostgreSQL", async () => {
    const blank = await acquireBlankTestDatabase("migration-0257-scoped-compute");
    if (!blank && requireRealDatabase) {
      throw new Error("[migration-0257-scoped-compute] PostgreSQL is unavailable");
    }
    if (!blank) return;
    ...
    try {
      ...
      `;
      expect(attemptAuthorization).toEqual({ enrollmentId: machine.enrollmentId });
      expect(
        await asActor(app, ownerB, async (tx) => {
          const [row] = await tx<Array<{ allowed: boolean }>>`
