...
const migrationName = "0397_workspace_html_site_sources.sql";
...
describe("migration 0397 workspace HTML Site sources", () => {
  ...
  test("upgrades legacy versions without rewriting them and enforces new version bounds", async () => {
    ...
    const [account] = await admin<{ id: string }[]>`
      ...
      values ('migration 0397 account')
      returning id`;
    const [workspace] = await admin<{ id: string }[]>`
      insert into workspaces (account_id, name)
      values (${account!.id}, 'migration 0397 workspace')
      returning id`;
