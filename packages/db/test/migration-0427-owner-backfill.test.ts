...
const cutover = "0427_unified_skill_lifecycle.sql";
const windowTables = [
...
describe("0427 owner-only Skill backfill", () => {
  test("migrates populated tenants as NOSUPERUSER NOBYPASSRLS and restores every FORCE policy", async () => {
    ...
    try {
        ...
        expect([...receipts]).toEqual([
          {
            ...
            actor: "service:skill-migration:0427",
          },
          {
      ...
      expect(
        await admin`select id from preference_registry_events where actor_subject_id='service:skill-migration:0427'`,
      ).toHaveLength(7);
      const restored =
