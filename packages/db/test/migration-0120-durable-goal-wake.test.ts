...
beforeAll(async () => {
  blank = await acquireBlankTestDatabase("migration-0120");
  if (!blank) {
    if (requireRealDatabase) {
      throw new Error(
        "[migration-0120] OPENGENI_REQUIRE_REAL_DB=1 but the real PostgreSQL harness is unavailable",
      );
    }
    available = false;
  }
...
describe("migration 0120 (durable goal wake)", () => {
  test("arms only idle legacy goals and preserves existing or blocked work", async () => {
    ...
    try {
      ...
      `);
      for (const file of files.filter((candidate) => candidate < "0120_")) {
      ...
      const [account] = await admin<{ id: string }[]>`
        insert into managed_accounts (name) values ('migration-0120-account') returning id`;
      ...
      const workspace = await insertWorkspace("migration-0120-workspace");
      const pausedWorkspace = await insertWorkspace("migration-0120-paused-workspace", true);
