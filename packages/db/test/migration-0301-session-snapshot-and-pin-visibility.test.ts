...
const migrationPath = join(
  ...
  "../drizzle/0301_session_snapshot_and_pin_visibility.sql",
);
...
beforeAll(async () => {
  shared = await acquireSharedTestDatabase("migration-0301-session-snapshot-and-pin-visibility");
  if (!shared && requireRealDatabase) {
    throw new Error(
      "[migration-0301-session-snapshot-and-pin-visibility] OPENGENI_REQUIRE_REAL_DB=1 but PostgreSQL is unavailable",
    );
  }
...
describe("migration 0301 session list snapshot and pin visibility", () => {
  ...
  test("keeps a stale pin invisible to the product but removable by its owner", async () => {
    ...
    try {
      ...
      // AFTER: restore 0301's owner escape.
      await shared.admin.unsafe(`
        drop policy session_visibility_isolation on session_pins;
        create policy session_visibility_isolation on session_pins as restrictive
          for all
