...
BEGIN
  IF to_regclass('pg_temp.knowledge_conversion_0461') IS NULL THEN
    RAISE EXCEPTION '0461 requires the codec-aware TypeScript migration runner' USING ERRCODE='55000';
  END IF;
  IF roles IS NULL OR jsonb_typeof(roles) <> 'array' THEN
    RAISE EXCEPTION '0461 requires explicit application database roles' USING ERRCODE='55000';
  END IF;
  IF jsonb_array_length(roles) NOT BETWEEN 1 AND 16 OR EXISTS (
    SELECT 1 FROM jsonb_array_elements(roles) item WHERE jsonb_typeof(item) <> 'string'
      ...
      OR item #>> '{}' <> btrim(item #>> '{}')
  ) THEN RAISE EXCEPTION '0461 received invalid application roles' USING ERRCODE='55000'; END IF;
  IF EXISTS (SELECT 1 FROM pg_stat_activity a
    ...
    RAISE EXCEPTION '0461 requires drained application sessions' USING ERRCODE='55000';
  END IF;
END $drain$;
...
BEGIN
  ...
  IF strpos(definition,old_fragment)=0 THEN RAISE EXCEPTION 'Scheduled generated-session fence changed before 0461'; END IF;
...
BEGIN
  ...
  IF strpos(definition,old_fragment)=0 THEN RAISE EXCEPTION 'Private-create authority fence changed before 0461'; END IF;
  EXECUTE replace(definition,old_fragment,new_fragment);
...
CREATE TEMP TABLE knowledge_session_file_owners_0461 ON COMMIT DROP AS
...
;
CREATE TEMP TABLE knowledge_file_owners_0461 ON COMMIT DROP AS
WITH session_owners AS (SELECT * FROM knowledge_session_file_owners_0461), original_references AS (
...
FROM knowledge_file_owners_0461 ownership WHERE f.id=ownership.file_id AND f.account_id=ownership.account_id;
...
UPDATE video_generation_operations o SET private_file_owner_subject_id=s.owner
  FROM knowledge_session_file_owners_0461 s WHERE o.session_id=s.id AND o.account_id=s.account_id;
UPDATE retained_screenshot_artifacts a SET private_file_owner_subject_id=s.owner
  FROM knowledge_session_file_owners_0461 s WHERE a.session_id=s.id AND a.account_id=s.account_id;
...
SELECT w.account_id,w.id,'workspace:'||w.id,1,
  ...
  gen_random_uuid(),'migration:0461','service:knowledge-migration:0461','epoch'::timestamptz
...
SELECT m.account_id,w.id,'personal:'||m.subject_id,m.subject_id,1,r.settings,
  gen_random_uuid(),'migration:0461:personal','service:knowledge-migration:0461','epoch'::timestamptz
...
SELECT s.account_id,s.workspace_id,
  ...
  'chat:'||s.id,1,'{"knowledge":"off"}',gen_random_uuid(),'migration:0461:chat','service:knowledge-migration:0461','epoch'::timestamptz
...
RETURNS jsonb LANGUAGE sql STABLE SET search_path FROM CURRENT AS $$
  SELECT coalesce(p_revision.agent_learning_context,(SELECT op.actor->'legacyInstructionContext'
    FROM agent_instruction_operations op WHERE op.account_id=p_revision.account_id AND op.revision_id=p_revision.id
      AND op.request_hash='migration:0461:instruction-review' AND op.actor->>'kind'='migration' LIMIT 1))
$$;
  ...
  IF p_request->>'operation'='recover' THEN
    FOR candidate IN
      ...
      WHERE imported.account_id=p_account AND imported.origin_workspace_id=p_workspace
        AND imported.request_hash='migration:0461:instruction-review' AND imported.actor->>'kind'='migration'
    ...
    LOOP
      BEGIN
        recovered:=knowledge_instruction_confirm_legacy(p_account,p_workspace,p_actor,jsonb_build_object(
          'operationId',overlay(overlay(md5('0461:instruction-confirm:'||candidate.request_id||':'||candidate.decision_id) placing '5' from 13 for 1) placing '8' from 17 for 1)::uuid,
...
SELECT r.account_id,r.workspace_id,md5('0461:instruction-review:'||r.id)::uuid,r.id,'migration:0461:instruction-review',
  jsonb_build_object('kind','migration','subjectId','service:knowledge-migration:0461','legacyInstructionContext',
  ...
  jsonb_build_object('operationId',md5('0461:instruction-review:'||r.id)::uuid,'revisionId',r.id,
...
DO $copied$
BEGIN
  IF (SELECT count(*) FROM pg_temp.knowledge_conversion_0461 WHERE completed)<>1 THEN
    RAISE EXCEPTION '0461 Knowledge conversion did not complete' USING ERRCODE='55000';
  END IF;
END $copied$;
