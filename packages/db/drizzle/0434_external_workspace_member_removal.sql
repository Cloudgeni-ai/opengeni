...
BEGIN
  ...
  IF (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1 THEN
    RAISE EXCEPTION '0434 removal declaration drift' USING ERRCODE = '55000';
  END IF;
...
$service$;
  IF (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1 THEN
    RAISE EXCEPTION '0434 removal authority drift' USING ERRCODE = '55000';
  END IF;
  ...
  IF (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1
    ...
    RAISE EXCEPTION '0434 removal receipt or fence drift' USING ERRCODE = '55000';
  END IF;
