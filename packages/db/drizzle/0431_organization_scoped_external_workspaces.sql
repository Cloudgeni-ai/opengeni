...
BEGIN
  IF roles IS NULL OR jsonb_typeof(roles) <> 'array' THEN
    RAISE EXCEPTION '0431 requires an explicit application database role list' USING ERRCODE = '55000';
  END IF;
  IF jsonb_array_length(roles) NOT BETWEEN 1 AND 16 OR EXISTS (
    ...
    RAISE EXCEPTION '0431 received invalid application database roles' USING ERRCODE = '55000';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION '0431 requires all application database sessions stopped' USING ERRCODE = '55000';
  END IF;
END
$drain$;
...
BEGIN
  ...
  IF strpos(definition, 'RETURN organization_membership_command_0263(p_command);') = 0 THEN
    RAISE EXCEPTION '0431 organization membership wrapper drifted' USING ERRCODE = '55000';
  END IF;
  ...
  IF (length(definition) - length(replace(definition, old_target, ''))) / length(old_target) <> 1 THEN
    RAISE EXCEPTION '0431 organization membership conflict target drifted' USING ERRCODE = '55000';
  END IF;
  ...
  IF (length(definition) - length(replace(definition, old_personal_guard, ''))) / length(old_personal_guard) <> 1 THEN
    RAISE EXCEPTION '0431 personal workspace conflict guard drifted' USING ERRCODE = '55000';
  END IF;
