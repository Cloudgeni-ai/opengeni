...
-- Nested parent-cascade is the only Skill-history delete path. Direct deletes
-- stay forbidden. Cross-scope superseded_by/related_preference_id/corrects_revision_id
-- keep surviving audit bytes/IDs by remaining restrictive at commit.
...
BEGIN
  ...
  RETURN OLD;
END $$;
CREATE FUNCTION opengeni_private.guard_workspace_owned_skill_history_delete()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF TG_OP = 'DELETE' AND pg_trigger_depth() > 1 THEN RETURN OLD; END IF;
  RAISE EXCEPTION 'preference registry history is immutable' USING ERRCODE = '55000';
END $$;
DO $skill_delete_search_path$
BEGIN
  EXECUTE format(
  ...
  EXECUTE format(
    'ALTER FUNCTION opengeni_private.guard_workspace_owned_skill_history_delete() SET search_path = pg_catalog, %I, pg_temp',
    current_schema()
  );
END $skill_delete_search_path$;
REVOKE ALL ON FUNCTION opengeni_private.guard_workspace_owned_skill_head_delete() FROM PUBLIC;
REVOKE ALL ON FUNCTION opengeni_private.guard_workspace_owned_skill_history_delete() FROM PUBLIC;
...
CREATE TRIGGER preference_registry_revisions_immutable
  ...
  FOR EACH ROW EXECUTE FUNCTION opengeni_private.guard_workspace_owned_skill_history_delete();
...
CREATE TRIGGER preference_registry_events_immutable
  ...
  FOR EACH ROW EXECUTE FUNCTION opengeni_private.guard_workspace_owned_skill_history_delete();
DROP TRIGGER preference_registry_snapshots_immutable ON preference_registry_snapshots;
...
CREATE TRIGGER skill_write_receipts_immutable
  BEFORE UPDATE OR DELETE ON skill_write_receipts
  FOR EACH ROW EXECUTE FUNCTION opengeni_private.guard_workspace_owned_skill_history_delete();
DROP TRIGGER company_brain_preference_proposal_receipts_immutable ON company_brain_preference_proposal_receipts;
