...
-- leave the logical turn recovering with no active attempt, and then let its
-- Temporal workflow complete. Seed one delayed durable wake for those exact
-- rows; current workflows may consume it as an idempotent hint, while closed
-- workflows are restarted by the ordinary outbox dispatcher.
...
WITH orphaned AS (
  ...
  WHERE session_row.status = 'recovering'
