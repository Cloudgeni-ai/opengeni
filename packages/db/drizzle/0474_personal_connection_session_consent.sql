...
-- Personal Connection runtime authority was activated in 0264. Its exact-session
-- consent must not depend on the later private-session product rollout. Preserve
-- 0305 owner/membership/workspace, action, epoch, acknowledgement, and RLS fences.
-- Only Connection session grants become issuable before that rollout. Standing
-- grants and every other resource kind retain their existing activation gate.
-- CREATE OR REPLACE preserves the exact existing routine ACLs; no direct DML,
-- owner backfill, activation receipt, visibility, or runtime snapshot is changed.
...
BEGIN
  ...
  END IF;
  IF p_resource_kind <> 'connection' AND NOT EXISTS (
...
BEGIN
  ...
  IF p_context = 'workspace_shared' AND p_workspace_shared_acknowledged IS NOT TRUE THEN
    RAISE EXCEPTION 'workspace_shared requires durable shared-output acknowledgement'
      USING ERRCODE = '42501';
  END IF;
  IF (p_resource_kind <> 'connection' OR p_mode <> 'session') AND NOT EXISTS (
    SELECT 1 FROM session_tenancy_activations activation
    WHERE activation.account_id = p_account_id AND activation.activation_version = 1
  ) THEN
    RAISE EXCEPTION 'session tenancy product is not activated' USING ERRCODE = '42501';
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE grant_value.id = p_grant_id AND grant_value.account_id = p_account_id
      AND grant_value.workspace_id = p_workspace_id AND authority.resource_kind = 'connection'
      AND authority.organization_membership_id = owner_membership_id
      AND grant_value.owner_organization_membership_id = owner_membership_id
  ) THEN
    RAISE EXCEPTION 'session tenancy product is not activated' USING ERRCODE = '42501';
...
DO $harden$
BEGIN
  EXECUTE format('ALTER FUNCTION %I.list_self_user_resource_authorities(uuid,uuid,text,uuid,integer) SET search_path = pg_catalog, %I, pg_temp', current_schema(), current_schema());
  EXECUTE format('ALTER FUNCTION %I.issue_self_user_resource_grant(uuid,uuid,uuid,text,text,text,uuid,integer,boolean) SET search_path = pg_catalog, %I, pg_temp', current_schema(), current_schema());
  EXECUTE format('ALTER FUNCTION %I.revoke_self_user_resource_grant(uuid,uuid,uuid) SET search_path = pg_catalog, %I, pg_temp', current_schema(), current_schema());
END $harden$;
