...
-- Decouple explicit standing, shared-output personal Connection consent from the
-- private-session product rollout. Exact-session Connection consent is also independent;
-- other resource kinds and standing private-context grants retain their gates. No activation, credential copy, runtime selection,
-- task capture, or ordinary-message authority is synthesized here.
-- Replaces the 0305 lifecycle bodies while retaining their exact routine ACLs.
...
DECLARE
  ...
  connection_origin_workspace_id uuid;
  active_connection_found boolean;
BEGIN
  PERFORM set_config(
    'opengeni.organization_tenancy_lifecycle', 'personal_resource_grant_management', true
  );
  ...
  IF p_context = 'workspace_shared' AND p_workspace_shared_acknowledged IS NOT TRUE THEN
    RAISE EXCEPTION 'workspace_shared requires durable shared-output acknowledgement'
      USING ERRCODE = '42501';
  END IF;
  IF NOT (p_resource_kind = 'connection' AND (p_mode = 'session'
    OR (p_mode = 'always' AND p_context = 'workspace_shared'))) AND NOT EXISTS (
    SELECT 1 FROM session_tenancy_activations activation
    WHERE activation.account_id = p_account_id AND activation.activation_version = 1
  ) THEN
    RAISE EXCEPTION 'session tenancy product is not activated' USING ERRCODE = '42501';
  ...
  -- Consent never resurrects a disconnected credential or trusts an orphaned
  -- authority row. Only the exact owner's live Connection can gain standing use.
  IF p_resource_kind = 'connection' AND p_mode = 'always'
    AND p_context = 'workspace_shared' THEN
    SELECT authority.origin_workspace_id INTO connection_origin_workspace_id
    FROM organization_user_resource_authorities authority
    WHERE authority.id = p_authority_id AND authority.account_id = p_account_id
      AND authority.organization_membership_id = owner_membership_id;
    -- The locked owner authority names the exact origin; no caller-selected
    -- workspace or credential identity is used for this bounded RLS read.
    PERFORM set_config('opengeni.workspace_id', connection_origin_workspace_id::text, true);
    PERFORM 1 FROM connections connection_value
    JOIN organization_user_resource_authorities authority
      ON authority.id = connection_value.authority_id
      AND authority.account_id = connection_value.account_id
    WHERE authority.id = p_authority_id
      AND authority.resource_id = connection_value.id
      AND authority.origin_workspace_id = connection_value.workspace_id
      AND connection_value.account_id = p_account_id
      AND connection_value.owner_organization_membership_id = owner_membership_id
      AND connection_value.subject_id = caller_subject
      AND connection_value.authority_scope = 'user'
      AND connection_value.status = 'active'
    FOR SHARE OF connection_value;
    active_connection_found := FOUND;
    PERFORM set_config('opengeni.workspace_id', p_workspace_id::text, true);
    IF NOT active_connection_found THEN
      RAISE EXCEPTION 'active owner Connection not found' USING ERRCODE = '42501';
    END IF;
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE grant_value.id = p_grant_id AND grant_value.account_id = p_account_id
      ...
      AND authority.organization_membership_id = owner_membership_id
      AND grant_value.owner_organization_membership_id = owner_membership_id
      AND (grant_value.mode = 'session' OR (grant_value.mode = 'always'
