...
BEGIN
  IF roles IS NULL OR jsonb_typeof(roles) <> 'array' THEN
    RAISE EXCEPTION '0415 requires application database roles' USING ERRCODE = '55000';
  END IF;
  ...
  ) THEN RAISE EXCEPTION '0415 invalid application roles' USING ERRCODE = '55000'; END IF;
  IF EXISTS (SELECT 1 FROM pg_stat_activity a JOIN jsonb_array_elements_text(roles) r ON r = a.usename
    ...
    RAISE EXCEPTION '0415 requires stopped application sessions' USING ERRCODE = '55000';
  END IF;
END
$drain$;
