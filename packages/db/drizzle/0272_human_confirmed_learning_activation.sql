...
-- Human-confirmed governed-learning activation. A `remember` proposal that the
-- workspace learning policy would not activate automatically may still be
-- activated once the exact initiating human answered the bound structured
-- human-input question (`remember:<proposalId>` = `save`) on the same
-- session/turn generation. The activation is recorded on the same immutable
-- receipt ledger with an explicit authority kind and the human-input request
-- id, so the human answer that authorized it stays provable.

ALTER TABLE "governed_learning_activation_receipts"
  ADD COLUMN "authority_kind" text NOT NULL DEFAULT 'automatic',
  ADD COLUMN "human_input_request_id" uuid;
ALTER TABLE "governed_learning_activation_receipts"
  ADD CONSTRAINT "governed_learning_activation_receipts_authority_kind_check"
    CHECK ("authority_kind" IN ('automatic', 'human_confirmed')),
  ADD CONSTRAINT "governed_learning_activation_receipts_human_input_check"
...
DO $governed_learning_human_confirmed_hardening$
DECLARE
...
BEGIN
  ...
  END IF;
END
$governed_learning_human_confirmed_hardening$;
