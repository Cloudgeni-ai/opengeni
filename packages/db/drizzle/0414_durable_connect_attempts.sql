...
BEGIN
  IF roles IS NULL OR jsonb_typeof(roles) <> 'array' THEN
    RAISE EXCEPTION '0414 requires an explicit application database role list' USING ERRCODE = '55000';
  END IF;
  IF jsonb_array_length(roles) NOT BETWEEN 1 AND 16 OR EXISTS (
    ...
    RAISE EXCEPTION '0414 received invalid application database roles' USING ERRCODE = '55000';
  END IF;
  IF EXISTS (
    ...
    RAISE EXCEPTION '0414 requires all application database sessions stopped' USING ERRCODE = '55000';
  END IF;
END
$drain$;
