...
-- Slack delivery eligibility is decided by this SECURITY DEFINER claim before
-- the pump ever reads a page. A blocked child worker announces itself on its
-- Slack-originated parent as system.update.pending, and a budget or
-- continuation-cap goal pause as goal.paused, so both types must arm a claim.
-- The pump still decides what is worth posting: only a child_requires_action
-- notice and a limits / max_auto_continuations pause produce a message.
-- Rolling-safe: an old API image simply drains the extra claim without posting.
