...
BEGIN
  ...
  IF (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1 THEN
    RAISE EXCEPTION '0436 self membership guard drift' USING ERRCODE = '55000';
  END IF;
  ...
  IF (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1 THEN
    RAISE EXCEPTION '0436 self membership scope drift' USING ERRCODE = '55000';
  END IF;
  ...
  IF (length(definition) - length(replace(definition, anchor, ''))) / length(anchor) <> 1
    ...
    RAISE EXCEPTION '0436 private create policy drift' USING ERRCODE = '55000';
  END IF;
  EXECUTE replace(definition, anchor,
