...
BEGIN
  insertion := $gate$
  -- 0494 exact accepted account gate, inside the canonical lifecycle locks.
  ...
  IF (length(definition)-length(replace(definition,anchor,'')))/length(anchor) <> 1
    ...
    RAISE EXCEPTION '0494 connection resolver prerequisite drift' USING ERRCODE = '55000';
  END IF;
...
BEGIN
  ...
  IF strpos(definition,'personal_connection_delegations jsonb') = 0
    ...
    RAISE EXCEPTION '0494 outbox claim prerequisite drift' USING ERRCODE = '55000';
  END IF;
