...
-- Session creator/turn initiator provenance is owned by the earlier
-- session_turn_initiators migration. Memory consumes that immutable fact and
-- never infers a human from a service or legacy creator.
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'opengeni_app') THEN
    EXECUTE format(
      'GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE %I.knowledge_memory_relationships, %I.knowledge_memory_operations TO opengeni_app',
      current_schema(),
      current_schema()
    );
    EXECUTE format(
      'GRANT SELECT, INSERT ON TABLE %I.knowledge_memory_deletion_audits, %I.knowledge_memory_export_audits TO opengeni_app',
      current_schema(),
      current_schema()
    );
  END IF;
