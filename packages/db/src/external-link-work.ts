...
type Source = { kind: "direct" } | { kind: "causal" | "child"; turnId: string }
...
export async function externalLinkWorkSnapshotIsLive(db: Database, raw: unknown): Promise<boolean> {
  ...
  try {
    const identity = await ensureExternalIdentity(db, { accountId: snapshot.actor.accountId, ...snapshot.identity });
    if (identity.id !== snapshot.actor.externalIdentityId || identity.subjectId !== snapshot.actor.externalSubjectId ||
      identity.authorizationRevision !== snapshot.actor.externalAuthorizationRevision) return false;
    const resolved = await resolveExternalIdentityLink(db, { identity, linkId: snapshot.actor.linkId!, expectedRevision: snapshot.actor.linkRevision! });
    return resolved !== null && resolved.link.nativeSubjectId === snapshot.actor.effectiveSubjectId &&
      snapshot.permissions.every(permission => resolved.link.permissions.includes(permission) ||
...
export async function getExternalLinkTurnSnapshot(db: Database, scope: Scope, turnId: string): Promise<ExternalLinkWorkSnapshot | null> {
  return withWorkspaceRls(db, scope.workspaceId, async tx => {
    const [row] = await rawRows<{ canonical_snapshot: unknown }>(tx, sql`select canonical_snapshot
...
export async function getExternalLinkTurnAuthorization(db: Database, scope: Scope, turnId: string) {
  ...
  return { authorized: await externalLinkWorkSnapshotIsLive(db, snapshot), permissions: snapshot.permissions };
}

export async function captureExternalLinkTurnAuthority(db: Database, input: Scope & {
  sessionId: string; turnId: string; snapshot: ExternalLinkWorkSnapshot; source?: Source;
}): Promise<void> {
  const snapshot = ExternalLinkWorkSnapshot.parse(input.snapshot);
  const source = input.source ?? { kind: "direct" };
  if (snapshot.actor.accountId !== input.accountId) throw new Error("Linked turn organization mismatch");
  if (source.kind === "direct" && !await externalLinkWorkSnapshotIsLive(db, snapshot))
    throw new Error("Linked turn authority revoked");
  await withWorkspaceSubjectRls(db, input.workspaceId, snapshot.actor.effectiveSubjectId, async tx => {
    const existing = await getExternalLinkTurnSnapshot(tx, input, input.turnId);
    if (existing) {
      if (stableJson(existing) !== stableJson(snapshot)) throw new Error("Linked turn replay changed authority");
      return;
    }
...
export async function inheritExternalLinkTurnAuthority(db: Database, input: Scope & {
  sessionId: string; turnId: string; sourceTurnId: string; kind: "causal" | "child";
}): Promise<void> {
  ...
  await captureExternalLinkTurnAuthority(db, { ...input, snapshot, source: { kind: input.kind, turnId: input.sourceTurnId } });
}

export async function getExternalLinkTaskSnapshot(db: Database, input: Scope & {
...
export async function captureExternalLinkTaskAuthority(db: Database, task: ScheduledTask, raw: ExternalLinkWorkSnapshot): Promise<void> {
  const snapshot = ExternalLinkWorkSnapshot.parse(raw);
  if (task.accountId !== snapshot.actor.accountId) throw new Error("Linked task organization mismatch");
  await withWorkspaceSubjectRls(db, task.workspaceId, snapshot.actor.effectiveSubjectId, async tx => {
    const existing = await getExternalLinkTaskSnapshot(tx, { ...task, taskId: task.id, taskRevision: task.authorityRevision });
    if (existing) {
      if (stableJson(existing) !== stableJson(snapshot)) throw new Error("Linked task replay changed authority");
      return;
    }
...
export async function cloneExternalLinkTaskAuthority(db: Database, task: ScheduledTask, sourceRevision: number): Promise<void> {
  if (task.authorityRevision === sourceRevision) return;
  const snapshot = await getExternalLinkTaskSnapshot(db, { ...task, taskId: task.id, taskRevision: sourceRevision });
  if (snapshot) await captureExternalLinkTaskAuthority(db, task, snapshot);
}

export async function captureScheduledExternalLinkTurnAuthority(db: Database, input: Scope & {
  sessionId: string; turnId: string; runId: string;
}): Promise<void> {
  const run = await withWorkspaceRls(db, input.workspaceId, async tx => {
    const [row] = await rawRows<{ task_id: string; task_authority_revision: number | string }>(tx,
  ...
  const snapshot = await getExternalLinkTaskSnapshot(db, { ...input, taskId: run.task_id, taskRevision });
  if (snapshot) await captureExternalLinkTurnAuthority(db, { ...input, snapshot,
