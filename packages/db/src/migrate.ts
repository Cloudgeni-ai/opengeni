...
async function executeMigrationFile(
  ...
  if (sqlText.includes(SKILL_METADATA_MIGRATION_MARKER)) {
    if (file !== "0427_unified_skill_lifecycle.sql")
      throw new Error("Skill metadata stage is restricted to migration 0427");
    const parts = sqlText.split(SKILL_METADATA_MIGRATION_MARKER);
    if (parts.length !== 2) throw new Error("0427 requires exactly one Skill metadata stage");
