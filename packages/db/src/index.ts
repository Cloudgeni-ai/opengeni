...
export async function createKnowledgeMemory(db: Database, input: CreateKnowledgeMemoryInput): Promise<KnowledgeMemory> {
  ...
  const [row] = await db.insert(schema.knowledgeMemories).values({
...
export async function updateKnowledgeMemory(db: Database, memoryId: string, input: UpdateKnowledgeMemoryInput): Promise<KnowledgeMemory> {
  ...
  const reviewedBy = input.reviewedBy === null
    ...
    : input.reviewedBy !== undefined
      ...
      : undefined;
  const [row] = await db.update(schema.knowledgeMemories).set({
    ...
    updatedAt: new Date(),
  }).where(eq(schema.knowledgeMemories.id, memoryId)).returning();
  if (!row) {
...
export async function getKnowledgeMemory(db: Database, memoryId: string): Promise<KnowledgeMemory | null> {
  const [row] = await db.select().from(schema.knowledgeMemories).where(eq(schema.knowledgeMemories.id, memoryId)).limit(1);
  return row ? mapKnowledgeMemory(row) : null;
}

export async function listKnowledgeMemories(db: Database, options: ListKnowledgeMemoryOptions = {}): Promise<KnowledgeMemory[]> {
  ...
  if (cleanDbString(options.scope)) {
    conditions.push(eq(schema.knowledgeMemories.scope, cleanDbString(options.scope)!));
  }
  ...
  const rows = conditions.length > 0
    ? await db.select().from(schema.knowledgeMemories).where(and(...conditions)).orderBy(desc(schema.knowledgeMemories.updatedAt)).limit(limit)
    : await db.select().from(schema.knowledgeMemories).orderBy(desc(schema.knowledgeMemories.updatedAt)).limit(limit);
