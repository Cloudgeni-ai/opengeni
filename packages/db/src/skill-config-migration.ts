...
/** Maintenance-only: caller holds the 0427 owner window and drained runtime fence. */
export async function migrateLegacySkillConfigurations(tx: postgres.TransactionSql): Promise<void> {
  ...
  if (blockers.length)
    throw new Error(
      `0427 Skill configuration preflight: ${blockers.length} repair-required sources; drain or explicitly replace pinned sources before retry: ${blockers.slice(0, 30).join(", ")}`,
    );
  for (const { source, converted } of changes) {
    ...
    if (updated.length !== 1)
      throw new Error(`0427 Skill configuration CAS conflict: ${source.kind}:${source.id}`);
  }
}
