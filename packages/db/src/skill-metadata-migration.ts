...
/** Exact runner seam for 0432, not a general callback or runtime capability. */
...
export async function stageSkillMetadataMigration(tx: postgres.TransactionSql): Promise<void> {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      throw new Error(
        `Skill ${source.source_kind}:${source.source_id} needs explicit frontmatter repair before 0432`,
        { cause },
      );
    }
