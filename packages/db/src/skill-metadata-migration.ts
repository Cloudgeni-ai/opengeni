...
/** Exact runner seam for 0433, not a general callback or runtime capability. */
...
export async function stageSkillMetadataMigration(tx: postgres.TransactionSql): Promise<void> {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      throw new Error(
        `Skill ${source.source_kind}:${source.source_id} needs explicit frontmatter repair before 0433`,
        { cause },
      );
    }
