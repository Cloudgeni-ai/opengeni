...
export type CreateDbOptions = {
   ...
   */
  searchPath?: string;
  /** Immutable target schema captured from the server-owned connection setup. */
  targetSchema?: string;
 ...
 */
type DbBinding = { rlsStrategy: RlsStrategy; targetSchema: string; userLookup?: UserLookup };
...
export function createDb(databaseUrl: string, options: CreateDbOptions = {}): DbClient {
  ...
  dbBindings.set(db as unknown as object, {
    ...
    targetSchema: options.targetSchema ?? targetSchemaFromSearchPath(options.searchPath),
...
export function registerDbBinding(
  db: Database,
  binding: { rlsStrategy?: RlsStrategy; targetSchema?: string; userLookup?: UserLookup },
): void {
  dbBindings.set(db as unknown as object, {
    rlsStrategy: binding.rlsStrategy ?? "force",
    targetSchema: binding.targetSchema ?? "public",
...
export function databaseTargetSchemaFor(db: Database): string {
  return dbBindings.get(db as unknown as object)?.targetSchema ?? "public";
}

function targetSchemaFromSearchPath(searchPath: string | undefined): string {
  const first = searchPath?.split(",", 1)[0]?.trim();
  if (!first) return "public";
  const unquoted = first.replace(/^"|"$/g, "");
  if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(unquoted)) {
    throw new Error("createDb searchPath must begin with a valid target schema");
  }
  return unquoted;
}
