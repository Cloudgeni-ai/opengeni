...
describe("runtime event normalization", () => {
  ...
  test("intrinsic filesystem tools recheck live attempt authority before touching the sandbox", async () => {
    ...
    const [capability] = buildAgentCapabilities(testSettings(), [], {
      structuredToolTransport: false,
      authorizeAttemptExecution: () => { if (!live) throw new Error("Link revoked"); },
    });
    const bound = (capability as any).bind({ createEditor: () => ({}), viewImage: async () => {
      reads++; return { type: "text", text: "fixture" };
    } });
    ...
    expect(reads).toBe(1);
    live = false;
    await expect(tool.invoke(undefined, '{"path":"/tmp/fixture.png"}')).rejects.toThrow("Link revoked");
    expect(reads).toBe(1);
  });
  ...
  test("live execution fence rechecks a prepared call before consuming approval or executing tools", async () => {
    ...
    const prepared = await prepareAgentTools(testSettings(), [], {
      ...
      attemptId: "55555555-5555-4555-8555-555555555555",
      executionGeneration: 1,
      authorizeAttemptExecution: async () => { if (!live) throw new Error("Link revoked"); },
      attemptToolDefinitions: [{
        identity: { serverId: "fixture", toolName: "execute" }, modelName: "fixture__execute",
        codemodePath: ["fixture", "execute"], inputSchema: { type: "object", additionalProperties: false },
        source: "interaction", approval: "none",
        lifecycle: { prepare: async () => ({ begin: async () => { began++; } }) },
        execute: async () => { executed++; return { content: [{ type: "text", text: "Executed" }] }; },
      }],
    });
    try {
      const call = await prepared.attemptToolEnvironment!.prepareCall({
        operationId: crypto.randomUUID(), catalogDigest: prepared.attemptToolCatalog!.digest,
        identity: { serverId: "fixture", toolName: "execute" }, arguments: {},
        caller: { kind: "codemode", subjectId: "agent:test" },
      });
      expect(began).toBe(0);
      live = false;
      await expect(call.execute()).rejects.toThrow("Link revoked");
      expect(began).toBe(0); expect(executed).toBe(0);
      live = true;
      await call.execute();
      expect(began).toBe(1); expect(executed).toBe(1);
      live = false;
      await expect(prepared.attemptToolEnvironment!.call({
        operationId: crypto.randomUUID(), catalogDigest: prepared.attemptToolCatalog!.digest,
        identity: { serverId: "fixture", toolName: "execute" }, arguments: {},
        caller: { kind: "codemode", subjectId: "agent:test" },
  ...
  test("durable host broker revocation at the physical MCP fence sends no provider request", async () => {
    ...
    const authNeeded: ToolAuthNeededPayload[] = [];
    const resolveCredential = buildHostConnectionTokenResolver(async (request) => {
      credentialCalls++;
      return {
        status: "ok",
