...
test("Site skill composition needs no writable application directory", () => {
  ...
  try {
    ...
    const result = Bun.spawnSync(
      ...
      if (native.length !== 8) throw new Error("missing native Skill artifacts");
      ...
      if (JSON.stringify(defaults) !== JSON.stringify(["document-parsing", "opengeni-help", "opengeni-visualize"]))
