...
describe("Vercel AI Gateway request fence", () => {
  ...
  test("preserves xhigh reasoning for Gemini 3.7 Flash and GLM 5.3 requests", async () => {
    for (const model of ["google/gemini-3.7-flash", "zai/glm-5.3"]) {
      let captured: Record<string, unknown> | null = null;
      const routed = vercelGatewayRoutingFetch("vercel-gateway-managed", (async (
        _input: RequestInfo | URL,
        init?: RequestInit,
      ...
      await routed("https://ai-gateway.vercel.sh/v1/responses", {
        method: "POST",
        body: JSON.stringify({ model, reasoning: { effort: "xhigh" } }),
      });
      expect(captured?.reasoning).toEqual({ effort: "xhigh" });
      expect(JSON.stringify(captured)).not.toContain('"max"');
    }
  });
  ...
  test("does not alter incomplete Kimi or complete DeepSeek, Gemini, or GLM call/result batches", async () => {
    ...
    for (const value of [
      ...
      {
        model: "deepseek/deepseek-v4-flash-0731",
        input: grouped.map((item) => ({ ...item })),
      },
      {
        model: "google/gemini-3.7-flash",
        input: grouped.map((item) => ({ ...item })),
      },
      { model: "zai/glm-5.3", input: grouped.map((item) => ({ ...item })) },
    ]) {
      let captured: Record<string, unknown> | null = null;
      const routed = vercelGatewayRoutingFetch("vercel-gateway-managed", (async (
        _input: RequestInfo | URL,
