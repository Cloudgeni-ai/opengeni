...
export function renderWorkspaceGovernanceContext(
  ...
  const rendered = [
    ...
    "Use knowledge_save for useful facts, decisions, incidents, and outcomes; use instruction_policy_save only for minimal universal workspace rules. Read the current instruction before every change, preserve unrelated rules, append new rules by default, use an exact anchored edit for updates or removals, and replace the complete instruction only when explicitly requested. Use the Skill management tools for reusable procedures. The accepted Agent learning policy governs each destination. Pending changes remain inactive and do not block the task.",
