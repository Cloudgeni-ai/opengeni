...
description: Create, inspect, edit, import, and export durable OpenGeni presentation artifacts and explicit PPTX file boundaries. Use for decks, slides, masters, layouts, rich text, tables, charts, images, groups, notes, and visual presentation QA.
---
...
- To begin empty, call `editable_artifact_create` with modality `presentation`.
- To begin from a ready workspace PPTX, call `editable_artifact_import` with its `fileId`.
  The source remains immutable provenance; the returned artifact becomes the
  working deck.
- Use a standalone local PPTX only when the user explicitly asks for local-file
...
3. Make the smallest coherent edit. One `editable_artifact_apply` call is one atomic
   command batch. Use inspected ids for existing objects and
...
- Durable presentation commands do not yet author animations or arbitrary
  OOXML relationships. Never hide a gap by changing mutable truth back to a
  local PPTX.
