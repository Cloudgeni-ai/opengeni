...
const encoded = (value: unknown, pretty = false) =>
  Buffer.from(JSON.stringify(value, null, pretty ? 2 : undefined));
...
test("Garage binary pins trace to the existing compose image's exact manifest chain", async () => {
  ...
  for (const arch of ["amd64", "arm64"] as const) {
    ...
    const descriptor = provenance.index.manifests.find(
    ...
    const asset = assetFor(
      "garage",
      detectTarget("linux", arch === "amd64" ? "x64" : "arm64", "6"),
    );
...
function timestampsOnly(pax: Buffer) {
  for (let offset = 0; offset < pax.length;) {
    ...
    if (space < 0 || !/^[1-9][0-9]*$/.test(pax.subarray(offset, space).toString()))
      throw new Error("Invalid PAX length");
    const length = Number(pax.subarray(offset, space).toString());
    if (
      !Number.isSafeInteger(length) ||
      length <= space - offset + 1 ||
      offset + length > pax.length
    )
      throw new Error("Invalid PAX bound");
    const record = pax.subarray(space + 1, offset + length).toString();
    if (!/^(atime|ctime|mtime)=[0-9]+(?:\.[0-9]+)?\n$/.test(record))
      throw new Error("Only PAX timestamps permitted (no path/link/size overrides)");
    offset += length;
  }
}
...
function garageFromPinnedLayer(tar: Buffer): Buffer {
  ...
  for (const [expectedName, expectedType] of expected) {
    ...
    if (
      number(148, 156) !==
      header.reduce((sum, byte, i) => sum + (i >= 148 && i < 156 ? 32 : byte), 0)
    )
      throw new Error("Tar checksum mismatch");
    const prefix = field(345, 500);
    const name = `${prefix ? `${prefix}/` : ""}${field(0, 100)}`;
    const type = field(156, 157);
    if (name !== expectedName || type !== expectedType || field(157, 257) !== "")
      throw new Error("Unexpected layer entry; no links or whiteouts accepted");
    const size = number(124, 136);
    if (!Number.isSafeInteger(size) || size > 100 * 1024 * 1024 || offset + 512 + size > tar.length)
      throw new Error("Layer size bound");
  ...
  if (!binary || tar.length - offset < 1024 || !tar.subarray(offset).every((b) => b === 0))
    throw new Error("Unexpected trailing layer content");
  return binary;
}
  ...
  try {
    for (let redirects = 0; redirects <= 4; redirects++) {
      ...
      if (
        ...
        throw new Error("Unexpected OCI origin");
      const headers: Record<string, string> = {
        Accept:
          "application/vnd.docker.distribution.manifest.list.v2+json, application/vnd.docker.distribution.manifest.v2+json",
      };
      if (parsed.hostname === "registry-1.docker.io" && token)
      ...
      if (!response.ok || !response.body) {
        await response.body?.cancel();
        throw new Error(`OCI HTTP ${response.status}`);
      }
...
test.skipIf(process.env.OPENGENI_VERIFY_GARAGE_PROVENANCE !== "1")(
  "live: both native Garage binaries match the digest-verified official OCI image",
  async () => {
    const auth = JSON.parse(
      (
        await registryGet(
          "https://auth.docker.io/token?service=registry.docker.io&scope=repository:dxflrs/garage:pull",
        )
      ).toString(),
    ) as { token: string };
    if (typeof auth.token !== "string" || !auth.token)
      throw new Error("Missing public registry token");
    ...
    for (const arch of ["amd64", "arm64"] as const) {
      ...
      const configBytes = await registryGet(
        `${base}/blobs/${target.manifest.config.digest}`,
        auth.token,
      );
