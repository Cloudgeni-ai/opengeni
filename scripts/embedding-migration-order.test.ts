...
test("embedding migrations append after main's published 0431 without duplicate ordinals", async () => {
  const contract = await buildSchemaContract();
  const tail = contract.migrations.filter((m) => Number(m.path.slice(0, 4)) >= 432);
  expect(tail).toHaveLength(21);
  expect(tail.map((m) => Number(m.path.slice(0, 4)))).toEqual(
    Array.from({ length: 21 }, (_, i) => 432 + i),
  );
  expect(contract.latestMigration).toBe("0452_canonical_session_scope_subject.sql");
