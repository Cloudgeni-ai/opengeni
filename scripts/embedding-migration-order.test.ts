...
test("embedding migrations append after main's published 0425 without duplicate ordinals", async () => {
  const contract = await buildSchemaContract();
  const tail = contract.migrations.filter((m) => Number(m.path.slice(0, 4)) >= 426);
  expect(tail).toHaveLength(20);
  expect(tail.map((m) => Number(m.path.slice(0, 4)))).toEqual(
    Array.from({ length: 20 }, (_, i) => 426 + i),
  );
