...
test("embedding migrations append after main's published 0422 without duplicate ordinals", async () => {
  const contract = await buildSchemaContract();
  const tail = contract.migrations.filter((m) => Number(m.path.slice(0, 4)) >= 423);
  expect(tail).toHaveLength(20);
  expect(tail.map((m) => Number(m.path.slice(0, 4)))).toEqual(
    Array.from({ length: 20 }, (_, i) => 423 + i),
  );
  expect(contract.latestMigration).toBe("0442_social_connection_versions.sql");
  ...
  expect(contract.migrations.find((m) => m.path.startsWith("0422_"))?.path).toBe(
