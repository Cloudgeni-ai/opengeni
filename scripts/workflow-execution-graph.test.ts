...
describe("workflow execution graph manifest", () => {
  test("the checked-in manifest exactly describes the repository graph", async () => {
    ...
    expect(inspection.manifest.workflows).toHaveLength(24);
    expect(inspection.manifest.actions).toHaveLength(2);
    expect(inspection.manifest.uncappedRuns).toHaveLength(210);
  ...
  test("removing any one of the 27 approved caps invalidates the committed graph", async () => {
