...
# Managed sandbox desktop and terminal viewers need the same stable stream
# authority even when Connected Machines are disabled. A relay-only secret
# cannot sign those viewer grants. Preserve a configured stream/delegation key;
# persist a missing local/test key so independently restarted roles agree.
if [ "${OPENGENI_SANDBOX_DESKTOP_ENABLED:-false}" = "true" ] ||
  [ "${OPENGENI_SANDBOX_TERMINAL_ENABLED:-true}" = "true" ] ||
  { [ "${OPENGENI_SANDBOX_SELFHOSTED_ENABLED:-false}" = "true" ] &&
    [ -z "${OPENGENI_SELFHOSTED_RELAY_TOKEN_SECRET:-}" ]; }; then
  case "${OPENGENI_ENVIRONMENT:-local}" in
  local | test)
    if [ -z "${OPENGENI_STREAM_TOKEN_SECRET:-}" ] &&
      ...
      {
        printf '\n%s\n' '# Generated by scripts/dev-stack.sh for local sandbox streams.'
        printf 'OPENGENI_STREAM_TOKEN_SECRET=%s\n' "$OPENGENI_STREAM_TOKEN_SECRET"
      } >>.env
      chmod 600 .env
      echo "Generated and persisted a local sandbox stream secret in .env."
    fi
    ;;
  esac
fi
...
if [ "$OPENGENI_DEV_BACKEND" = "native" ]; then
  ...
  OPENGENI_OBJECT_STORAGE_FIXTURE="$(bun scripts/dev-native-storage.ts resolve \
    "$(pwd)/.opengeni/native/${COMPOSE_PROJECT_NAME}" "${OPENGENI_OBJECT_STORAGE_FIXTURE:-}")"
else
...
if [ "$OPENGENI_OBJECT_STORAGE_FIXTURE" = "minio" ]; then
  ...
  if [ "$OPENGENI_DEV_BACKEND" = "native" ]; then
    choose_port OPENGENI_GARAGE_RPC_HOST_PORT 3901
  fi
