...
describe("backend-aware read-only preflight", () => {
  test("Docker checks daemon, Compose, Buildx, but not native infrastructure", async () => {
    ...
    expect(await collectDevelopmentPrerequisites({ ...prebuilt, environment: {} }, host)).toEqual({ backend: "docker", errors: [] });
  ...
  test("auto falls back to complete native checks when daemon probe fails", async () => {
    ...
    host.probe = (command, args) => command === "docker" ? { ok: false, stdout: "secret" } : probe(command, args);
  ...
  test("explicit Docker does not fall back and reports all Docker probe failures", async () => {
    ...
    const result = await collectDevelopmentPrerequisites({ ...prebuilt, environment: { OPENGENI_DEV_BACKEND: "docker" } }, host);
  ...
  test("verified prebuilt runtime skips Rust/cc; a separate source relay still requires them", async () => {
    const { host } = fixture({ which: (command) => ["rustup", "cc", "cargo"].includes(command) ? null : `/bin/${command}` });
    expect((await collectDevelopmentPrerequisites({ ...prebuilt, environment: {} }, host)).errors).toEqual([]);
    const result = await collectDevelopmentPrerequisites({ ...prebuilt, relayRuntime: "source-build", environment: {} }, host);
    expect(result.errors).toHaveLength(3);
    expect(result.errors.join("\n")).toContain("relay");
  });
  ...
  test("MinIO is explicit, pinned, and does not require Garage", async () => {
    const { host, commands } = fixture();
    const options = { ...prebuilt, environment: { OPENGENI_DEV_BACKEND: "native", OPENGENI_OBJECT_STORAGE_FIXTURE: "minio" } };
  ...
  test("invalid selectors do not print their possibly sensitive values", async () => {
    const { host } = fixture();
    const result = await collectDevelopmentPrerequisites({ ...prebuilt, environment: { OPENGENI_DEV_BACKEND: "token=private", OPENGENI_OBJECT_STORAGE_FIXTURE: "token=private" } }, host);
  ...
  test("does not accept outdated NATS, wrong PostgreSQL, or a Temporal CLI without start-dev", async () => {
    ...
    host.probe = (command, args) => {
      ...
      if (command === "pg_config" && args[0] === "--version") return { ok: true, stdout: "PostgreSQL 16.2" };
  ...
  test("remote/disabled sandboxes do not require Buildx", async () => {
    const { host, commands } = fixture();
    await collectDevelopmentPrerequisites({ ...prebuilt, environment: { OPENGENI_SANDBOX_BACKEND: "none" } }, host);
  ...
  test("real probe bounds hung commands, caps output, and suppresses stderr", async () => {
    ...
    expect(await host.probe(process.execPath, ["-e", "setInterval(() => {}, 1000)"])).toEqual({ ok: false, stdout: "" });
    expect(Date.now() - before).toBeLessThan(4500);
    expect(await host.probe(process.execPath, ["-e", "console.error('sensitive'); process.exit(1)"])).toEqual({ ok: false, stdout: "" });
    expect(await host.probe(process.execPath, ["-e", "console.log('x'.repeat(100000))"])).toEqual({ ok: false, stdout: "" });
    expect(await host.probe("/nonexistent/opengeni-preflight-test", [])).toEqual({ ok: false, stdout: "" });
  }, 6000);
