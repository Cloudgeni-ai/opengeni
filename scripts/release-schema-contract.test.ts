...
describe("release schema contract", () => {
  ...
  test("registers forward migrations in order after published history", async () => {
    ...
    expect(completeSourceContract.latestMigration).toBe("0426_unified_skill_lifecycle.sql");
    expect(
      completeSourceContract.migrations.find(
        (migration) => migration.path === "0426_unified_skill_lifecycle.sql",
      ),
    ).toMatchObject({ deploymentMode: "maintenance" });
  ...
  test("registers forward migrations without repinning host-export history", async () => {
    ...
    const automaticSessionTitleMigrationPaths = new Set([
      ...
      "0426_unified_skill_lifecycle.sql",
    ]);
    ...
    if (unifiedSkillLifecycle)
      completeSourceContract = {
        ...completeSourceContract,
        latestMigration: "0426_unified_skill_lifecycle.sql",
      };
    expect(completeSourceContract).toMatchObject({
      ...
      ...(unifiedSkillLifecycle ? { latestMigration: "0426_unified_skill_lifecycle.sql" } : {}),
    });
