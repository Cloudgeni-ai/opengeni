...
describe("release schema contract", () => {
  ...
  test("registers forward migrations in order after published history", async () => {
    ...
    expect(completeSourceContract.latestMigration).toBe("0424_scheduled_task_creator_policy.sql");
    expect(
      completeSourceContract.migrations.find(
        (migration) => migration.path === "0424_scheduled_task_creator_policy.sql",
      ),
    ).toMatchObject({ deploymentMode: "rolling" });
    expect(
      completeSourceContract.migrations.find(
        (migration) => migration.path === "0423_session_agent_access_scope.sql",
      ),
    ).toMatchObject({ deploymentMode: "rolling" });
    expect(
      completeSourceContract.migrations.find(
  ...
  test("registers forward migrations without repinning host-export history", async () => {
    ...
    const automaticSessionTitleMigrationPaths = new Set([
      ...
      "0423_session_agent_access_scope.sql",
      "0424_scheduled_task_creator_policy.sql",
    ]);
    ...
    if (sessionAgentAccessScope)
      completeSourceContract = {
        ...completeSourceContract,
        latestMigration: "0423_session_agent_access_scope.sql",
      };
    if (scheduledTaskCreatorPolicy)
      completeSourceContract = {
        ...completeSourceContract,
        latestMigration: "0424_scheduled_task_creator_policy.sql",
      };
