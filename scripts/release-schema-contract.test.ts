...
describe("release schema contract", () => {
  test("installs the workspace activity initializer before its conflict-safe backfill", async () => {
    const migration = await readFile(
      join(
        import.meta.dir,
        "../packages/db/drizzle/0214_session_activity_commit_gate.sql",
      ),
      "utf8",
    );
  ...
  test("classifies the Codex quota owning-human cutover as maintenance-only", async () => {
    ...
    expect(
      contract.migrations.find(
        (migration) =>
          migration.path === "0065_codex_subscription_overview.sql",
      ),
  ...
  test("is deterministic across creation order and classifies only executable SQL migrations", async () => {
    ...
    expect(await buildSchemaContract(first)).toEqual(
      await buildSchemaContract(second),
    );
  ...
  test("changes when either a path or file content changes", async () => {
    ...
    expect((await buildSchemaContract(changedContent)).sha256).not.toBe(
      baselineHash,
    );
    expect((await buildSchemaContract(changedPath)).sha256).not.toBe(
      baselineHash,
    );
  });
  ...
  test("registers forward migrations without repinning host-export history", async () => {
    ...
    const automaticSessionTitlePolicyFence =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0353_automatic_session_title_policy_fence.sql",
      );
    const automaticSessionTitleQuarantineIndex =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0354_automatic_session_title_quarantine_index.sql",
      );
    const automaticSessionTitleQuarantine =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0355_automatic_session_title_quarantine.sql",
      );
    const sessionVariableSetAttachments =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0352_session_variable_set_attachments.sql",
      );
    const setBasedInsightsSessionVisibility =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0356_set_based_insights_session_visibility.sql",
      );
    ...
    const insightsForceRlsReadCapability =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0359_insights_force_rls_read_capability.sql",
      );
    const organizationIdentityConfirmationPrompt =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0360_organization_identity_confirmation_prompt.sql",
      );
    const rememberKnowledgeMemoryMaterialization =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
    ...
    const organizationRecoveryCustody = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0363_organization_recovery_custody.sql",
    );
    const workspaceLearningPolicySnapshotLockOrder =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0364_workspace_learning_policy_snapshot_lock_order.sql",
      );
    const permissionScopedWorkClaims = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0365_permission_scoped_work_claims.sql",
    );
    const sessionDiscoveryTitleSearchIndex =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0366_session_discovery_title_search_index.sql",
      );
    const sessionDiscoveryGoalSearchIndex =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0367_session_discovery_goal_search_index.sql",
      );
    const sessionDiscoveryClaimSearchIndex =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0368_session_discovery_claim_search_index.sql",
      );
    const localHumanPersonalAuthority = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0369_local_human_personal_authority.sql",
    );
    const workspaceMemberManagementScope =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0370_workspace_member_management_scope.sql",
      );
    const workspaceMemberCandidateInventory =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0371_workspace_member_candidate_inventory.sql",
      );
    const orderedVariableSetRuntimeAuthority =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0372_ordered_variable_set_runtime_authority.sql",
      );
    const sandboxSharedPreparation = completeSourceContract.migrations.some(
      (migration) => migration.path === "0373_sandbox_shared_preparation.sql",
    );
    const sessionEventCursors = completeSourceContract.migrations.some(
      (migration) => migration.path === "0374_session_event_cursors.sql",
    );
    const sessionRecoveryObservability = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0375_session_recovery_observability.sql",
    );
    const organizationWorkspaceInventory =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0376_organization_workspace_inventory.sql",
      );
    const scopedRigHealthProjection = completeSourceContract.migrations.some(
      (migration) => migration.path === "0377_scoped_rig_health_projection.sql",
    );
    const scopedRigHealthAuditTimestamp =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0378_scoped_rig_health_audit_timestamp.sql",
      );
    const sessionEventRawLaneActivation =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0379_session_event_raw_lane_activation.sql",
      );
    const autonomousCompanyProfileAgentPolicy =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0380_autonomous_company_profile_agent_policy.sql",
      );
    const organizationCodexSubscriptionInheritance =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0381_organization_codex_subscription_inheritance.sql",
      );
    const organizationApiKeyProvenance = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0382_organization_api_key_provenance.sql",
    );
    const codexCooldownReconciliation = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0383_codex_cooldown_reconciliation.sql",
    );
    const codexCooldownRevisionGuardPrivileges =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0384_codex_cooldown_revision_guard_privileges.sql",
      );
    const connectedMachineLegacyTildeWorkdirs =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0385_connected_machine_legacy_tilde_workdirs.sql",
      );
    const localOrganizationAdministration =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0386_local_organization_administration.sql",
      );
    const boundHostExportSessionPayloads =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0387_bound_host_export_session_payloads.sql",
      );
    const sandboxProviderDeadlineInteractions =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0388_sandbox_provider_deadline_interactions.sql",
      );
    const modelCatalogAndGatewayCustomModels =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0389_model_catalog_and_gateway_custom_models.sql",
      );
    const continuabilityProjectionPerformance =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0390_continuability_projection_performance.sql",
      );
    const automaticSessionTitleMigrationPaths = new Set([
      ...
      "0390_continuability_projection_performance.sql",
    ]);
    const migrationsBeforeAutomaticSessionTitles =
      completeSourceContract.migrations.filter(
    ...
    expect(completeSourceContract).toMatchObject({
      fileCount:
        ...
        (workspaceLearningPolicySnapshotLockOrder ? 1 : 0) +
        (permissionScopedWorkClaims ? 1 : 0) +
        (sessionDiscoveryTitleSearchIndex ? 1 : 0) +
        (sessionDiscoveryGoalSearchIndex ? 1 : 0) +
        (sessionDiscoveryClaimSearchIndex ? 1 : 0) +
        ...
        (continuabilityProjectionPerformance ? 1 : 0),
      latestMigration: continuabilityProjectionPerformance
        ? "0390_continuability_projection_performance.sql"
  ...
  test("preserves published host-export history and appends the forward repair", async () => {
    ...
    const companyBrainMigrationPaths = [
      ...
      completeSourceContract.migrations.some(
        (migration) => migration.path === path,
      ),
    );
    const companyBrainMigrations = new Map(
      completeSourceContract.migrations.map((migration) => [
        migration.path,
        migration,
      ]),
    );
    const goalPersistence = companyBrainMigrations.get(
      "0238_goal_persistence_policy.sql",
    );
    if (goalPersistence) {
      expect(goalPersistence).toMatchObject({ deploymentMode: "rolling" });
    }
    const taskTreeNotes = companyBrainMigrations.get(
      "0239_task_tree_notes.sql",
    );
    if (taskTreeNotes) {
    ...
    const atomicConnectedMachineAttachments =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0338_atomic_connected_machine_attachments.sql",
      );
    const documentAuthorityReclassification =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0339_document_authority_reclassification.sql",
      );
    const tenancyBackfillActivationEvidence =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0340_tenancy_backfill_activation_evidence.sql",
      );

    const slackRoutingProbeFence = completeSourceContract.migrations.some(
      (migration) =>
        migration.path === "0341_slack_routing_probe_organization_fence.sql",
    );

    const slackRoutePromptSinglePending =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0342_slack_route_prompt_single_pending.sql",
      );
    const personalDocumentForceRlsRepair =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0343_personal_document_force_rls_lock_repair.sql",
      );
    const privateSessionVisibilityTransitionGate =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0344_private_session_visibility_transition_gate.sql",
      );
    const tenantScopedSessionTenancyFence =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0345_tenant_scoped_session_tenancy_fence.sql",
      );
    const documentMigrationAuditSurface =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0346_document_migration_audit_surface.sql",
      );
    const connectionAuthorityConvergenceEvidence =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0347_connection_authority_convergence_evidence.sql",
    ...
    const greenfieldSessionTenancyActivation =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0349_greenfield_session_tenancy_activation.sql",
      );
    const organizationSharedWorkspaceAdministration =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path ===
          "0350_organization_shared_workspace_administration.sql",
    ...
    const organizationUserSetupDelivery =
      completeSourceContract.migrations.some(
        (migration) =>
          migration.path === "0351_organization_user_setup_delivery.sql",
      );
    const managedAuthSessionSets = completeSourceContract.migrations.some(
      (migration) => migration.path === "0362_managed_auth_session_sets.sql",
    );
    ...
    expect(
      ...
      sha256:
        "7167987a841c839601069749768f37032c2890136e42128116954c4d92baad22",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0336_atomic_session_fork_visibility.sql",
      ),
    ).toMatchObject({
      sha256:
        "75604a1d9cce1bf698184e87d4f0405493e200e06378938b2f10adbfaa565f2d",
      deploymentMode: "rolling",
    });
    const forwardMigrationPaths = [
      ...companyBrainMigrationPaths,
      ...appendedMigrationPaths,
    ];
    ...
    const transitionReaper = completeSourceContract.migrations.find(
      (migration) =>
        migration.path === "0237_interaction_transition_reaper.sql",
    );
    if (transitionReaper) {
    ...
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0314_unregistered_organization_invitations.sql",
      ),
    ).toMatchObject({ deploymentMode: "maintenance" });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0348_named_signup_and_user_setup.sql",
      ),
    ).toMatchObject({ deploymentMode: "maintenance" });
    expect(
      completeSourceContract.migrations.find(
    ...
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0331_managed_organization_creation.sql",
      ),
    ).toMatchObject({
      sha256:
        "62912af210c1db18c7b58de8ad2c233f2fc6759a6ef4509c8e6352d75dd0641f",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "6de1a4fc6f0dc0e67852cef996b6d0e01326d180c67d188cb7fcb9c704bc5cdd",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0333_session_turn_prompt_routing.sql",
      ),
    ).toMatchObject({
      sha256:
        "3eab54938c65c4db7af9ae3421045e6f44af01a27b06129093a58f1ae22ea05c",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0334_connected_machine_workspace_root.sql",
      ),
    ).toMatchObject({
      sha256:
        "6cf8700390b42b354ba66414a94a7d2d48ce4756110037e9b4f104bdd8d92a9c",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "0bd5490d1e7f2b9b9195c89d602478ffd023b6c035f7151dd2fc89e898daf053",
    ...
    expect(
      ...
      sha256:
        "1992505e5994cbef2d650b0eebae2a6c033b567ecbb9cf27301846c500dea66a",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "fbe4c79cb20c809767dad12e697ab6ad1becfc8b03eb314cbda38d6069a258f1",
      deploymentMode: "rolling",
    });
    ...
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0251_connected_machine_operation_policy.sql",
      ),
    ).toMatchObject({
      sha256:
        "a37e307de730bc47ccf4ed6bf517427ebb0d20199640f54e8e525ae1ce046663",
      deploymentMode: "maintenance",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "cdce8c6b6644b07c672918a94c3e0e01f09d771dc33a61e38ec91eec763bf0c1",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0305_personal_resource_grant_management.sql",
      ),
    ).toMatchObject({
      sha256:
        "b6c20178b35b279314872c7cc79048f028d2b1ed070a49f5ad9e0490cb8e5b0c",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0306_atomic_personal_resource_attachments.sql",
      ),
    ).toMatchObject({
      sha256:
        "4aa927065e39ecda0cbf118e9f861d728f8b213e4b72238e2cdbccea002e2af4",
      deploymentMode: "maintenance",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "c7ab1856c718dbb9c6097b9e04585d34fa5de557cc27b09466e5c0f3e6c20e19",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0240_enrollment_connection_authority.sql",
      ),
    ).toMatchObject({
      sha256:
        "5dd85b5f7bf5940a397cb67938b82e2b902a0529872a3d7158b7cfcfc04e885c",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "a13ddf193d04fbc5beac33f33641358f2486eef88744e06f3bfeb02366761da7",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0244_slack_app_home_refresh_queue.sql",
      ),
    ).toMatchObject({
      sha256:
        "f098df63a6ed21e88362faf0d6c5e36321604bb300ac155529adb9b17da30858",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0245_model_context_contribution_facts.sql",
      ),
    ).toMatchObject({
      sha256:
        "437bb07ffe12f9c714bd2a40d0ecd8ed9df1fd9003f4d057fe11101999841f40",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0246_integration_personal_instance_authority.sql",
      ),
    ).toMatchObject({
      sha256:
        "1717d5cdaa298501f20463eef43822a2b1421984f30cab7cb381c2773c505388",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "98b1e6059e955b7a8022ff45f977b44075a7f854828422e06d9879a8487d62f7",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0250_direct_retained_process_owner_liveness.sql",
      ),
    ).toMatchObject({
      sha256:
        "8ae147f5de31c4173758e8e64bbb2597e8a3fc49d983a7a518f229fb95ef90f5",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0252_scheduled_personal_resource_delegation.sql",
      ),
    ).toMatchObject({
      sha256:
        "ddc1c34835e4f5ac7ae5039e5c0dae5971d6e6284167eb9199151175f8766169",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "cfcffb54e01c41927aeed024a1ba610bac45ac3d5271a7c1b147f4beb28c2428",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0254_scoped_variable_set_authority.sql",
      ),
    ).toMatchObject({
      sha256:
        "e99064ac5acd73e79c3a75872d71a5c8710bcedbbd12fbe3eee989d7f167c8cf",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0255_company_brain_governed_write_proposals.sql",
      ),
    ).toMatchObject({
      sha256:
        "5d6527267b8de9cb9539e97a0cd30051dc9b2059fd5935261aa8c762d5d6a0d3",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "457f7ae4bdaaf6f65bc5245f5f3c0ee7b28de94aafce284bb80bfbb4fb690da4",
      deploymentMode: "maintenance",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0256_connection_authority_delegation.sql",
      ),
    ).toMatchObject({
      sha256:
        "669f96626b41fc0cf8c82914d1e39183925af1717510032fc70950ee7040ff84",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "e9349cb0f88673fcf602aba495e8ad90970c7d46ed1490acffc530baf2d6c484",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "e4b03de61786cd7f22fe203cb498142bdf55e3cda15b0065536967e54341d2b0",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0260_task_note_knowledge_promotion.sql",
      ),
    ).toMatchObject({
      sha256:
        "1f067ac286f94effc2a98bf5d016fab6e2d563054bbb3eafa358ce4b653191fe",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "c00d29214a9181301fa3076322992a2958ed4d017028a7956a52b97c930af8e6",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
      ...
      sha256:
        "4830b11f4a856ab1992924f68a6567649d93026b7a3093d9a2c66ee6a7b485bb",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0268_governed_learning_decision_receipts.sql",
      ),
    ).toMatchObject({
      sha256:
        "c76df472ad5bc0af0a17918b5b594b2013fb9a2ce6e7e35d429c1dd807bd3054",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0269_governed_learning_activation_controller.sql",
      ),
    ).toMatchObject({
      sha256:
        "6fa48ea195c33edae1a5cf8a857b832e34fb16b3fb73ac9d0fe1629f03a9892c",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0262_scoped_connected_machines_and_rigs.sql",
      ),
    ).toMatchObject({
      sha256:
        "87901ff0b301b010a18e04ddd2137f291554071c8c9f73bec9b52b69cadd1cb8",
      deploymentMode: "rolling",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0264_connection_authority_runtime_activation.sql",
      ),
    ).toMatchObject({
      sha256:
        "1f72cf8be5a791fb42a4bf7b19f81cef4cce9c7f92dfb1dc8626127a1a8b420c",
      deploymentMode: "maintenance",
    });
    expect(
      completeSourceContract.migrations.find(
        (migration) =>
          migration.path === "0273_scheduled_variable_set_materialization.sql",
      ),
    ).toMatchObject({
      sha256:
        "c1065c50461e8f273931766158b8a0011818e3fc72fecb221407c6e689152694",
      deploymentMode: "rolling",
    });
    const migrations = new Map(
      sourceContract.migrations.map((migration) => [migration.path, migration]),
    );
    const sessionVisibilityContractHash = (
      includesActivation: boolean,
    ): string | null => {
    ...
    const releaseSchemaContractHash = (
      includesActivation: boolean,
    ): string | null => {
    ...
    const currentMainContractHash = migrations.has(
                              ...
                              : migrations.has(
                                ...
                                : migrations.has(
                                  ...
                                  : migrations.has(
                                    ...
                                    : migrations.has(
                                          "0216_pack_component_ownership.sql",
                                        )
                                      ? "85a5f5320fd7c673bfe16240d4615b93ce635e9724d8f1bc467ce336e5c93022"
                                      : migrations.has(
                                            "0214_session_activity_commit_gate.sql",
                                          )
    ...
    const activationMigration = migrations.get(
    ...
    if (activationMigration) {
      ...
      expect(activationMigration).toMatchObject({
        sha256:
          "43945bc115ddf5e7b4b6e73a757c6bb63dde6929e1b3a89714c9cf330de87a12",
        deploymentMode: "rolling",
      });
      expect(
        migrations.get("0236_session_visibility_slack_policy.sql"),
      ).toMatchObject({
        sha256:
          "64f9beb146d973cc0a6ab9f8cdef29955ef9edb68ecc9b07756eda5414709299",
        deploymentMode: "rolling",
      });
      ...
      expect(contractWithoutActivation.sha256).toBe(
      ...
      sourceContract.latestMigration =
        contractWithoutActivation.latestMigration;
      sourceContract.sha256 = contractWithoutActivation.sha256;
    }
    expect(sourceContract.sha256).toBe(
      releaseSchemaContractHash(false) ?? currentMainContractHash,
    );
    const contract = {
      ...sourceContract,
    ...
    expect(
      migrations.get("0065_codex_subscription_overview.sql"),
    ).toMatchObject({
      deploymentMode: "maintenance",
    });
    ...
    expect(currentMainMigrations).toEqual(
      currentMainMigrations.length === 0
        ...
        : [
            "0105_session_turn_instructions.sql",
            "0106_session_attempt_mcp_approval_policies.sql",
            ...(currentMainMigrations.includes(
              "0107_host_export_lineage_contract.sql",
            )
              ? ["0107_host_export_lineage_contract.sql"]
              : []),
    ...
    expect(contract.fileCount).toBe(
      108 +
        ...
        (migrations.has("0131_slack_bot_install_and_post_idempotency.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0138_sandbox_checkpoint_artifacts_and_deadlines.sql")
          ? 1
          : 0) +
        (migrations.has("0139_codex_provider_artifact_invalidations.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0153_mcp_personal_connection_delegations.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0160_session_realtime_delegation_terminal.sql")
          ? 1
          : 0) +
        (migrations.has("0161_session_realtime_context_projection.sql")
          ? 1
          : 0) +
        (migrations.has("0162_session_realtime_connection_promotion.sql")
          ? 1
          : 0) +
        (migrations.has("0163_session_realtime_delegation_progress.sql")
          ? 1
          : 0) +
        ...
        (migrations.has(
          ...
          ? 1
          : 0) +
        (migrations.has("0170_resumable_transcription_recordings.sql")
          ? 1
          : 0) +
        (migrations.has("0170_session_control_wake_revision.sql") ? 1 : 0) +
        (migrations.has("0171_social_connection_subject_ownership.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0175_resumable_transcription_provider_deadline.sql")
          ? 1
          : 0) +
        (migrations.has("0176_lossless_canonical_json.sql") ? 1 : 0) +
        (migrations.has("0177_session_events_workspace_turn_type_index.sql")
          ? 1
          : 0) +
        (migrations.has("0178_permissioned_secret_reads.sql") ? 1 : 0) +
        (migrations.has("0179_slack_private_shortcut_delivery_gate.sql")
          ? 1
          : 0) +
        (migrations.has("0180_retained_screenshot_lifecycle_fences.sql")
          ? 1
          : 0) +
        (migrations.has("0181_connected_machine_removal.sql") ? 1 : 0) +
        (migrations.has("0182_connected_machine_remove_session_default.sql")
          ? 1
          : 0) +
        (migrations.has("0183_model_call_provider_cost_estimates.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0188_image_generation_retention_failure.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0195_editable_artifact_import_authorization.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0210_browser_auth_network_interventions.sql")
          ? 1
          : 0) +
        ...
        (migrations.has("0225_session_visibility_fork_activation.sql")
          ? 1
          : 0) +
    ...
    expect(
      migrations.get("0289_session_composer_policy_authority.sql"),
    ).toMatchObject({
      sha256:
        "478e7ba49b6940bdd849223a0965b7dcc20a0d4428f0fc85078961dbd3984285",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0293_confirm_time_rule_rebaseline.sql"),
    ).toMatchObject({
      sha256:
        "0a208593d2ab2190407ef127af1398be88765ebe978afe0ca18c3ce5bd32e617",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0294_preference_activation_authority.sql"),
    ).toMatchObject({
      sha256:
        "0e26e7b9869eb91946932b0ba255d4493e48a80fc62923b3ae54580997d6fbbb",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0288_attached_browser_reenrollment.sql"),
    ).toMatchObject({
      sha256:
        "a225b10590a065397ee35c810e6fb962e451b1745677ac4a0212431e60f2b3b6",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0292_truthful_tenancy_inventory_counters.sql"),
    ).toMatchObject({
      sha256:
        "da2484dce64877a3ac254682640148c48747ec89b36737204824e65600c476d5",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0283_editable_spreadsheet_authored_state.sql"),
    ).toMatchObject({
      sha256:
        "b746e4ef7738f755cd8578faccf54582433d69987f155b1c57896e07f92e63eb",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0263_organization_membership_lifecycle.sql"),
    ).toMatchObject({
      sha256:
        "1119554dc06a768c92f7189a97b438ebdc011747a6c8d7cefc992962f2293593",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0285_organization_tenancy_inventory.sql"),
    ).toMatchObject({
      sha256:
        "942c606d0d85064f427d2374aba9851c5c3087a49764b9f61bef76d3b8dac0f7",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0282_variable_set_session_attach_attribution.sql"),
    ).toMatchObject({
      sha256:
        "ad47dfc672380757da5a46a049b56ff72af54a7fffb73e929e364c6ae29cfda7",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0281_viewer_holder_authority_claims.sql"),
    ).toMatchObject({
      sha256:
        "17d90d51ca95222aec87abf296a3a3936537739a40d4700c932f1cfad6742f00",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0280_connection_and_variable_set_audit_attribution.sql"),
    ).toMatchObject({
      sha256:
        "6157e7d2023622a332faddf0f7a823bc941f0bce6e866523736866b1eabad556",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0279_workspace_connection_use_lane.sql"),
    ).toMatchObject({
      sha256:
        "db923fce3e8f6c12a1230add7b9e92fb0b6ba1090f456474069dee401c61367c",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0278_workspace_membership_removal_fencing.sql"),
    ).toMatchObject({
      sha256:
        "d16d4e0632ed1315ae32ba928d84a218729421e7d2ccfad5ad5806eb44ab0771",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0277_workspace_writer_authority_attribution.sql"),
    ).toMatchObject({
      sha256:
        "74c27f9366ae809d6e0300991c3d5f279138b3e2673ea3fb16c94c1f4d3ecfb6",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0275_scheduled_connection_authority.sql"),
    ).toMatchObject({
      sha256:
        "99d20fdb37735b1bb0b95f3b94ebbaf10f8f44ab8484a1e8d1095e73d6f76ec9",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0214_session_activity_commit_gate.sql"),
    ).toMatchObject({
      sha256:
        "26c84bc34bc51d19f9532cf3f2c64a649f100a724cb73d968e17e7c4ecf8de36",
      deploymentMode: "maintenance",
    });
    if (migrations.has("0218_organization_tenancy_foundation.sql")) {
      expect(
        migrations.get("0218_organization_tenancy_foundation.sql"),
      ).toMatchObject({
        sha256:
          "6377522b4a7295150828bee39fffc90643adc46fec1907f1acc9671909ad6e75",
        deploymentMode: "rolling",
      });
    }
    if (
      ...
      expect(
        ...
        sha256:
          "dcbc4dcf9c09255af4140ef1117938ec21148a1245dad85926aeb52c58e6e88f",
        deploymentMode: "rolling",
      });
    }
    expect(
      migrations.get("0197_knowledge_source_sync_schedules.sql"),
    ).toMatchObject({
      sha256:
        "edd425be4e4db07f4fcab1e520ece71dc1a692072ce28256ec2b86248442f3c8",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0065_enrollment_credential_generation.sql"),
    ).toMatchObject({
      sha256:
        "2e25fa2dfb8a95a7a9ba1ef5aa9bd219755af998b3317bcdf4d7acc4f67264fe",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0181_connected_machine_removal.sql")).toMatchObject({
      sha256:
        "c933a0781ac3c78272c5049637e3718299382e93272ea04095077f8ce7148f0a",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0182_connected_machine_remove_session_default.sql"),
    ).toMatchObject({
      sha256:
        "c15bd3b6d71f15be9e163481c3cb698d8f46620d655a72a5e56d193bc16310cd",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0198_memory_slack_publication_delivery.sql"),
    ).toMatchObject({
      sha256:
        "6cdc5d8cc22ff7c2e8abd0982223f75c414641857587bb7c0a736a88c20f0c1e",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0199_workspace_learning_policy.sql")).toMatchObject({
      sha256:
        "92286a9bbe8982fde66780bbe03fd92c0e5dbf1c9ff1fbf08a1d17fd2f8d595f",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0202_document_index_checkpoints.sql")).toMatchObject(
      {
        sha256:
          "93b24b48e587fca5288af6042947f4592a3767bc6882fc8d5f52c973214e94ea",
        deploymentMode: "rolling",
      },
    );
    expect(
      migrations.get("0212_slack_installation_bindings.sql"),
    ).toMatchObject({
      sha256:
        "aba5b603fc88237a2446fc66803f3c940f2f50e111fbe882abf4420307374b34",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0203_durable_video_generation.sql")).toMatchObject({
      sha256:
        "a48d600cd51a4ed6ed2a68d443959b8754364ab7d60709b1d5cd23f2a5013d3a",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0204_video_generation_funding.sql")).toMatchObject({
      sha256:
        "bdb70908f6e61ecc1d69228e2c05183c70527bf1bebac74bdab3294243ed9c3b",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0205_attempt_tool_catalogs.sql")).toMatchObject({
      sha256:
        "65e121d067dc3df2cd23a4e78e9ad094a0450a0be648720af0ff50f2002418d5",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0206_browser_sessions.sql")).toMatchObject({
      sha256:
        "bb21e1ec77eb9a92435abfd6bbaa969355ec61179b5ea4837ca363fc1f667c2f",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0207_browser_identities.sql")).toMatchObject({
      sha256:
        "03790bd77a2b6e237c22d3dabc6f5a579feaf18931d5ce71da084d0958729e5e",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0208_attached_browser_devices.sql")).toMatchObject({
      sha256:
        "5389ae763a5f8fa0b55f93f626dd4370154702695ab2c80da0a627c435b5792d",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0209_computer_sessions.sql")).toMatchObject({
      sha256:
        "b6be91fdc83b54677e7a326245f0ac2fee93b032c26bc26b011eb3879745fcb0",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0210_browser_auth_network_interventions.sql"),
    ).toMatchObject({
      sha256:
        "44fd93e8cf8b08ab7d5088a3b075a557a770474a5ac5c7d14ef1ebffe1c0e771",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0211_editable_artifact_session_links.sql"),
    ).toMatchObject({
      sha256:
        "0d24c5387951f232e72d3af5fe116fd268c0d30d59e0ac55959ba87a2309c966",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0212_browser_state_transfer_hardening.sql"),
    ).toMatchObject({
      sha256:
        "28aa25ba8262d54343dd092d21ec8853e8d050c91768a6350054724dae141f76",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0213_browser_interaction_authority.sql"),
    ).toMatchObject({
      sha256:
        "ffb7ed93832e830d379cca04efb017193f29da6e07ccd1f4cb3f469c343a2fa2",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0214_browser_download_saves.sql")).toMatchObject({
      sha256:
        "9c7b7ba708e77dd75870c1826d3ad8fdc1a5ce5d25c8a58b60edd13a27059059",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0215_browser_controller_host.sql")).toMatchObject({
      sha256:
        "048418cf8c8c6445d18977ba93c3ca87debde85fee038be6a90edfae6e4b93f6",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0216_browser_auth_health_evidence.sql"),
    ).toMatchObject({
      sha256:
        "9e42263bce8f501b01beff1983474deb232367d1fdbdf268fa42ac2e19f6a879",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0217_external_browser_auth_operations.sql"),
    ).toMatchObject({
      sha256:
        "eeecfc514f894589446d350e8af7097387312536ad6e32d39ced3f13485a02cd",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0219_site_auth_maintenance_sessions.sql"),
    ).toMatchObject({
      sha256:
        "e8e786a510ae169013c5eebab17b3cbe6fc3fd239c88e544c081f4767a0e13bd",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0220_memory_slack_append_only_cascade.sql"),
    ).toMatchObject({
      sha256:
        "17a34e7b943b1c4456bfb64cd8fe6fbb410b4ca7238b6ad02e24343483d54db1",
      deploymentMode: "rolling",
    });
    if (migrations.has("0222_session_visibility_authority_epochs.sql")) {
      expect(
        migrations.get("0222_session_visibility_authority_epochs.sql"),
      ).toMatchObject({
        sha256:
          "c28ab83cf3e79e11bd643dc485aeee44f4ba37eeefc7176af894eace9eeabeb3",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0224_slack_post_outcome_reconciliation.sql")) {
      expect(
        migrations.get("0224_slack_post_outcome_reconciliation.sql"),
      ).toMatchObject({
        sha256:
          "fc88cdb5664c74480e1437d5e061e9e32f109afd17eb946985af38791f267c49",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0229_slack_inbox_file_fact.sql")) {
      expect(migrations.get("0229_slack_inbox_file_fact.sql")).toMatchObject({
        sha256:
          "390361e1dba5830fabeaf7712e4108841a2ba267e8396599a632854da9b32ca9",
        deploymentMode: "rolling",
      });
    }
    expect(
      migrations.get("0230_user_scoped_variable_sets_rigs.sql"),
    ).toMatchObject({
      sha256:
        "560adbe658efa212ec44ad18f6af22ac874568d60a331beddae4102d00a09e5f",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0226_personal_codex_authority_foundation.sql"),
    ).toMatchObject({
      sha256:
        "34b72f6ab031596c90f2f35957c707aaf013c2f52aee8ca92a70fdb8ab9cb9ce",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0231_integration_definition_identity_cutover.sql"),
    ).toMatchObject({
      sha256:
        "535a806746d99d9b338df230c133477ca0e3d554ae6950077a3409b077f4b1cb",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0232_integration_facet_authority_cutover.sql"),
    ).toMatchObject({
      sha256:
        "acf779d1bd931ea208be8a97b9f930a2387f53151722198704f0aecbe76f1fb6",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0233_skill_and_integration_authority_cutover.sql"),
    ).toMatchObject({
      sha256:
        "26ed3d2ffcaf572623ad263aaa0103625b4ed1e9d28bcb7b5d35eb972d9762d1",
      deploymentMode: "maintenance",
    });
    if (migrations.has("0247_terraform_stacks_provenance_repair.sql")) {
      expect(
        migrations.get("0247_terraform_stacks_provenance_repair.sql"),
      ).toMatchObject({
        sha256:
          "0bb6196b0a89e4b9d2271ec098793e2c8cbeecf4c9ef7ff036f3ed26e1792cff",
        deploymentMode: "maintenance",
      });
    }
    if (
      ...
      expect(
        migrations.get("0248_terraform_stacks_component_resolution_fence.sql"),
      ).toMatchObject({
        sha256:
          "4fe9beb4cdf182a4391f7268bc83d11fdbc3125c105d48c8ca0bf41349be095f",
        deploymentMode: "maintenance",
      });
    }
    if (migrations.has("0223_pending_tool_event_output.sql")) {
      expect(
        migrations.get("0223_pending_tool_event_output.sql"),
      ).toMatchObject({
        sha256:
          "851cdb5dfe14f1cf6323e6cf59e55269b87d2db4406ea2ad10147553635eb707",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0234_xai_subscription_authority.sql")) {
      expect(
        migrations.get("0234_xai_subscription_authority.sql"),
      ).toMatchObject({
        sha256:
          "be0c23d624d545226068ce7627a2b458fe9aad996ea7ee750753c81eeb8988a8",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0235_canonical_human_login_bindings.sql")) {
      expect(
        migrations.get("0235_canonical_human_login_bindings.sql"),
      ).toMatchObject({
        sha256:
          "c62984fc3016e0433e5dbc2049e14e11b343d9221b92b2e5dcc5238339579200",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0236_session_visibility_slack_policy.sql")) {
    ...
    if (migrations.has("0238_recover_unclaimed_session_turns.sql")) {
      expect(
        migrations.get("0238_recover_unclaimed_session_turns.sql"),
      ).toMatchObject({
        sha256:
          "7d63ad62f2dc91f8c5de87b95a35a366d4b23d4fc76f320b5376ef2412a2002d",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0241_atomic_personal_resource_delegation.sql")) {
      expect(
        migrations.get("0241_atomic_personal_resource_delegation.sql"),
      ).toMatchObject({
        sha256:
    ...
    if (migrations.has("0243_google_drive_object_acl_authority.sql")) {
      expect(
        migrations.get("0243_google_drive_object_acl_authority.sql"),
      ).toMatchObject({
        sha256:
          "1cc4b297460ba64d252230ceddc9eaaf4d6ea9b02afcd56518900d5b569bfcfe",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0245_model_context_contribution_facts.sql")) {
      expect(
        migrations.get("0245_model_context_contribution_facts.sql"),
      ).toMatchObject({
        sha256:
          "437bb07ffe12f9c714bd2a40d0ecd8ed9df1fd9003f4d057fe11101999841f40",
        deploymentMode: "rolling",
    ...
    if (migrations.has("0255_company_brain_governed_write_proposals.sql")) {
      expect(
        migrations.get("0255_company_brain_governed_write_proposals.sql"),
      ).toMatchObject({
        sha256:
          "5d6527267b8de9cb9539e97a0cd30051dc9b2059fd5935261aa8c762d5d6a0d3",
        deploymentMode: "rolling",
    ...
    expect(
      migrations.get("0183_model_call_provider_cost_estimates.sql"),
    ).toMatchObject({
      sha256:
        "2cb087b69996c62e8836f2d65c9e2af3fb580fe1822d327600bf40e3a6977d64",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0184_sandbox_drain_teardown_fence.sql"),
    ).toMatchObject({
      sha256:
        "fa757f718fe7239df60514768f8b50abb1905bba5f928adda5001bad4927634e",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0185_temporal_schedule_cleanup_outbox.sql"),
    ).toMatchObject({
      sha256:
        "801c2848adbc4c58a701bd1e7039b4c9b4fb4beb0a49b950585a5c79b536fd9d",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0186_sandbox_capture_provider_contract.sql"),
    ).toMatchObject({
      sha256:
        "fc7d8d0eeba1800727f4f0f72020fac50290c629d9819f5b608d33f8b6af6bce",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0187_generated_image_artifacts.sql")).toMatchObject({
      sha256:
        "d29994ffbe9bb0bd4f048341c11ccd3c12ffd559ba8052535ea4b60df4f7543b",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0188_image_generation_retention_failure.sql"),
    ).toMatchObject({
      sha256:
        "e4fa1cc2700d67befb0d517ad0dd4255481f71aefe400dd5717461b76f12ccd3",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0189_retained_session_image_formats.sql"),
    ).toMatchObject({
      sha256:
        "79daa0cb3f7649faafc30fba8976bfbea2881fe84d82a407cb071576c37bfd0c",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0190_timeline_annotations.sql")).toMatchObject({
      sha256:
        "ebc8c3c9998d0a292b548bfa98889309e8802e0619f0aeef31b5a0959ef42825",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0191_editable_artifact_engine.sql")).toMatchObject({
      sha256:
        "5d51991c1ba63c298d7d59032956cac752c5b29645f594cf87f1aaa1919c7d1e",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0192_editable_artifact_live_tickets.sql"),
    ).toMatchObject({
      sha256:
        "e96f7a9537cc074516d9f08758f03880fdb0cfdab8559c8a7c814bf8229ba9ef",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0193_editable_artifact_authorization.sql"),
    ).toMatchObject({
      sha256:
        "b4312978eba8f9211c4bcaada45fd0930052a567b969092cb603b4607228e5b8",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0194_editable_artifact_durable_exports.sql"),
    ).toMatchObject({
      sha256:
        "6d69b72b6257a4e9372214b216ba10ffb81ddb23f47b3e21ec0077c49aef518e",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0195_editable_artifact_import_authorization.sql"),
    ).toMatchObject({
      sha256:
        "c34cef47b396a9bdc518180d2012c2d6c1fc96313449a76ff2c50403acc32c7f",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0196_rig_provider_images.sql")).toMatchObject({
      sha256:
        "0d47354968aeec0ba9d329351593fd8f3b5469a3157e74f837dbdb16b64bc07f",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0201_company_profile_authority.sql")).toMatchObject({
      sha256:
        "780eb3550bbe5a6811cde7f8cfd69cf8f98e3181e03d1700f325961e71b1b272",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0180_retained_screenshot_lifecycle_fences.sql"),
    ).toMatchObject({
      sha256:
        "184bd3bb0360d63abc72e09ad5461646679320c61c0a24a5e67cc3af5a7d008a",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0179_slack_private_shortcut_delivery_gate.sql"),
    ).toMatchObject({
      sha256:
        "eabb9498659f0fe7a9aa080568f4a6963bc4e51bb9b7df897c0a9f7060671824",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0178_permissioned_secret_reads.sql")).toMatchObject({
      sha256:
        "a671934c8c969fe14edc322fd64c1423605620977d98660efc942b955673357b",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0177_session_events_workspace_turn_type_index.sql"),
    ).toMatchObject({
      sha256:
        "24eaf3a8c0eb5cfabfbfaf96544b77ea8021be9ce92730a0ec663493d2651650",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0176_lossless_canonical_json.sql")).toMatchObject({
      sha256:
        "796f2758f2d6ed46ed9d4fd44e191e64fc6fd65eaefcafef5597158426006538",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0175_resumable_transcription_provider_deadline.sql"),
    ).toMatchObject({
      sha256:
        "8e91b37db947c1430b5d12ed17a19038d14788ff581e9e36f68461d807db28cb",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0174_session_wake_live_interruption.sql"),
    ).toMatchObject({
      sha256:
        "492f93a4ba0f715f3d37cdc539d5e09dd277b8a882d24b1730d01e245e2f85cf",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0173_codex_auth_boundaries.sql")).toMatchObject({
      sha256:
        "450075954cb9c8bfc346ccf09991edd362cc5ffdcf4e94d1c404d3fc6795e2ca",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0171_social_connection_subject_ownership.sql"),
    ).toMatchObject({
      sha256:
        "939893142dc109c77b2a665e76e58533002cebbf3320dba28277e2caea825deb",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0170_session_control_wake_revision.sql"),
    ).toMatchObject({
      sha256:
        "cec3593e377f1cdc6aac3441b89d57e7c19d7377ff31991f340e14af1e64453d",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0170_resumable_transcription_recordings.sql"),
    ).toMatchObject({
      sha256:
        "b75688206e0f2d7c431fdfc7a465d4680fde59a129a090b2e532dd5b1c896bbd",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get(
      ...
      sha256:
        "71d36ab95a1711c78ab36a09af9a14ddbf6ee84a3bac5ec5fdb460c768c54ef8",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0172_retire_model_visible_github_token.sql"),
    ).toMatchObject({
      sha256:
        "6e2123085f5574a046eaea7db7b5168540625d554771ee9e5639787bbde4c713",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get(
      ...
      sha256:
        "d778468a5f7dd77046ddb26a29ea819a081ba07cbe1bb82c82e84d609a8dc4b7",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0167_document_index_replay_authority.sql"),
    ).toMatchObject({
      sha256:
        "a6709b8c4c4bdd8bef82770aad11d2fb7424e858a657137315b36bb71496bb96",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0166_connection_disconnect_idempotency.sql"),
    ).toMatchObject({
      sha256:
        "47e93eac839e160995f732f18986753d45221b1aedf98852c19b0b271236dc1b",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0165_document_authority_foundation.sql"),
    ).toMatchObject({
      sha256:
        "bde7f3088069f54e71aeb46375c3bda050e6af3a1d8d5464c38dca3d742baa28",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0164_session_realtime_models.sql")).toMatchObject({
      sha256:
        "6140bfa6efaca5d4fe34892529db2ada756b49f47d9fbe783137dc32d3b6afdc",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0163_session_realtime_delegation_progress.sql"),
    ).toMatchObject({
      sha256:
        "b843fb79d150242591f160d2e841ec3053e2b5a61af86fe09cff400d0adcdb1d",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0162_session_realtime_connection_promotion.sql"),
    ).toMatchObject({
      sha256:
        "118312d240b3cb0f67241466447d8c726ffcd0d050429c3e5a4fdf89bd9fafe2",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0161_session_realtime_context_projection.sql"),
    ).toMatchObject({
      sha256:
        "f2ece368fdbb9c07b0d5e305bc92f0aecefc929e9399d51b81c9c7ced28c3889",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0160_session_realtime_delegation_terminal.sql"),
    ).toMatchObject({
      sha256:
        "678147cf0298d01bf20acae291a33a275d3e8a66b82b289d7d876f3f9e8e0ecd",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0159_session_realtime_ledger.sql")).toMatchObject({
      sha256:
        "471a4be63a30ab7315115a49e43deb5bf993e494df81be9c25274c2cce89b7e1",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0158_session_realtime_mode.sql")).toMatchObject({
      sha256:
        "a005e2b0d0e41dc7350643facce2b080b2ede379d88c7d3725c0757e2ca65f15",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0157_session_policy_role_snapshots.sql"),
    ).toMatchObject({
      sha256:
        "84ad34345fe587ac8d952a4ddb7c55b1fec6a381b9ba5a73af6bcf253860f737",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0156_slack_reaction_trigger.sql")).toMatchObject({
      sha256:
        "080cd2840c3fd4c6c81b838cf7b0a4f1fa7e48ab4ced18c05e76da895934a945",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0155_connector_action_policies.sql")).toMatchObject({
      sha256:
        "eec332244acd1e38f6964cd455960e82b1c62cf63652acf946fe0e78b3faa785",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0154_scoped_knowledge_foundation.sql"),
    ).toMatchObject({
      sha256:
        "9e52f88b51b81d405de8f7c3c70a083a1a091d49b98042f96127565f352e9874",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0153_mcp_personal_connection_delegations.sql"),
    ).toMatchObject({
      sha256:
        "e27385280b8342ddd2bad9fea000d823e0e96c46b1c340403afaca81613610cf",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0152_hierarchical_memory_foundation.sql"),
    ).toMatchObject({
      sha256:
    ...
    expect(migrations.get("0151_slack_delivery_backoff.sql")).toMatchObject({
      sha256:
        "787a92127bb43c1beee6506c087e4bd9e53d933d06db2609afcd97e7c6642679",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0128_github_installation_authority.sql"),
    ).toMatchObject({
      sha256:
        "365793b2a204a70e214adb90298b522acbb6dcfae22a46681a58f41a6938e6f0",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0131_slack_bot_install_and_post_idempotency.sql"),
    ).toMatchObject({
      sha256:
        "b9516f61a23ecd363536f159bd772426a0da52eedfb8195a7b22f7be6a131bf2",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0132_connection_subject_isolation.sql"),
    ).toMatchObject({
      sha256:
        "c52786e8732b49d223db2bb1c9789455304ad2b8750fdb46f261fd1da04dab44",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0133_session_skills.sql")).toMatchObject({
      sha256:
        "f0aac6c242a4dbad8d6d717d09f15b295855030cf0a8979151f6928ea7ea6ff6",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0135_durable_machine_input_batches.sql"),
    ).toMatchObject({
      sha256:
        "c2d642594077a74956fd2eaa64fee8fafcd748ad432d2c9c6f4450019970617c",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0122_codex_capacity_same_turn.sql")).toMatchObject({
      sha256:
        "84e97abff7394d9fcca110012d9ceaede9ae683280a8a4a7335bcf9ec5d52d4e",
      deploymentMode: "maintenance",
    });
    ...
    const boundarySql = await readFile(
      join(
        import.meta.dir,
        "../packages/db/drizzle/0110_nested_agent_depth_boundary.sql",
      ),
      "utf8",
    );
    ...
    expect(
      boundarySql.indexOf("DO $reconcile$", sourceTableLock),
    ).toBeGreaterThan(sourceTableLock);
    expect(
      contract.migrations
        .map((migration) => migration.path)
        .filter((path) =>
          /^(?:010[3-9]|011[0-9]|012[0-9]|013[0-9]|014[0-9]|015[0-9]|016[0-4])_/.test(
            path,
          ),
        ),
    ).toEqual([
    ...
    expect(
      migrations.get("0143_session_codex_compaction_mode.sql"),
    ).toMatchObject({
      sha256:
        "574cfe6fc5ab24135e84d3932fd936e134ebe28bce8ac3cb5db97a549683906f",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0150_slack_task_interactions.sql")).toMatchObject({
      sha256:
        "97f23742bedcc0131ae21ba08a02eb8b5a739dafe790d22477622bf01746eaf6",
      deploymentMode: "rolling",
    });
    expect(
      new Set(contract.migrations.map((migration) => migration.path)).size,
    ).toBe(contract.fileCount);
    expect(migrations.get("0097_host_export_outbox.sql")).toMatchObject({
      sha256:
        "918763f2438efd06232f221305db6acac76e2bee5fa436e9665a860794c43d03",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0103_host_export_root_session.sql")).toMatchObject({
      sha256:
        "7a1a5c22bd7f0f5e38c5641257f709c99d7cfa0b4816fcdab2f8cbe0ba9db743",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0104_host_export_root_session_backfill.sql"),
    ).toMatchObject({
      sha256:
        "42d29994ac12b7118f0a1e3c252615509e887ee84bb1854056c9bf90e578760d",
      deploymentMode: "maintenance",
    });
    if (migrations.has("0107_host_export_lineage_contract.sql")) {
      expect(
        migrations.get("0107_host_export_lineage_contract.sql"),
      ).toMatchObject({
        sha256:
          "82dfa0f18f59d6a6c65c02bdfca72d4e728cc67d12fb075a85b2233d7affe091",
        deploymentMode: "rolling",
      });
    }
    if (migrations.has("0108_fence_invalidated_warming_epochs.sql")) {
      expect(
        migrations.get("0108_fence_invalidated_warming_epochs.sql"),
      ).toMatchObject({
        sha256:
          "5039f21076d55cdf7acc45c613ca5c422ed21eecb84ee9725bfa8d9eeb78810f",
        deploymentMode: "rolling",
      });
    }
    expect(
      migrations.get("0117_sandbox_recovery_generations.sql"),
    ).toMatchObject({
      sha256:
        "365284a9ab495173780d54c4b1470824891b15a7290735bf09b72c2f5fdbc48b",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0118_new_session_drafts.sql")).toMatchObject({
      sha256:
        "69ae71b80392eea964c47cadff876cff58db699d6c2470482d35aaf0931de70c",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0119_pending_tool_output_policy.sql")).toMatchObject(
      {
        sha256:
          "a70e7f605cf4f2c5677e30ccf80f29674107fc88d346c9fdc0882e0b9f314c25",
        deploymentMode: "rolling",
      },
    );
    expect(migrations.get("0120_durable_goal_wake.sql")).toMatchObject({
      sha256:
        "5c24fb49679513e2a7cc387e738b2bbdc9b5b3f465c023ea97921882f035d983",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0121_goal_update_idempotency.sql")).toMatchObject({
      sha256:
        "e90f030e9dfb3c2dc040b2192cdd874fad264020f06b9c82f1c3dcd30a9769ca",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0123_session_tool_policy_version.sql"),
    ).toMatchObject({
      sha256:
        "d23abd0ea9ac21c114a397eaa2a6a524652b9554683dd8e68937ee232e22711e",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0124_session_event_duplicate_lookup.sql"),
    ).toMatchObject({
      sha256:
        "115dbd71c528c78d340cf3be476f2973db273740e8bcbf47196da96c2d9f94c1",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0125_document_drops_visibility.sql")).toMatchObject({
      sha256:
        "d2d9f0c7f4b5cc239df3d764d04e0b9fecf2ab9a37ade02735607393c03cb18f",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0126_document_access_constraints.sql"),
    ).toMatchObject({
      sha256:
        "0519e1050c51af5b2f1a02def53f20e5e1e116277d85811d0bfef9c567302dba",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0127_document_default_base_index.sql"),
    ).toMatchObject({
      sha256:
        "7deb5fde59f2e0e336a18a72630a00348dc641041f7df60639b50df79b4c4480",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0129_retained_process_reconciliation.sql"),
    ).toMatchObject({
      sha256:
        "913758633b31bb3a4f56dca19e7113630f5d12300d075827ac36179e79305dbd",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0130_workspace_instruction_policies.sql"),
    ).toMatchObject({
      sha256:
        "12226a4560dc1150ffe2c3549f821d2483cc4d4a09ef74747ed13376404fc7c5",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0134_session_first_party_mcp_tools.sql"),
    ).toMatchObject({
      sha256:
        "7255a5dfea703b00e01aeab6d003728e60ad7ff78c87af2dc61bd8dda456391e",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0136_unified_session_tool_policy.sql"),
    ).toMatchObject({
      sha256:
        "8a12370895795b1e8e2ef193a65c97592bf793a410ef6fcae9cc21162561feee",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0137_preference_registry.sql")).toMatchObject({
      sha256:
        "8197520ec68e685e2658b2d33f8daf64e03d1d4922fa9c3ccd97c6f73849c5c5",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0138_sandbox_checkpoint_artifacts_and_deadlines.sql"),
    ).toMatchObject({
      sha256:
        "c32c70ff47930c77f45482dc5f66b26b13dd444d440397837cca134f97c48e14",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0139_codex_provider_artifact_invalidations.sql"),
    ).toMatchObject({
      sha256:
        "977c161505854ef352ade74d396e3c2205e8d3391789fb3535973bd0d20f953b",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0140_sandbox_restore_and_reaper_fences.sql"),
    ).toMatchObject({
      sha256:
        "fe8441b669fd99fa0463378c34dd75ceca3077af7529c85c552a534c530828d8",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0141_social_connection_credentials.sql"),
    ).toMatchObject({
      sha256:
        "d5a032bd8c6b61c45f48793963185bcab964de88981eba66d32b4f9de74e8edf",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0142_sandbox_archive_capture_gate.sql"),
    ).toMatchObject({
      sha256:
        "48546fe4f106da0b36edf8a47da56e35b123fd0a63dc9c2eaed887756d35144f",
      deploymentMode: "maintenance",
    });
    expect(
      migrations.get("0144_sandbox_viewer_force_drain_gate.sql"),
    ).toMatchObject({
      sha256:
        "0b03627860a310eb27caa94cf7f67d614f0198a7a01d36b383c7874edba41b38",
      deploymentMode: "maintenance",
    });
    expect(migrations.get("0145_model_call_facts.sql")).toMatchObject({
      sha256:
        "93bb3f262f465ca2128b6dadc5e166bfdc78ded610e57a803ce1d3e6a8599dde",
      deploymentMode: "rolling",
    });
    expect(
      migrations.get("0146_slack_bot_delete_idempotency.sql"),
    ).toMatchObject({
      sha256:
        "9084b0cd13c1924dd23af09b80d65b58223721259a23b1240e75f5af91b8cfb5",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0147_draft_latency_mode.sql")).toMatchObject({
      sha256:
        "8269cfad28f1fc8d943f55b89ef5715bc18e9817283ac3a36502127f12935fca",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0148_session_turn_latency_mode.sql")).toMatchObject({
      sha256:
        "fe753b8b5866c4f619ae3b360442659db26f639db867b622b1f2a0520e9c80dd",
      deploymentMode: "rolling",
    });
    expect(migrations.get("0149_workspace_artifacts.sql")).toMatchObject({
      sha256:
        "ae3bdf08e8a47e04dd0b025f143b40b0e31f31c9e7b3eb51c0d21233be265bd3",
      deploymentMode: "maintenance",
    });
  });
});
...
async function contractWithoutMigrations(excludedPaths: readonly string[]) {
  ...
  const directory = await mkdtemp(
    join(tmpdir(), "opengeni-schema-contract-filtered-"),
  );
