...
describe("release schema contract", () => {
  ...
  test("registers forward migrations without repinning host-export history", async () => {
    ...
    const workspaceMemberManagementScope = completeSourceContract.migrations.some(
      (migration) => migration.path === "0369_workspace_member_management_scope.sql",
    );
    const workspaceMemberCandidateInventory = completeSourceContract.migrations.some(
      (migration) => migration.path === "0370_workspace_member_candidate_inventory.sql",
    );
    const automaticSessionTitleMigrationPaths = new Set([
      ...
      "0369_workspace_member_management_scope.sql",
      "0370_workspace_member_candidate_inventory.sql",
    ]);
