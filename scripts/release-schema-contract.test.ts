...
describe("release schema contract", () => {
  ...
  test("classifies the fail-closed Rig verification cutover as maintenance-only", async () => {
    ...
    expect(
      contract.migrations.find(
        (migration) => migration.path === "0388_fail_closed_rig_version_activation.sql",
      ),
    ).toMatchObject({ deploymentMode: "maintenance" });
  });
  ...
  test("registers forward migrations without repinning host-export history", async () => {
    ...
    const durableRigProviderImageCleanup = completeSourceContract.migrations.some(
      (migration) => migration.path === "0389_durable_rig_provider_image_cleanup.sql",
    );
    ...
    const automaticSessionTitleMigrationPaths = new Set([
      ...
      "0388_fail_closed_rig_version_activation.sql",
      "0389_durable_rig_provider_image_cleanup.sql",
    ]);
