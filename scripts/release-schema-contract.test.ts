...
describe("release schema contract", () => {
  ...
  test("registers forward migrations without repinning host-export history", async () => {
    ...
    const governedAppsPersistence = completeSourceContract.migrations.some(
      (migration) => migration.path === "0382_governed_apps_persistence.sql",
    );
    expect(
      completeSourceContract.migrations.find(
        (migration) => migration.path === "0382_governed_apps_persistence.sql",
    ...
    const automaticSessionTitleMigrationPaths = new Set([
      ...
      "0382_governed_apps_persistence.sql",
    ]);
    ...
    expect(completeSourceContract).toMatchObject({
      fileCount:
        ...
        (governedAppsPersistence ? 1 : 0),
      latestMigration: governedAppsPersistence
        ? "0382_governed_apps_persistence.sql"
