...
describe("release schema contract", () => {
  ...
  test("preserves published host-export history and appends the forward repair", async () => {
    ...
    const sessionVisibilityContractHash = (
      ...
      includesTurnInstructions: boolean,
    ): string | null => {
      if (!migrations.has("0236_session_visibility_slack_policy.sql")) return null;
      const includesSlackInboxFileFact = migrations.has("0229_slack_inbox_file_fact.sql");
      if (includesTurnInstructions) {
        if (includesSlackInboxFileFact) {
          return includesActivation
            ? "4e087d3c40bf8149e52f7f678dc8e3056eb654e016031bc32b9d886fe7927292"
            : "2f20520bc5466d2c2788b8c03251f5bf7ebefb08daa0c27d12da564a0f257960";
        }
        return includesActivation
          ? "22805dac6a2e0a269300ca53e0a601697874a2a7d8546d159cd16926d203f110"
          : "7f02ca3cb4af8abdb3cf09aa46f5ffe8f89e7b7c469dba34c3b670b863adf4f4";
      }
      if (includesSlackInboxFileFact) {
        return includesActivation
    ...
    const includesTurnInstructions = migrations.has("0237_realtime_turn_instructions.sql");
    ...
    if (activationMigration) {
      expect(sourceContract.sha256).toBe(
        sessionVisibilityContractHash(true, includesTurnInstructions),
      );
      ...
      sourceContract.sha256 = sessionVisibilityContractHash(false, includesTurnInstructions)!;
    }
    expect(sourceContract.sha256).toBe(
      sessionVisibilityContractHash(false, includesTurnInstructions) ?? currentMainContractHash,
    );
    const contract = {
      ...sourceContract,
      sha256:
    ...
    expect(contract.fileCount).toBe(
      108 +
        ...
        (migrations.has("0237_realtime_turn_instructions.sql") ? 1 : 0),
    );
    expect(contract.sha256).toBe(
      sessionVisibilityContractHash(false, includesTurnInstructions) ?? currentMainContractHash,
    );
    expect(contract.latestMigration).toBe(
      migrations.has("0237_realtime_turn_instructions.sql")
        ? "0237_realtime_turn_instructions.sql"
    ...
    expect(migrations.get("0237_realtime_turn_instructions.sql")).toMatchObject({
      sha256: "6c1d802e9a77c206cc2b379e20c176d28c4da58cd05f4fd4de3dd6a4dcd56a92",
      deploymentMode: "rolling",
    });
