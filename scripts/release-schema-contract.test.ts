  ...
  test("preserves published host-export history and appends the forward repair", async () => {
    ...
    expect(contract.fileCount).toBe(
      108 +
        ...
        (migrations.has("0134_durable_machine_input_batches.sql") ? 1 : 0),
    );
    expect(contract.sha256).toBe(
      "d9d9f919fadfcf8efc5b36a1f30edd68f183a6eb9ea9cdf1f6a040b43731f4ed",
    );
    expect(contract.latestMigration).toBe("0134_durable_machine_input_batches.sql");
    ...
    expect(migrations.get("0134_durable_machine_input_batches.sql")).toMatchObject({
    ...
    expect(
      ...
      "0134_durable_machine_input_batches.sql",
    ]);
