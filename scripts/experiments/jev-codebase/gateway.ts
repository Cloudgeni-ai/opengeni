...
export type Receipt = {
  ...
  id: string; arm: Arm; model: string; stateHash: string; timestamp: string;
  reservedUsd?: number; elapsedMs?: number; inputTokens?: number; outputTokens?: number;
  cachedTokens?: number; nominalUsd?: number; reportedUsd?: number | null;
  resolvedModel?: string; answers?: Record<string, Judgment>; reason?: string;
};
...
export async function preflight() {
  ...
  const response = await fetch("https://ai-gateway.vercel.sh/v1/models", { signal: AbortSignal.timeout(15000) });
  if (!response.ok) throw new Error("pricing_unavailable");
  const catalog = await response.json() as { data: { id: string; pricing: Record<string, string> }[] };
  const prices = Object.fromEntries(Object.entries(MODELS).map(([arm, id]) => {
    const model = catalog.data.find(m => m.id === id);
    if (!model) throw new Error("model_unavailable");
    const p = model.pricing;
    const price = { input: Number(p.input), output: Number(p.output), cached: Number(p.input_cache_read ?? p.input) };
    if (Object.values(price).some(v => !Number.isFinite(v) || v < 0)) throw new Error("pricing_invalid");
    return [arm, price];
  })) as Record<Arm, Price>;
  return { gateway, prices, checkedAt: new Date().toISOString() };
}

export function createJudge(arm: Arm, setup: Awaited<ReturnType<typeof preflight>>, journal: string, maxRequests = 80, maxUsd = 0.50): Judge {
  let stopped = false;
  return async (state, questions) => {
    if (stopped) throw new Error("run_stopped");
    const history: Receipt[] = existsSync(journal) ? readFileSync(journal, "utf8").trim().split("\n").filter(Boolean).map(s => JSON.parse(s)) : [];
    // Uncertain prior requests are never replayed or treated as zero-cost failures.
    const starts = history.filter(r => r.kind === "started");
    if (history.some(r => r.kind === "failed") || starts.some(r => !history.some(c => c.id === r.id && c.kind === "completed"))) throw new Error("unsettled_or_failed_journal");
    ...
    const used = history.filter(r => r.kind === "completed").reduce((n, r) => n + Math.max(r.nominalUsd ?? 0, r.reportedUsd ?? 0), 0);
    if (starts.length >= maxRequests || used + reserve > maxUsd) throw new Error("run_budget_exhausted");
    const base = { id: crypto.randomUUID(), arm, model: MODELS[arm], stateHash: hash(payload), timestamp: new Date().toISOString() };
    const record = (value: Receipt) => appendFileSync(journal, JSON.stringify(value) + "\n", { mode: 0o600 });
    ...
    try {
      ...
      if (![inputTokens, outputTokens, cachedTokens].every(n => Number.isFinite(n) && n >= 0) || cachedTokens > inputTokens) throw new Error("usage_unavailable");
      ...
      if (reportedUsd !== null && (!Number.isFinite(reportedUsd) || reportedUsd < 0)) throw new Error("invalid_cost");
      const nominalUsd = (inputTokens - cachedTokens) * price.input + cachedTokens * price.cached + outputTokens * price.output;
