...
function browserProofSource(): string {
  return [
    ...
    'import { COMMITTED_TRANSACTION_PROTOCOL_VERSION, SPREADSHEET_COLLABORATION_SNAPSHOT_VERSION } from "@opengeni/contracts/editable-artifact-versions";',
    ...
    "    });",
    "    try {",
    '      await kernel.loadSnapshot(modality === "spreadsheet" ? { modality, artifactId: artifactId(modality), sequence: 0, stateHash, digest, kernelVersion: identity.kernelVersion, modelSchemaVersion: identity.modelSchemaVersion, operationProtocolVersion: COMMITTED_TRANSACTION_PROTOCOL_VERSION, snapshotVersion: SPREADSHEET_COLLABORATION_SNAPSHOT_VERSION, causalFrontier: [], bytes: snapshotBytes } : { modality, artifactId: artifactId(modality), sequence: 0, stateHash, digest, kernelVersion: identity.kernelVersion, modelSchemaVersion: identity.modelSchemaVersion, nativeRevision, bytes: snapshotBytes });',
    ...
    '        const packedSheetId = "11111111111111110000000000000001";',
    '        const authored = await kernel.authorPending({ ...authorBase(modality, identity), causalBase: [], selectiveUndoTargets: [], commandBytes: encodeSpreadsheetArtifactCommandBatch({ version: identity.commandVersion, commands: [{ kind: "sheet.create", sheetId: packedSheetId, name: "Packed", after: null }, { kind: "cells.set", sheet: { kind: "created-in-batch", sheetId: packedSheetId, createCommandIndex: 0 }, anchor: { row: 10, column: 1 }, rows: 1, columns: 1, cells: [0.1] }, { kind: "cells.set", sheet: { kind: "created-in-batch", sheetId: packedSheetId, createCommandIndex: 0 }, anchor: { row: 3, column: 3 }, rows: 1, columns: 1, cells: [{ formula: "=1/(1+$B$11)^4" }] }] }) });',
    ...
    '        await assertIncidentProjection(kernel, packedSheetId, "spreadsheet calculation");',
    "        await kernel.loadSnapshot({ modality, artifactId: artifactId(modality), sequence: 0, stateHash, digest, kernelVersion: identity.kernelVersion, modelSchemaVersion: identity.modelSchemaVersion, operationProtocolVersion: COMMITTED_TRANSACTION_PROTOCOL_VERSION, snapshotVersion: SPREADSHEET_COLLABORATION_SNAPSHOT_VERSION, causalFrontier: [], bytes: snapshotBytes });",
    "        await kernel.replacePending([authored]);",
    '        await assertIncidentProjection(kernel, packedSheetId, "spreadsheet reload");',
    ...
    "async function assertIncidentProjection(kernel: any, sheetId: string, label: string) { const viewport = await kernel.querySpreadsheetViewport({ sheetId, startRow: 3, startColumn: 3, rowCount: 1, columnCount: 1, maxCells: 1, maxBytes: 4096 }); const cell = viewport.cells[0]; if (cell?.formula !== '=1/(1+$B$11)^4' || cell.value.kind !== 'number' || numberBits(cell.value.value) !== 0x3fe5db3f08b0ab7dn) throw new Error(`${label} projection mismatch`); }",
    "function numberBits(value: number) { const bytes = new ArrayBuffer(8); new DataView(bytes).setFloat64(0, value, false); return new DataView(bytes).getBigUint64(0, false); }",
