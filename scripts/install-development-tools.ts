...
export function detectTarget(
  os: string = platform(),
  cpu: string = arch(),
  kernel: string = release(),
): Target {
  if (os === "win32")
    throw new Error(
      "Native Windows is unsupported: the full launcher requires Bash. Install WSL2, clone into its Linux filesystem, and run Bun and this script inside WSL2 (not PowerShell/Git Bash).",
    );
  if (os !== "linux" && os !== "darwin") throw new Error(`Unsupported OS: ${os}`);
  if (cpu !== "x64" && cpu !== "arm64")
    throw new Error(`Unsupported architecture: ${cpu}; supported: x64, arm64.`);
  const microsoft = /microsoft/i.test(kernel);
  if (os === "linux" && microsoft && !/wsl2|microsoft-standard/i.test(kernel))
    throw new Error("WSL1 is unsupported; convert the distribution to WSL2 and run inside Linux.");
...
export function assetFor(tool: ToolName, target: Target): Asset {
  if (tool === "garage") {
    ...
    return {
      tool,
      version: "2.3.0",
      url: provenance.nativeUrl,
      sha256: provenance.binarySha256,
      member: "garage",
      binary: "garage",
      checksumSource: garageProvenance.image,
      format: "binary",
    };
  }
  ...
  if (tool === "nats") {
    ...
    return {
      ...
      sha256: hashes.nats[key],
      member: `${stem}/nats-server`,
      binary: "nats-server",
      checksumSource: `${base}/SHA256SUMS`,
      format: "tar.gz",
    };
  }
  ...
  return {
    tool,
    version: "1.4.1",
    url: `${base}/temporal_cli_1.4.1_${target.os}_${target.arch}.tar.gz`,
    sha256: hashes.temporal[key],
    member: "temporal",
    binary: "temporal",
    checksumSource: `${base}/checksums.txt`,
    format: "tar.gz",
  };
}
...
export function extractBinary(archive: Uint8Array, asset: Asset): Buffer {
  if (sha256(archive) !== asset.sha256)
    throw new Error(`${asset.tool}: SHA-256 mismatch; nothing installed.`);
  ...
  for (let offset = 0; offset + 512 <= tar.length;) {
    ...
    if (type !== "" && type !== "0" && type !== "5")
      throw new Error("Unsupported tar entry (links and extensions forbidden)");
    ...
    if (name === asset.member) {
      if (result || type === "5" || size === 0)
        throw new Error("Invalid or duplicate binary entry");
...
async function ensureDirectory(path: string): Promise<void> {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink())
    throw new Error(`Refusing non-directory/symlink: ${path}`);
}
...
export async function installBinary(
  ...
  if (!["nats-server", "temporal", "garage"].includes(asset.binary))
    throw new Error("Unsupported binary destination");
  ...
  try {
    ...
    if (
      ...
      !(existing.mode & 0o111) ||
      sha256(await readFile(destination)) !== sha256(binary)
    )
      throw new Error(
        `Existing tool differs; preserving ${destination}. Move it manually before retrying.`,
      );
    return destination;
  } catch (error) {
...
export function parseArgs(args: string[]) {
  ...
  for (const arg of args) {
    if (arg === "--install") install = true;
      ...
      if (
        !names.length ||
        names.some((name) => !["garage", "nats", "temporal"].includes(name)) ||
        new Set(names).size !== names.length
      )
        throw new Error("--tools accepts unique comma-separated garage,nats,temporal names");
...
export async function main(args = process.argv.slice(2)) {
  ...
  const assets = options.tools
    .filter((name) => name !== "garage" || target.os === "linux")
    .map((name) => assetFor(name, target));
  const blockers =
    options.tools.includes("garage") && target.os !== "linux" ? [garageDiagnostic(target)] : [];
  const manualPreconditions = [
    ...
    "For native Linux/WSL2 infrastructure, provision PostgreSQL 17 server/client with matching pgcrypto and pgvector extensions. Ensure pg_config, psql and pg_isready are on PATH; pg_config --bindir must contain postgres, initdb, pg_ctl and createdb. This script never sudo-installs packages or changes database data.",
    target.os === "darwin"
      ? "Use Docker Desktop for infrastructure on macOS; installing NATS/Temporal alone does not enable a supported native stack."
      : "Debian/Ubuntu with PGDG guidance: install postgresql-17 postgresql-client-17 postgresql-17-pgvector through your administrator; other distributions use equivalent packages. Build tools are only needed if an artifact or opted-in relay source build is required.",
  ...
  if (options.install && !blockers.length) {
    ...
    for (const asset of assets)
      binaries.push({ asset, binary: extractBinary(await downloadAsset(asset.url), asset) });
    for (const { asset, binary } of binaries)
      installed.push(await installBinary(root, target, asset, binary));
  }
  const result = {
    schemaVersion: 1,
    status: blockers.length ? "blocked" : options.install ? "installed" : "plan",
    target,
    binDirectory: toolBin(root, target),
    assets,
    installed,
    blockers,
    manualPreconditions,
  ...
  if (options.json) console.log(JSON.stringify(result, null, 2));
    ...
    console.log(
      "Explicit supported subset: bun scripts/install-development-tools.ts --install --tools=nats,temporal --json",
    );
  }
