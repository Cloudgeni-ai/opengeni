...
export function repositoryMetadata(repository: string, revision: string) {
  ...
  if (!pending) { pending = load(repository, revision); repositories.set(key, pending); }
  return pending;
}
async function load(repository: string, revision: string) {
  const directory = await mkdtemp(join(tmpdir(), "opengeni-plugin-metadata-"));
  try {
    const response = await fetch(`https://codeload.github.com/${repository}/tar.gz/${revision}`, { signal: AbortSignal.timeout(45000) });
    if (!response.ok || !response.body) throw Error(`Archive HTTP ${response.status}`);
    const reader = response.body.getReader(); const chunks: Uint8Array[] = []; let size = 0;
    try { while (true) { const part = await reader.read(); if (part.done) break; size += part.value.length; if (size > 32 * 1024 * 1024) throw Error("Archive exceeds 32 MiB"); chunks.push(part.value); } } finally { await reader.cancel(); }
    const archive = join(directory, "source.tar.gz"); await Bun.write(archive, new Blob(chunks));
    ...
    for (const name of names) {
      ...
      if (/(^|\/)(\.mcp\.json|\.codex-plugin\/plugin\.json|\.claude-plugin\/plugin\.json)$/.test(path)) {
