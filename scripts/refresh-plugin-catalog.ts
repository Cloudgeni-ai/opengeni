...
const sources = [
  { id: "openai", repository: "openai/plugins", index: ".agents/plugins/marketplace.json", manifest: ".codex-plugin/plugin.json" },
  { id: "anthropic", repository: "anthropics/claude-plugins-official", index: ".claude-plugin/marketplace.json", manifest: ".claude-plugin/plugin.json" },
];
...
for (const source of sources) {
  ...
  const tree = await json(`https://api.github.com/repos/${source.repository}/git/trees/${revision}?recursive=1`);
  ...
  for (let offset = 0; offset < index.plugins.length; offset += 6) {
    entries.push(...await Promise.all(index.plugins.slice(offset, offset + 6).map(async (entry: any) => {
      const local = typeof entry.source === "string" ? entry.source : entry.source?.source === "local" ? entry.source.path : null;
      let path = local?.replace(/^\.\//, "");
      let repo = source.repository, pin = revision, base = raw, repoPaths = paths;
      let external: Map<string, string> | null = null;
      let inspectionError: string | null = null;
      if (!local) {
        const location = typeof entry.source?.url === "string" ? entry.source.url.match(/^https:\/\/github\.com\/([\w.-]+\/[\w.-]+?)(?:\.git)?\/?$/) : null;
        if (location && /^[a-f0-9]{40}$/.test(entry.source?.sha ?? "")) {
          try {
            repo = location[1]; pin = entry.source.sha;
            external = await repositoryMetadata(repo, pin);
            path = (entry.source.path ?? "").replace(/^\.\//, "").replace(/\/$/, "");
            repoPaths = [...external.keys()]; base = "https://raw.githubusercontent.com/" + repo + "/" + pin + "/";
          } catch (error) { inspectionError = error instanceof Error ? error.message : "Could not inspect repository"; }
        } else inspectionError = "Source has no supported repository and pinned commit";
      }
      const inspected = path !== undefined;
      const prefix = path ? path + "/" : "";
      const read = async (file: string) => external ? (external.get(prefix + file) ? JSON.parse(external.get(prefix + file)!) : null) : json(base + prefix + file, true);
      const manifest = inspected ? await read(source.manifest) : null;
      const ui = manifest?.interface ?? {};
      const asset = (value: unknown) => typeof value !== "string" ? null : /^https:\/\//.test(value) ? value : inspected ? new URL(value, base + prefix).href : null;
      const pluginPaths = inspected ? repoPaths.filter(value => value.startsWith(prefix)).map(value => value.slice(prefix.length)) : [];
      const skills = inspected ? pluginPaths.filter(value => /(^|\/)SKILL\.md$/.test(value)).map(value => ({
        name: value.split("/").at(-2) ?? entry.name,
        sourceUrl: "https://github.com/" + repo + "/blob/" + pin + "/" + prefix + value,
      })) : null;
      let mcpConfig: any = manifest?.mcpServers && typeof manifest.mcpServers === "object" ? manifest.mcpServers : null;
      const mcpFile = typeof manifest?.mcpServers === "string" ? manifest.mcpServers.replace(/^\.\//, "") : ".mcp.json";
      if (inspected && pluginPaths.includes(mcpFile)) mcpConfig = await read(mcpFile);
      const servers = mcpConfig?.mcpServers ?? mcpConfig ?? {};
      const mcpServers = inspected ? Object.entries(servers).map(([name, config]: [string, any]) => ({
        requiresConfiguration: Boolean(config.headers && Object.keys(config.headers).length),
        name, transport: config.type ?? (config.url ? "http" : "stdio"),
      ...
      return {
        id: `${source.id}:${entry.name}`, name: entry.name,
        ...
        sourceUrl: inspected ? `https://github.com/${repo}/tree/${pin}/${path}` : entry.source?.url ?? entry.homepage ?? null,
        skills, mcpServers, components, componentInspection: inspected ? "repository" : "not_inspected", inspectionError,
        installation: "requires_adapter",
        upstream: { source: entry.source, policy: entry.policy ?? null },
      };
    })));
  }
  catalog.push({ provider: source.id, repository: source.repository, revision, indexUrl: raw + source.index, entries });
