...
function buildsBrowserControllerOnTargetPlatform(dockerfile: string): boolean {
  ...
  const ogtool = dockerfile.indexOf(
    "RUN cd packages/ogtool && bun run build",
    codemode,
  );
  const targetStage = dockerfile.indexOf(
  ...
  const nativeCompile = dockerfile.indexOf(
    "bun build --compile \\",
    targetArchitecture,
  );
...
describe("browser controller image build contract", () => {
  for (const imagePath of imagePaths) {
    test(`${imagePath} prepares shared inputs once and compiles natively for the target`, async () => {
      ...
      expect(buildsBrowserControllerOnTargetPlatform(crossCompiled)).toBe(
        false,
      );
      expect(
        buildsBrowserControllerOnTargetPlatform(missingPreparedSource),
      ).toBe(false);
    });
  }
  ...
  test("desktop image installs the pinned Chrome artifact without a mutable apt index", async () => {
    const dockerfile = await readFile(
      resolve(root, "docker/desktop.Dockerfile"),
      "utf8",
    );
    ...
    expect(dockerfile).not.toContain(
      "google-chrome-stable=${OPENGENI_GOOGLE_CHROME_VERSION}",
    );
    expect(dockerfile).not.toContain(
  ...
  test("Debian Chromium remains available from one retained security snapshot", async () => {
    ...
    for (const dockerfile of dockerfiles) {
      ...
      expect(
        dockerfile.indexOf("opengeni-chromium-snapshot.list"),
      ).toBeLessThan(
  ...
  test("startup distinguishes a live setsid/env child from a completed browserd exec", async () => {
    ...
    const startupLoop = source.indexOf(
      'for _ in $(seq 1 "$STARTUP_ATTEMPTS"); do',
    );
    const processAlive = source.indexOf(
      'if ! kill -0 "$PID" 2>/dev/null; then',
      startupLoop,
    );
    const browserdExecComplete = source.indexOf(
      'if ! same_process "$PID"; then',
      processAlive,
    );
    const readiness = source.indexOf(
      "if admin_ready; then",
      browserdExecComplete,
    );
