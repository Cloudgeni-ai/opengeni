...
const EXPECTED_JOB_BUDGETS = {
  ...
  "ci.yml:browser-acceptance": { stepCaps: 39, needed: 40, jobCap: 45 },
...
describe("workflow timeout contract", () => {
  test("all jobs and the exact 14 run plus 3 action steps use static native caps", async () => {
    ...
    expect(capped.filter((row) => row[4] === "run")).toHaveLength(14);
