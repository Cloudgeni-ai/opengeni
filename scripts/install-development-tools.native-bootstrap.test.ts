...
import {
  ...
  sha256,
  toolBin,
} from "./install-development-tools";
...
describe("native prerequisite bootstrap", () => {
  test("explicit opt-in and selection, no ambiguous arguments", () => {
    ...
    expect(parseArgs(["--install", "--tools=nats,temporal", "--json"])).toEqual({
      install: true,
      json: true,
      tools: ["nats", "temporal"],
    });
    for (const argument of ["--force", "--tools=", "--tools=nats,nats", "--tools=rust"])
      expect(() => parseArgs([argument])).toThrow();
  });
  ...
  test("all supported assets are pinned to official HTTPS releases/checksums", () => {
    ...
    expect(garageDiagnostic(detectTarget("darwin", "arm64", "24"))).toContain(
      "no official macOS binary",
    );
    expect(garageDiagnostic(target)).toContain(
      "derived from the existing digest-pinned official Docker image",
    );
    expect(() => assetFor("garage", detectTarget("darwin", "arm64", "24"))).toThrow(
      "no official macOS binary",
    );
  ...
  test("raw Garage binaries also fail closed on checksum mismatch", () => {
    ...
    expect(() => extractBinary(Buffer.from("unverified binary"), selected)).toThrow(
      "SHA-256 mismatch",
    );
  ...
  test("verifies before parsing and extracts only intended member", () => {
    ...
    expect(extract([{ name: "LICENSE", body: "license" }, { name: "temporal" }]).toString()).toBe(
      "binary",
    );
  ...
  test("rejects traversal, absolute paths, symlinks, hardlinks and extension records", () => {
    for (const name of ["../escape", "/absolute", "a/../b", "a\\b", "./temporal", "a//b"])
      expect(() => extract([{ name }])).toThrow("Unsafe tar path");
    for (const type of ["1", "2", "x", "g", "L"])
      expect(() => extract([{ name: "temporal", type }])).toThrow("Unsupported tar entry");
  });
  test("rejects untrusted download origins before networking", async () => {
    for (const url of [
      ...
      await expect(downloadAsset(url)).rejects.toThrow("Untrusted");
  });
  test("redirects cannot leave official hosts and loops are bounded", async () => {
    let calls = 0;
    const externalRedirect = async () => {
      calls++;
    ...
    expect(calls).toBe(5);
    await expect(
      downloadAsset(asset.url, async () => new Response("missing", { status: 404 })),
  ...
  test("project directory symlink cannot redirect installation", async () => {
    ...
    try {
      ...
      await expect(installBinary(root, target, asset, Buffer.from("fixture"))).rejects.toThrow(
        "symlink",
      );
    } finally {
      await rm(root, { recursive: true, force: true });
