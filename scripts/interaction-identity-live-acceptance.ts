...
async function main(): Promise<void> {
  ...
  const accessKey =
    process.env.OPENGENI_INTERACTION_ACCEPTANCE_ACCESS_KEY?.trim();
  ...
  const browsers: Array<
    ReturnType<typeof client.interaction.browsers.session>
  > = [];
  const checks: string[] = [];

  try {
    const sourceSessionId = await createSessionShell(
      client,
      workspaceId,
      runId,
      "source",
    ...
    if (
      ...
      throw new Error(
        "first browser revision was not published as immutable default v1",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "second browser revision did not preserve v1 as the default",
      );
    }
    checks.push("revision.v2-non-default-linear-history");

    const listed = await identity.revisions();
    const listedRevisionIds = new Set(
    ...
    if (
      ...
      throw new Error(
        "identity revision list did not expose the exact immutable history",
      );
    }
    ...
    const defaultConsumer = await client.interaction.browsers.open(
      workspaceId,
      {
        operationId: crypto.randomUUID(),
    ...
    if (defaultSession.baseRevisionId !== savedV1.revision.id) {
      throw new Error(
        "identity-only browser did not resolve the default revision",
      );
    }
    ...
    const explicitConsumer = await client.interaction.browsers.open(
      workspaceId,
      {
        operationId: crypto.randomUUID(),
    ...
    if (explicitSession.baseRevisionId !== savedV2.revision.id) {
      throw new Error(
        "explicit browser revision was not preserved by placement",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "concurrent identity publications did not elect exactly one default",
      );
    }
    if (
      ...
      throw new Error(
        "concurrent identity publications did not preserve independent branches",
      );
    }
    ...
    if (
      ...
      new Set(concurrentHistory.revisions.map((revision) => revision.id))
        .size !== 4
    ) {
      throw new Error(
        "concurrent identity history did not converge on the elected default",
      );
    }
    checks.push(
      "revision.concurrent-branches-single-default",
      "revision.replay-exactly-once",
    );

    await Promise.all([
      waitForActive(defaultConsumer),
      waitForActive(explicitConsumer),
    ]);
    ...
    const concurrentDefault = await client.interaction.browsers.open(
      workspaceId,
      {
        operationId: crypto.randomUUID(),
    ...
    if (concurrentDefaultSession.baseRevisionId !== winner.revision.id) {
      throw new Error(
        "fresh browser did not materialize the concurrent default winner",
      );
    }
    const winningMarker =
      winner.revision.id === left.revision.id ? markerV1 : markerV2;
    assertState(
      (await waitForState(concurrentDefault, winningMarker)).observation,
      winningMarker,
    );
    ...
    if (
      ...
      throw new Error(
        "selecting a future default did not advance exact identity authority",
      );
    }
    ...
    if (
      ...
      selectedReplay.identity.defaultRevisionId !==
        selected.identity.defaultRevisionId
    ) {
      throw new Error(
        "default-selection replay did not return the exact prior result",
      );
    }
    checks.push(
      "identity.select-future-default-cas",
      "identity.select-default-replay-exact",
    );
    ...
    if (
      ...
      !archivedList.identities.some(
        (candidate) =>
          candidate.id === identity.id && candidate.status === "archived",
      )
    ) {
      throw new Error(
        "archived BrowserIdentity visibility did not match the public contract",
      );
    }
    ...
    try {
      ...
      archivedRejected =
        cause instanceof OpenGeniApiError && cause.status === 409;
    }
    if (!archivedRejected)
      throw new Error("an archived BrowserIdentity remained launchable");
    ...
    if (
      ...
      throw new Error(
        "restoring BrowserIdentity changed its explicitly selected default",
      );
    }
    ...
    const selectedDefaultConsumer = await client.interaction.browsers.open(
      workspaceId,
      {
        operationId: crypto.randomUUID(),
    ...
    if (selectedDefaultSession.baseRevisionId !== savedV2.revision.id) {
      throw new Error(
        "restored identity did not materialize its explicitly selected default",
      );
    }
    assertState(
      (await waitForState(selectedDefaultConsumer, markerV2)).observation,
      markerV2,
    );
    checks.push(
    ...
    process.stdout.write(
      JSON.stringify({ status: "passed", output: args.output, checks }) + "\n",
    );
  } finally {
...
async function currentObservation(
  ...
  const target =
    targets.targets.find((candidate) => candidate.selected) ??
    targets.targets[0];
...
async function waitForActive(
  ...
  while (session.lifecycle !== "active" || !session.controller) {
    if (
      ["repair_required", "lost", "ended", "failed"].includes(session.lifecycle)
    ) {
      throw new Error(
    ...
    if (performance.now() >= deadline)
      throw new Error("identity browser did not become active");
    await Bun.sleep(100);
...
function assertState(observation: BrowserObservation, marker: string): void {
  ...
  if (!text.includes(expectedState(marker))) {
    throw new Error(
      `restored browser state was incomplete: ${JSON.stringify(text.slice(0, 500))}`,
    );
  }
}
...
async function createFixture(
  ...
  const publicEndpoint = requiredEnv(
  ...
  const storage = createObjectStorage({
    ...
    objectStorageRegion:
      process.env.OPENGENI_OBJECT_STORAGE_REGION?.trim() || "us-east-1",
    ...
    objectStorageAccessKeyId: requiredEnv(
      "OPENGENI_OBJECT_STORAGE_ACCESS_KEY_ID",
    ),
    objectStorageSecretAccessKey: requiredEnv(
...
function parseArgs(argv: string[]): Args {
  ...
  const apiUrl = new URL(values.get("--api-url") ?? "http://127.0.0.1:8200")
    .origin;
  return {
    apiUrl,
    ...
    output: resolve(
      values.get("--output") ??
        `.agent/evidence/interaction-identity-live-${Date.now()}.json`,
    ),
  };
}
...
async function defaultWorkspace(apiUrl: string): Promise<string> {
  ...
  if (!response.ok)
    throw new Error(`access discovery returned ${response.status}`);
