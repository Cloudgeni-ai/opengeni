...
import {
  ...
  RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_MEASUREMENT,
  RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_BUDGET,
  RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_MEASUREMENT,
...
describe("web bundle budget policy", () => {
  ...
  test("retains the exact Rig verification recovery plus current-main envelope", () => {
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_MEASUREMENT).toBe(2_288_890);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_BUDGET).toBe(2237 * KIB);
    expect(
      RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_BUDGET -
        RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_MEASUREMENT,
    ).toBe(1_798);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_MEASUREMENT).toBe(641_938);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_BUDGET).toBe(628 * KIB);
    expect(
      RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_BUDGET -
        RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_MEASUREMENT,
    ).toBe(1_134);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_FILE_COUNT).toBe(32);
  });
});
