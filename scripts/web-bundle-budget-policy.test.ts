...
import {
  ...
  DURABLE_COMPOSER_ATTACHMENTS_GZIP_MEASUREMENT,
  DURABLE_COMPOSER_ATTACHMENTS_RAW_BUDGET,
  DURABLE_COMPOSER_ATTACHMENTS_RAW_MEASUREMENT,
...
describe("web bundle budget policy", () => {
  ...
  test("retains the exact durable composer attachment restoration envelope", () => {
    expect(DURABLE_COMPOSER_ATTACHMENTS_RAW_MEASUREMENT).toBe(2_281_075);
    expect(DURABLE_COMPOSER_ATTACHMENTS_RAW_BUDGET).toBe(2229 * KIB);
    expect(
      DURABLE_COMPOSER_ATTACHMENTS_RAW_BUDGET - DURABLE_COMPOSER_ATTACHMENTS_RAW_MEASUREMENT,
    ).toBe(1_421);
    expect(DURABLE_COMPOSER_ATTACHMENTS_GZIP_MEASUREMENT).toBe(639_495);
    expect(DURABLE_COMPOSER_ATTACHMENTS_GZIP_BUDGET).toBe(626 * KIB);
    expect(
      DURABLE_COMPOSER_ATTACHMENTS_GZIP_BUDGET - DURABLE_COMPOSER_ATTACHMENTS_GZIP_MEASUREMENT,
    ).toBe(1_529);
    expect(EFFECTIVE_DIRECT_SESSION_RAW_BUDGET).toBe(DURABLE_COMPOSER_ATTACHMENTS_RAW_BUDGET);
  });
});
