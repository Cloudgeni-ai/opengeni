...
describe("web bundle budget policy", () => {
  test("retains at least one KiB above the exact recovery-custody graph", () => {
    expect(DIRECT_SESSION_RAW_MEASUREMENT).toBe(2_198_819);
    expect(DIRECT_SESSION_RAW_BUDGET).toBe(2149 * KIB);
    expect(DIRECT_SESSION_RAW_BUDGET - DIRECT_SESSION_RAW_MEASUREMENT).toBe(1_757);
