...
describe("web bundle budget policy", () => {
  test("retains at least one KiB above the exact current-main merged graph", () => {
    expect(DIRECT_SESSION_RAW_MEASUREMENT).toBe(2_188_272);
    expect(DIRECT_SESSION_RAW_BUDGET).toBe(2138 * KIB);
    expect(DIRECT_SESSION_RAW_BUDGET - DIRECT_SESSION_RAW_MEASUREMENT).toBe(1_040);
