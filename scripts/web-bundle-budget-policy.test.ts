...
describe("web bundle budget policy", () => {
  test("retains at least one KiB above the exact optional-operator-surface graph", () => {
    expect(DIRECT_SESSION_RAW_MEASUREMENT).toBe(2_197_216);
    expect(DIRECT_SESSION_RAW_BUDGET).toBe(2147 * KIB);
    expect(DIRECT_SESSION_RAW_BUDGET - DIRECT_SESSION_RAW_MEASUREMENT).toBe(1_312);
