...
import {
  ...
  SESSION_ROW_QUICK_ACTIONS_RAW_BUDGET,
  SESSION_ROW_QUICK_ACTIONS_RAW_MEASUREMENT,
...
describe("web bundle budget policy", () => {
  ...
  test("retains the exact session-row quick-actions envelope", () => {
    expect(SESSION_ROW_QUICK_ACTIONS_RAW_MEASUREMENT).toBe(2_284_688);
    expect(SESSION_ROW_QUICK_ACTIONS_RAW_BUDGET).toBe(2233 * KIB);
    expect(SESSION_ROW_QUICK_ACTIONS_RAW_BUDGET - SESSION_ROW_QUICK_ACTIONS_RAW_MEASUREMENT).toBe(
      1_904,
    );
    expect(
      SESSION_ROW_QUICK_ACTIONS_RAW_BUDGET - SESSION_ROW_QUICK_ACTIONS_RAW_MEASUREMENT,
    ).toBeGreaterThanOrEqual(MINIMUM_RAW_HEADROOM_BYTES);
  });
});
