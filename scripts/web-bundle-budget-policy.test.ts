...
describe("web bundle budget policy", () => {
  ...
  test("retains the exact Rig verification recovery plus current-main envelope", () => {
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_MEASUREMENT).toBe(2_290_826);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_BUDGET).toBe(2239 * KIB);
    expect(
      RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_BUDGET -
        RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_RAW_MEASUREMENT,
    ).toBe(1_910);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_MEASUREMENT).toBe(642_203);
    expect(RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_BUDGET).toBe(629 * KIB);
    expect(
      RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_BUDGET -
        RIG_VERIFICATION_RECOVERY_CURRENT_MAIN_GZIP_MEASUREMENT,
    ).toBe(1_893);
