...
describe("web bundle budget policy", () => {
  test("retains at least one KiB above the exact attachment-resolver graph", () => {
    expect(DIRECT_SESSION_RAW_MEASUREMENT).toBe(2_199_854);
    expect(DIRECT_SESSION_RAW_BUDGET).toBe(2150 * KIB);
    expect(DIRECT_SESSION_RAW_BUDGET - DIRECT_SESSION_RAW_MEASUREMENT).toBe(1_746);
