...
import {
  ...
  KIB,
  MANAGED_SOCIAL_SIGN_IN_CURRENT_MAIN_RAW_BUDGET,
  MANAGED_SOCIAL_SIGN_IN_CURRENT_MAIN_RAW_MEASUREMENT,
...
describe("web bundle budget policy", () => {
  ...
  test("retains the exact managed social sign-in current-main envelope", () => {
    expect(MANAGED_SOCIAL_SIGN_IN_CURRENT_MAIN_RAW_MEASUREMENT).toBe(2_226_486);
    expect(MANAGED_SOCIAL_SIGN_IN_CURRENT_MAIN_RAW_BUDGET).toBe(2176 * KIB);
    expect(
      MANAGED_SOCIAL_SIGN_IN_CURRENT_MAIN_RAW_BUDGET -
        MANAGED_SOCIAL_SIGN_IN_CURRENT_MAIN_RAW_MEASUREMENT,
    ).toBe(1_738);
