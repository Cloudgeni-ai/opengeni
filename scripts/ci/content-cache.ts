...
function walkFiles(directory: string, output: string[]): void {
  ...
  if (metadata.isSymbolicLink()) throw new Error(`cache outputs may not be symlinks: ${directory}`);
  if (metadata.isFile()) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true }).sort((a, b) =>
    ...
    if (entry.isSymbolicLink())
      throw new Error(`cache outputs may not be symlinks: ${join(directory, entry.name)}`);
