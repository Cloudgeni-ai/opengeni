...
const nativeInstall = {
  ...
  garage: "On Linux x64/arm64 install pinned Garage 2.3.0 with: bun scripts/install-development-tools.ts --install. Add the reported binDirectory to PATH. Garage downloads are verified against repository SHA-256 pins derived from the existing digest-pinned official OCI image, not an upstream native checksum manifest. On macOS use OPENGENI_DEV_BACKEND=docker; native supervision and official Garage binaries are unsupported. Preflight itself does not download or build Garage.",
