...
const live =
  process.env.OPENGENI_ENABLE_LIVE_TESTS === "true" && Boolean(baseUrl && apiKey && workspaceId);

describe("live SDK streaming", () => {
  test.skipIf(!live)(
    "creates a session and streams an ordered, gap-free turn",
    async () => {
      ...
      expect(
        types.some((type) => type === "agent.message.delta" || type === "agent.message.completed"),
      ).toBe(true);
      ...
      const text = seen
        .filter(
          (event) =>
            event.type === "agent.message.completed" || event.type === "agent.message.delta",
        )
      ...
      expect(replayed.slice(0, seen.length).map((event) => event.id)).toEqual(
