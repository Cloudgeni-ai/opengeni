...
describe("API component integration", () => {
  ...
  test("binds REST user memory to trusted subjects and audits explicit private-admin export", async () => {
    ...
    const serviceSession = await createSession(dbClient.db, {
      accountId: grant.accountId,
      workspaceId: grant.workspaceId,
      initialMessage: "service-created REST memory context",
      resources: [],
      metadata: {},
      model: "scripted-model",
      sandboxBackend: "none",
    ...
    const servicePrivateWrite = await app.request(
      workspacePath(grant.workspaceId, "/knowledge/memories"),
      {
        method: "POST",
        headers: jsonHeaders(serviceSessionToken),
        body: JSON.stringify({
          text: "Service-created sessions cannot claim a human preference.",
          scopeSpec: { type: "user" },
        }),
      },
    );
    expect(servicePrivateWrite.status).toBe(403);
