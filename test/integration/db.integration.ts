...
describe("DB integration", () => {
  ...
  test("keeps repeated access bootstrap read-only and conflict-free with session listing", async () => {
    ...
    expect(
      (
        await listSessionsForSubject(dbClient.db, grant.workspaceId, {
          subjectId,
          limit: 20,
        })
      ).sessions[0]?.id,
  ...
  test("RLS policies isolate knowledge memories for a non-owner app role", async () => {
    ...
    try {
      ...
      await updateKnowledgeMemory(dbClient.db, grantB.workspaceId, proposed.id, {
        status: "approved",
        reviewedBy: "test:rls-memory-curator",
      });
  ...
  test("exact-duplicate save dedupes against approved curated memory", async () => {
    ...
    const curated = await updateKnowledgeMemory(
      dbClient.db,
      grant.workspaceId,
      proposed.id,
      {
        status: "approved",
        reviewedBy: "test:curated-memory-reviewer",
      },
      memoryEmbedder,
    );
    const saved = await saveWorkspaceMemory(
  ...
  test("AC-7: working-set block reflects the memory setting and record state", async () => {
    ...
    expect(block).toContain("### Preference observations (non-authoritative)");
