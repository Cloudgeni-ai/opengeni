...
- A **trigger** points at one source. Its immutable revisions own event matching plus the complete session template. A mutable head selects the current revision and active/paused/disabled status.
...
A Pack may declare `automationTemplates`. Installing a Pack makes these frozen templates available but does not manufacture a provider connection. After the required connection/app is registered, setup instantiates a trigger tied to both the exact Pack installation and template. Pack-owned execution fields are immutable; pause/disable and display-name edits remain allowed. Disabling or uninstalling the Pack prevents a pending run from dispatching.
