...
- **Reconnect completion:** authorization repairs the connection only. It never resumes the prior turn or replays the original tool call. The return surface says that explicitly and asks the user to send a new message to retry. Same-call continuation would require a separately designed durable identity and transactional idempotency fence at least across workspace, session, turn, attempt, tool call, and auth flow; inference polling or blind replay of a potentially side-effectful tool is forbidden.
...
- **I3:** non-technical path — browse → connect → agent uses tool — live with zero manual config; `auth_needed` chip → connect → the UI confirms the original call was not replayed → the user sends a new message and that next turn proceeds.
