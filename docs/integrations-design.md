...
- **Connect/tools-list time:** credential-backed servers are best-effort: publish the authorization-needed event and skip the unavailable server while the turn continues. The Gmail REST bridge preserves the broker's reason at startup as well as tool-call time. For missing native personal authority, the web resolves the exact runtime server to its catalog entry and opens the existing **Use in this conversation** review, including shared-results acknowledgement. It never issues a grant from the event itself.
