...
- Turn startup: cumulative queue, queue-to-provider-dispatch, and queue-to-first-byte percentiles are dashboard SLIs rather than paging alerts because every milestone begins at durable queue time and includes intentional per-session FIFO wait. Page on the freshness-gated Temporal eligible backlog and slot-saturation alerts for worker admission, and on first-byte availability below the configured ratio with at least five terminal samples. Use the phase dashboard and provider request-phase metrics to diagnose attributable platform or provider latency.
