...
`lock_timeout` before each migration body. Migration-specific `SET LOCAL`
settings can override it. A lock-wait timeout fails the Job without recording
that migration in `schema_migrations`; the implicit transaction rolls back its
DDL, so resolve the blocking transaction and retry the forward migration Job.
This is a lock-acquisition limit, not a statement-duration limit or permission
to roll back an already-applied migration. Concurrent-index and batched-backfill
migrations retain their separately governed lock-wait settings.
