...
- Context compaction: the latest durable automatic start on a turn worker has no later successful automatic completion for 15 minutes. This alert follows each resolved model's actual threshold and does not infer pressure from one static token count.
