...
maintenance migration must be selected explicitly; migration 0387 uses
`OPENGENI_DEPLOYMENT_MAINTENANCE_CUTOVER=0387_model_catalog_and_gateway_custom_models`
...
operator-owned singleton, not a boot-time reconciliation loop. Migration 0387
changes the exact runtime-posture table/grant contract, so this is a drained
maintenance cutover rather than a rolling migration. A mixed pre/post-0387
...
2. Stop every API, control worker, and turn worker, then prove no configured
   ...
   ```bash
   export OPENGENI_DEPLOYMENT_MAINTENANCE_CUTOVER=0387_model_catalog_and_gateway_custom_models
...
3. Apply `0387_model_catalog_and_gateway_custom_models.sql`, provision roles,
   ...
   After commit, never restart a pre-0387 image or use it as an application
