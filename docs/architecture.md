...
- Recovery is *explicit*: graceful shutdown or ungraceful death is recoverable only after PostgreSQL proves the exact attempt has no admitted provider call with unknown completion. Typed heartbeat/schedule-to-start timeout failures enter `recoverDispatch`/`recoverSessionDispatch`, **bounded by a per-turn redispatch ceiling of 3**. Generic worker-death closure locks the attempt, pending receipt, and pre-call admission in one transaction. A linked pending receipt wins persistence-only recovery; no admission/receipt permits same-turn redispatch; an admitted but unlinked model/compaction call atomically fails with `ambiguous_model_call` and is never replayed.
- Immediately before every model or compaction provider invocation, the runtime awaits an exact durable `session_turn_model_call_admissions` row (attempt, positive call index, kind, provider/API/model, trigger, generation). Only one admission may remain unlinked per attempt. Provider completion creates the full obligation and atomically inserts `session_turn_persistence_receipts` plus the bidirectional admission↔receipt link. The exact sequence is **admission → provider invocation → complete obligation → atomic admission/receipt link → persistence-only phases → receipt settlement**. Function tools use their analogous provider-stable call-id registration before `invoke`. Temporal receives only a strict version-2 reference (receipt/turn/trigger/generation/attempt/kind/digest, at most 1 KiB), never provider output, history, usage, tool arguments, or compaction rows. Persistence failure runs only `persistTurnHandoffAndRecover`; a death before heartbeat is repaired by atomic row discovery. Receipt-fenced rejection reloads receipt+attempt: concurrent settlement continues, live pending retries, replaced ownership is stale. The control activity cannot import provider/runtime/tool execution, so response loss and old/new overlap replay persistence only. Malformed/mismatched evidence or an admitted+unlinked call atomically quarantines the exact attempt/turn/session with unknown-effect audit truth instead of generic redispatch.
...
5. **Execute.** `runAgentTurn` registers a first-class exact attempt, durably admits each exact model/compaction call before provider invocation, builds + runs the OpenAI Agents SDK stream inside a sandbox, publishes realtime events, writes attempt-fenced conversation truth to `session_history_items`, meters/bills usage when supplied, and atomically settles the turn (`idle` / `requires_action` / `recovering` / `failed` / `cancelled`). Provider completion atomically links its admission to the full pending PostgreSQL persistence receipt before ordinary writes; the internal `persistence_pending` result/heartbeat transfers only its bounded opaque reference to the retryable control lane. It is never a public session status or a full-turn retry. An unlinked admission is unknown-effect quarantine, not a retry signal.
...
- **Ungraceful death (heartbeat timeout):** surfaces as a typed `ActivityFailure` carrying the exact Temporal activity id. The `recoverDispatch` activity (backed by `recoverSessionDispatch`) locks the exact attempt, checks for a pending receipt, and only then either returns that receipt or closes the attempt for same-turn recovery. This single transaction prevents receipt establishment from racing generic redispatch. Recovery remains bounded by the per-turn death ceiling; exceeding it atomically fails the exact turn/session.
- **Persistence boundary at death:** the DB-only control activity first reconciles the exact attempt's pending PostgreSQL receipt (history/accounting/exact event or prepared compaction, then same-turn recovery). It uses the bounded reference from the last heartbeat/result when present and discovers the receipt under the same locks that fence attempt closure when death happened after its commit but before any heartbeat. Every receipt-fence rejection reloads exact receipt/attempt truth; a concurrently settled live receipt advances to recovery, a still-pending live receipt retries, and only superseded ownership is stale. A malformed reference, digest/identity mismatch, or invalid stored obligation atomically quarantines and fails closed. PostgreSQL confirmation decides durability; a NATS fanout failure cannot make inference or a tool eligible to run again.
...
The worker has two deliberately different retry domains. `runAgentTurn` remains
non-retryable because it can sample providers and execute tools. Each provider
call is durably admitted first; only provider completion may atomically link
that admission to its full receipt. The bounded control proxy may retry
`persistTurnHandoffAndRecover` indefinitely across pod loss because that
activity accepts a validated exact receipt and can reach only fenced Postgres
persistence plus NATS fanout. Canonical receipt parsing is pure and shared by
workflow replay and the control activity; completed model calls persist
`history → optional meter/event`, while completed compactions persist
`optional meter/event → prepared apply/skip`, then settle the receipt before
same-turn recovery. An admitted call with no linked receipt never enters this
retry domain.
...
| Completed-effect persistence handoff / persistence-only retry | `apps/worker/src/turn-persistence-handoff.ts`, `apps/worker/src/activities/session-state.ts`, `packages/events/src/index.ts` | [`run-lifecycle.md`](run-lifecycle.md) |
