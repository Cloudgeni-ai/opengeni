...
- **Session**: a durable, long-lived conversation/workspace context (one Postgres row + a Temporal workflow). Lifecycle status flows through `queued`/`running`/`requires_action`/`recovering`/`waiting_capacity`/`idle`/`failed`/`cancelled`. Pause is a separate recursive admission-control projection, never a lifecycle status.
- **Turn**: one logical unit of agent work, triggered by a waiting human/API prompt, an approval decision, or one coalesced internal-update batch (goals, schedules, child results, and similar machine updates). Codex capacity recovery preserves the same current turn directly; it does not create an update batch or queue row. Each execution attempt is one **non-retryable activity**. Inside it, the SDK loop makes as many model/tool calls as the work needs.
...
stateDiagram-v2
  ...
  running --> waiting_capacity: all eligible Codex subscriptions unavailable
  waiting_capacity --> recovering: revisioned capacity wake / reset timer
