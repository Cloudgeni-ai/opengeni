...
- **No OpenGeni credential crosses to the machine.** The env mint skips the GitHub-App installation token for a `selfhosted`-effective turn (`sandboxVariable setForRun({ skipGitHubToken })`), and selfhosted `exec` puts `env: {}` on the wire — structurally the token never reaches the machine, which uses its **own** git auth.
...
> Canonical: `apps/worker/src/activities/agent-turn.ts` (the `machinePrimary` branch), `apps/worker/src/activities/variable set.ts` (`skipGitHubToken`), `packages/runtime/src/index.ts` (`repositoryUsesSandboxClone`), `packages/runtime/src/sandbox/selfhosted/session.ts` (`toMachinePath`), `apps/worker/src/activities/sandbox-lease.ts`, [`../AGENTS.md`](../AGENTS.md).
...
- **Env layering precedence (fixed):** deployment allowlist < git identity < workspace variable set < run-scoped GitHub auth (later wins); reserved/loader-injection names are rejected.
