...
- **Session hierarchy**: server-authored immutable parent/root lineage gives roots depth `0`; each child increments depth by one. New descendants are admitted by the persisted nested-agent policy at the DB creation boundary, while legacy deeper trees remain readable/resumable. Depth does not replace recursive Pause/Resume, UI collapse, budgets, or concurrency control. Deep dive: [`nested-agent-depth.md`](nested-agent-depth.md).
...
| [`nested-agent-depth.md`](nested-agent-depth.md) | Server-enforced nested-session depth: precedence, overrides, denial atomicity/idempotency, legacy trees, Pause/budget interaction, and production canary. |
