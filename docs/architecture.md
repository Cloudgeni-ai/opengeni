...
- **Terminal/control race:** graceful, compaction, sandbox-supersession, ordinary terminal, approval, and worker-death paths use the same fenced session→turn settlement order. If interrupt/terminal/newer-dispatch truth already won, the loser returns stale without appending events or changing the current session.
- **Ungraceful death (heartbeat timeout):** surfaces as a typed `ActivityFailure` carrying the exact Temporal activity id. `requeueTurnAfterWorkerDeath` atomically re-dispatches from dual-written conversation truth, bounded by the redispatch ceiling (3); exceeding the ceiling atomically fails the exact turn/session, so the workflow performs no second failure write.
