...
- **CI.** `ci.yml` explains a fail-closed change-impact plan on pull requests, then runs deterministic memory-bounded unit/real-service/browser shards plus selected typecheck, guards, package/image builds, and deployment-artifact validation. Main, scheduled, and manual CI always run the full map to detect selector drift. `scripts/ci/` owns impact, cache, sharding, resource-budget, profiling, and local fast-path mechanics; [`toolchain.md`](toolchain.md) owns their operator contract. Existing protected check names remain stable. `.bun-version` pins this local/CI path; release-workflow pinning and publication remain OPE-25-owned and are not asserted here. `agent-ci.yml`/`agent-release.yml` own the path-filtered, independently versioned Rust agent (`agent-v*` tags).
