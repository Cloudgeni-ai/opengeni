...
| `apps/web` | `opengeni-web` | React 19 + Vite operator console. A thin shell over `@opengeni/sdk` + `@opengeni/react`. Leaf app — nothing imports it. | `apps/web/src/App.tsx`, `apps/web/src/context.tsx`, `apps/web/src/api.ts` |
...
| `react` | `@opengeni/react` | **Published.** React hooks + styled components over the SDK: live streaming, chat, timeline, sandbox surfaces, and provider-neutral voice dictation contracts/control. Connected-machine UI (machines dashboard, enrollment, status) lives on the opt-in **`@opengeni/react/machines`** subpath so the root import is the clean sandbox-only default and machine-free consumers never pull it in (the root re-exports it deprecated for back-compat, #144). | `OpenGeniProvider`, the `use*` hooks, `ChatComposer`/`MessageTimeline`/`WorkspaceDock`/`VoiceDictationControl`/etc., CSS subpath exports; `./machines` for machine UI; `./transcription/openai-realtime` for the optional browser adapter. | `packages/react/src/index.ts`, `packages/react/src/machines.ts`, `packages/react/src/transcription/types.ts`, `packages/react/src/client.ts` |
...
The experimental voice-dictation broker in `routes/transcription.ts` is a deliberately narrow, default-off provider boundary. The authoritative typed `workspaces.settings.transcription` policy must name the exact direct-OpenAI project/endpoint, no-retention/no-training/ZDR and residency eligibility, security/finance approvals, and bounded concurrency/rate/duration/cost values; the web omits the mic when this contract is absent, disabled, or malformed, and the broker rechecks it before provider access and under serialized database admission. `sessions:control` authorization precedes parsing, the supplied UUID must be a real session in the workspace, and `transcription_grants` provides FORCE-RLS one-active-session admission, conservative pre-provider reservation, usage/audit idempotency, expiry reconciliation, and exact workspace/subject/session/provider settlement binding under canonical/static limits. The deployment key never enters the browser or ledger. The browser sends SDP/audio directly to OpenAI through the lazy `@opengeni/react/transcription/openai-realtime` adapter; a disconnect replaces the grant, peer, channel, and SDP exchange within a bounded retry/setup deadline while fencing old generations. Partial text remains browser-ephemeral and only accepted finals enter the ordinary editable composer draft, so neither the broker nor recovery writes `session_events`, creates queue/events/attachments, or bypasses existing send/queue/steer/storage semantics. Provider-side eligibility, billing validation, settings UI/BYOK, fallback, and production acceptance gaps are recorded in [`design/transcription-and-realtime-voice-2026-07-19.md`](design/transcription-and-realtime-voice-2026-07-19.md).
...
Bootstraps `/v1/config/client`, resolves the auth mode, gates, then loads access context + workspaces. Its bespoke HTTP is client-config bootstrap, Better Auth `/v1/auth/*`, and the experimental short-lived transcription credential broker; everything else goes through the SDK. Routes are **hand-written** in `App.tsx` (TanStack Router generation is OFF). `context.tsx` is the cross-route hub. Almost all session/streaming logic lives in `@opengeni/react`, not here. The session route lazy-loads the OpenAI browser adapter so the default session bundle does not absorb provider code. Watch-outs: `/clear-view` is local/this-device-only; stale-bundle auto-reload on revision mismatch; one GitHub installation per session.
...
| Voice dictation / transcription | `packages/react/src/transcription/types.ts`, `packages/react/src/hooks/use-transcription.ts`, `apps/api/src/routes/transcription.ts`, `apps/web/src/routes/session.tsx` | [`design/transcription-and-realtime-voice-2026-07-19.md`](design/transcription-and-realtime-voice-2026-07-19.md) |
