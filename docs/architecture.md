...
- Recovery is *explicit*: graceful shutdown checkpoints a replay-safe turn as `recovering`; ungraceful death is detected via typed heartbeat/schedule-to-start timeout failures and re-dispatched through `recoverSessionDispatch`, **bounded by a per-turn redispatch ceiling of 3** before the session fails for real. Accepted/acceptance-unknown work first persists an exact-attempt/generation `failed_idle` no-replay disposition with an `incomplete` conversation checkpoint before final SDK-history access; the final-history transaction can only upgrade that marker to `complete` atomically with the rows. Worker-death recovery consumes either monotonic state into terminal turn truth rather than re-dispatching. Incomplete history refuses automatic goal/internal continuation, while a locked canonical `queuedHumanWork` result keeps any independent queued human/API turn eligible for the next FIFO peek/claim.
...
- Recoverable conditions preserve the session and context, but only boundaries with explicit idempotency resume the same turn. Provider rate-limit/5xx/network uncertainty, accepted Responses invalid content, and escaped MCP timeouts first establish an exact-attempt no-replay fence, mark the **current turn** failed, and never replay that provider call, full turn, or completed tool. Final history atomically upgrades the fence from `incomplete` to `complete`. Only `complete` may pace a new active-goal continuation; `incomplete` is still terminal for the failed turn and suppresses goal/internal continuation, but an independent queued human/API turn remains canonical queue work and must reach the next FIFO claim. OPE-64 compaction recovery, explicit worker-death recovery, and definitive credential failover retain their separately fenced same-turn semantics. The max-turns segment cap and budget/credit exhaustion idle gracefully. Repeated productive compactions are allowed; no-shrink recovery idles instead of churning. Resolved model metadata controls raw/effective/auto-compact limits end-to-end (Codex subscription: 272,000 / 258,400 / 244,800), and local checkpoint rebuilds retain one cumulative 20,000-token newest-user-message budget. A failed session would reject the very retry it asks for.
...
- **Provider loss is an exact-instance fence, never an operation retry.** A definitive provider `NOT_FOUND` retires only the matching warm `(lease_epoch, instance_id)` and advances the lease epoch once. The winning observer reports pending/degraded/unrecoverable recovery; concurrent stale observers report superseded. A routed operation that discovered the loss is never replayed because a mutation's provider outcome is ambiguous. Drain teardown likewise requires a verified capture before provider deletion; `NOT_FOUND` before capture preserves an existing archive or records typed unrecoverable state when none exists.
- **A route fence does not prove a mutation was rejected.** Read-only operations retain bounded epoch-fence re-resolution. Mutating exec/PTY/write/materialization/desktop/editor operations are invoked once when typed transport or fence evidence leaves acceptance unknown, then surface `sandbox_mutation_acceptance_unknown` with the exact active epoch and no raw provider cause or guessed execution identity. Only a trusted typed `neverSent`/`fenced` admission proof (or a future backend preserving the exact provider execution identity) permits bounded retry. DNS/gRPC `UNAVAILABLE` is transport uncertainty, not authoritative provider `NotFound`, even when nested prose mentions absence. Canonical: `packages/runtime/src/sandbox/routing/routing-session.ts`; provider existence and liveness remain in the backend resolver/provider layer.
- **Never terminate a box whose `/workspace` snapshot could not be captured** — a capture failure or empty/unverified result re-throws *before* any terminate.
...
- **Ungraceful death (heartbeat timeout):** surfaces as a typed `ActivityFailure` carrying the exact Temporal activity id. `recoverSessionDispatch` recovers the same turn from durable conversation truth only when no terminal no-replay fence exists, bounded by the per-turn death ceiling; exceeding it atomically fails the exact turn/session. An exact accepted/acceptance-unknown `failed_idle` marker is instead terminally consumed without a replacement attempt. A `complete` marker reports a successful conversation checkpoint; an `incomplete` marker reports failure and refuses automatic goal/internal continuation, but neither permits redispatch. The same locked result carries whether separately queued human/API work exists so a baselined prompt cannot be stranded. The closed attempt and marker preserve that exact classification if a terminal settlement commits but its activity response is lost.
