...
- **Turn capacity uses the Temporal activity queue, never the visible prompt queue.** A prompt behind a paused session is durable human input but is not runnable and never schedules `runAgentTurn`. `DescribeTaskQueue` backlog count/age is therefore the eligible-runnable backlog signal; a missing/malformed stats object is a stale read, never fresh zero, and alerts require an exact namespace/release/environment sample less than 45 seconds old (including an explicit absent-series branch). Dedicated turn workers reserve Temporal activity slots through a cgroup-aware supplier before polling: density remains capped at 16, each reserved turn is budgeted at a hard 100 MiB, and 512 MiB of native/runtime headroom is retained. Live cgroup usage contracts new availability. Both pre-construction and post-native-construction startup checks must still admit one safe turn. Before driver decoding, active model-facing history is server-measured and rejected above 32 MiB UTF-8 JSON, 4,096 rows, 200,000 decoded nodes, or 100,000 object properties; it is never silently trimmed. Approval-only SDK RunState is separately bounded at 32 MiB/200,000 nodes/100,000 properties with 256 KiB/256-item pending approvals, and ordinary turns read only its ownership metadata rather than double-holding the blob with conversation history. Canonical: `apps/worker/src/concurrency.ts`, `packages/db/src/index.ts`, [`design/turn-worker-density-2026-07-16.md`](design/turn-worker-density-2026-07-16.md).
