...
- A completed function-tool call is registered by provider-stable call id before its `invoke` begins. A completed model/compaction response heartbeats one exact persistence obligation before lease renewal, history, accounting, event, or replacement writes. Persistence failure or ambiguous commit runs only the retryable DB control activity `persistTurnHandoffAndRecover`; it confirms the exact receipt and then recovers the same turn. It cannot import provider/runtime/tool execution, so activity-response loss and old/new worker overlap can replay persistence but never the completed inference/effect. A malformed present handoff fails closed rather than entering generic worker-death redispatch.
...
- **Persistence boundary at death:** a valid pending receipt in the last heartbeat is completed first by the DB-only control activity (history/accounting/exact event or prepared compaction, then same-turn recovery). A present invalid receipt refuses redispatch. PostgreSQL confirmation decides durability; a NATS fanout failure cannot make inference or a tool eligible to run again.
