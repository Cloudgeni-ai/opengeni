...
- Recovery is *explicit*: graceful shutdown checkpoints the same turn as `recovering`; ungraceful death is detected via typed heartbeat/schedule-to-start timeout failures and re-dispatched through the `recoverDispatch` activity backed by `recoverSessionDispatch`, **bounded by a per-turn redispatch ceiling of 3** before the session fails for real. Generic worker-death closure locks the exact attempt and checks its pending receipt in the same transaction; a receipt established before those locks is returned for persistence-only recovery, while establishment after closure is fenced.
- A function-tool call is registered by provider-stable call id before its `invoke` begins. Before Temporal receives any post-effect handoff, the worker inserts the full tool/model/compaction persistence obligation and SHA-256 digest as one pending PostgreSQL receipt owned by the exact attempt. Temporal then receives only a strict version-2 reference (receipt/turn/trigger/generation/attempt/kind/digest, at most 1 KiB), never provider output, history, usage, tool arguments, or compaction rows. Persistence failure or ambiguous commit runs only the retryable DB control activity `persistTurnHandoffAndRecover`; a death before the heartbeat is repaired by atomically discovering the pending receipt while fencing generic attempt closure. A receipt-fenced write rejection reloads the exact receipt and live attempt: concurrent settlement continues recovery, a live pending receipt retries the control activity, and only replaced ownership is stale. That activity cannot import provider/runtime/tool execution, so activity-response loss and old/new worker overlap can replay persistence but never the completed inference/effect. Malformed or mismatched evidence atomically quarantines the receipt and fails the exact live attempt/turn/session with unknown-effect audit truth rather than entering generic worker-death redispatch.
...
non-retryable because it can sample providers and execute tools. The bounded
control proxy may retry `persistTurnHandoffAndRecover` indefinitely across pod
loss because that activity accepts a validated exact receipt and can reach only
fenced Postgres persistence plus NATS fanout. Canonical receipt parsing is pure
and shared by workflow replay and the control activity; completed model calls
persist `history → meter → exact event`, while completed compactions persist
`meter → exact event → prepared apply/skip`, before same-turn recovery.
