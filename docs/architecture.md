...
- Recoverable conditions **idle or explicitly resume** the session (keep context) rather than fail it: retryable provider failures, escaped MCP request timeouts (checkpointed without replaying completed tool calls), provider context overflow after strict-shrink compaction recovery, the max-turns segment cap, and budget/credit exhaustion. Repeated productive compactions are allowed; no-shrink recovery idles instead of churning. Resolved model metadata controls raw/effective/auto-compact limits end-to-end (Codex subscription: 272,000 / 258,400 / 244,800), and local checkpoint rebuilds retain one cumulative 20,000-token newest-user-message budget. A failed session would reject the very retry it asks for.
...
- **Credential recovery is long-lived; connections are not.** The enrollment `oge_` bearer lasts 30 days and remains subject to an active-row + exact-generation DB check. NATS user JWTs and `ogr_` relay producer tokens last at most five minutes. One minute before the earliest absolute expiry, the agent bearer-authenticates `POST /v1/enrollments/self/refresh`, validates exact identity and consent, atomically persists the replacement before publishing it in memory, and gives each new connection an immutable credential snapshot. Refresh and revoke serialize on the same enrollment-row lock; live relay sockets disconnect exactly at their credential expiry. PTY reader/writer workers and the child process remain owned outside that socket lifecycle, while PTY and desktop pumps redial indefinitely with bounded full-jitter, snapshot the refreshed producer credential, and reserve terminal settlement for typed close/fence/protocol rejection or a real local exit. This persistence begins only after the first-byte/frame readiness barrier: a typed startup timeout or failure signals cancellation and joins the half-started supervisor (including PTY process cleanup) before returning, while dropping the registration future signals cancellation and lets the supervisor finish that cleanup asynchronously instead of leaking an unreachable background worker.
- **Revoked machines stay history but leave routing.** Revocation clears only matching active-sandbox pointers and increments each affected session's `active_epoch`. The revoked enrollment remains available to administrators as history, but fleet discovery omits it and turn routing classifies a revoked/missing enrollment as `offline_enrollment`. Loss of a machine-home enrollment fails loudly rather than provisioning a phantom cloud box.
- **Native lifecycle cleanup fails closed.** Linux probes user and system systemd unit paths independently. macOS persists only a per-user LaunchAgent plist and uses exact `launchctl bootout gui/<uid> <plist>` / `bootstrap gui/<uid> <plist>` operations. An ambiguous cleanup preserves the binary and credentials and blocks remote revoke.
...
**"Two tokens, one key."** The agent and the viewer dial **independently** with **different** tokens but the **same** channel key: the agent presents an enrollment-identity-scoped, five-minute `ogr_` **producer** token (`verify_relay_token`); the viewer presents the control-plane-minted `ogs_` **stream** token (`verify_stream_token`), validated **including the lease/active-epoch fence** so a stale-epoch viewer can never reach a swapped-away box. The relay validates each side on its own merits, disconnects the live socket at that side's exact token expiry, and pairs by key — the agent never mints the viewer token. The Rust token verify is the single place the relay and the TypeScript control plane must agree on the HMAC envelope; it mirrors `packages/contracts`'s `signStreamToken`/`signRelayToken` byte-for-byte (proven by `agent/crates/opengeni-relay/tests/cross_stack_token.rs`).
...
**Rigs.** A workspace can also own named **rigs**: versioned sandbox machine definitions (image + setup script + self-declared health checks + credential-hook refs + default variable-set refs). A session freezes its bound rig's active version at creation; a rig version's image is the **top** of image precedence — rig > pack > deployment default — overriding both. `rigs:use` can propose an exact already-worked `setup_append` and request clean verification, but a green run only marks the change verified/awaiting-manager; **every** durable version mint or activation requires `rigs:manage` through the manager REST/UI path. Rig CI executes the exact candidate setup artifact in one Bash process with hard timeouts and never injects variable-set values. The rig-setup hook runs once per box, marker-guarded, and is skipped entirely on `selfhosted` (Connected Machine) turns. See [`rigs.md`](rigs.md).
...
- **Principal model: membership authorizes people; credentials authorize themselves.** Workspace grants resolve from two independent sources: `workspace_memberships` rows (subjects `user:*` — the *only* authorization source for people, so a missing row means revoked) and self-carried grants (a workspace-scoped `api_keys` row → subject `api_key:<id>`; a signed delegated token → its payload grants). A non-user subject *may* also hold a membership row (roster surface + per-member personal state such as session pins), but its absence must never deny it — **never use "has a membership row" as an authorization or liveness check for arbitrary subjects** (doing so 403'd every workspace-scoped API key in production, 2026-07-13). Per-subject personal state (pins, session-list snapshots) is membership-locked and removal-cleaned for members, TTL-bounded for everyone else.
- **The deployment shared-key boundary.** `x-opengeni-access-key` gates the whole deployment (with an exempt-path allow-list: `config/client`, auth, Stripe webhook, GitHub callbacks, install, and five method-specific Connected-Machine POSTs: device start/poll, token exchange, bearer-authenticated self-refresh, and bearer-authenticated self-revoke). This is a *different header* from product/delegated `Authorization: Bearer`; approval, lookup, mint, list, and administrator revoke stay protected.
...
- **Cryptographic per-workspace NATS isolation.** Each agent connection is scoped by a signed JWT lasting at most five minutes to `agent.<ws>.>` + `_INBOX.>`; the auth-callout fails closed (a responder bug never grants access) and requires the active enrollment's exact credential generation.
- **Relay token isolation.** The pixel/terminal data plane is gated by two distinct HMAC tokens (`ogr_` producer, `ogs_` viewer) over a shared channel key. The producer token lasts five minutes and live sockets expire with their credential; the viewer token is fenced on the active lease epoch (§7.11), so a stale-epoch viewer can never reach a swapped-away box.
...
| [`rigs.md`](rigs.md) | Rigs: versioned sandbox machine definitions, frozen per-session binding, exact-artifact verification, manager-only promotion, `rigs:use`/`rigs:manage`, MCP surface. |
