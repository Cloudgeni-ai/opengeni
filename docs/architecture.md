...
- **Sandbox create is tracked before setup.** A worker that wins `cold -> warming` must persist the provider instance id on the lease immediately after provider create returns, before readiness/display/setup. If later setup fails, it terminates the just-created sandbox before retrying. A turn waiting on another worker's warming lease is bounded by `OPENGENI_SANDBOX_WARMING_TIMEOUT_MS` (default 600000) and fails clearly on capacity/create timeout instead of heartbeating forever. Standalone rig verification must never manufacture a fake turn/viewer lease. Its bounded exact-instance ownership ships through a mandatory two-phase shared-queue rollout: Phase A installs a tenant-consistent `rig_verification` owner registry and makes every Modal reaper recognize active, unexpired exact owner rows while verifier creation remains on the legacy path; Phase B adds exact owner registration behind a strict default-off, pre-create gate and may activate only after live proof that no Phase-A-predecessor reaper remains. The reaper's initial unified ownership projection is not authoritative for destruction because registration can commit while provider enumeration is in flight: every candidate is re-read through the exact current-instance database projection immediately before provider termination, and a failed or inconsistent revalidation preserves the box for a later sweep. The common two-minute create/registration grace also covers fresh stale-tagged candidates; older wrong, expired, stale, and unattributed instances remain collectible. Every exact verifier identity is retained before the registration await so commit/ack ambiguity can still be deactivated, and provider termination settles independently. The verifier activity heartbeats through bounded cleanup and uses `WAIT_CANCELLATION_COMPLETED`; a local 13-minute deadline within its 15-minute server timeout feeds setup/check commands through the runtime process-cancellation controller, then reserves two minutes for command quiescence, exact deactivation, and provider termination. Cleanup calls all start independently and are bounded inside that reserve. A hard worker loss or unresolved call does not falsely claim `finally`: registered boxes fall back to the 20-minute owner TTL plus reaper cadence, while never-registered provider creates fall back to the common two-minute orphan grace plus cadence. The runtime app role can read tenant-visible owner rows but cannot mutate the registry directly; pinned tenant-fenced functions own register/rebind and exact deactivation, and repeated role provisioning preserves that exception. Provider tags are diagnostic and never authorize retention. Canonical rollout and rollback contract: [`rigs.md`](rigs.md#bounded-verifier-ownership-rollout). Canonical ownership code: `apps/worker/src/activities/rig-verification.ts`, `packages/db/src/index.ts`, `apps/worker/src/activities/sandbox-lease.ts`, `packages/runtime/src/sandbox/providers/modal.ts`.
