...
- **CI.** `ci.yml` explains a fail-closed change-impact plan on pull requests, then runs deterministic memory-bounded unit/real-service/browser shards plus selected typecheck, guards, package/image builds, and deployment-artifact validation. Main, scheduled, and manual CI always run the full map to detect selector drift. `scripts/ci/` owns impact, cache, sharding, resource-budget, profiling, and local fast-path mechanics; [`toolchain.md`](toolchain.md) owns their operator contract. Existing protected check names remain stable. `release.yml` uses the same `.bun-version` pin for changesets Version-PR/publish + GHCR images (dormant until `RELEASE_ENABLED=true`); `agent-ci.yml`/`agent-release.yml` own the path-filtered, independently versioned Rust agent (`agent-v*` tags).
