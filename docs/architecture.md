...
### 5.6 Durable waits and one-shot background jobs

An existing signed turn can suspend without inference on structured human input (`ask_user`), one absolute timestamp (`wait_until`), one authenticated matching event (`wait_for_event`), or one asynchronous provider command (`start_background_job`). PostgreSQL owns the wait/job rows and terminal transition; the session workflow reconstructs timers after restart/`continueAsNew`; revisioned workflow-wake outbox repair covers commit→signal loss; and NATS remains best-effort live fanout. `ask_user` privately resumes the same saved SDK `RunState` exactly once. Passive wait and job terminals add one nested `scheduled_occurrence`, preserving the closed internal-update union and human-only visible queue. A background provider start is fenced once; replacement controllers reattach, while a vanished provider becomes `lost` and is never rerun. Recurring task definitions are a different system. Deep dive: [`durable-waits-and-background-jobs.md`](durable-waits-and-background-jobs.md).

> Canonical: `packages/db/src/index.ts` (durable wait/job stores), `apps/worker/src/workflows/session.ts`, `apps/worker/src/workflows/background-job.ts`, `apps/api/src/routes/durable-waits.ts`, [`durable-waits-and-background-jobs.md`](durable-waits-and-background-jobs.md).

### 5.7 Scheduled tasks (cron)
...
### 5.8 Usage metering, entitlements & billing
...
| Durable waits / ask-user / event ingress / one-shot background jobs | `packages/db/src/index.ts`, `apps/worker/src/workflows/session.ts`, `apps/worker/src/workflows/background-job.ts`, `apps/api/src/routes/durable-waits.ts` | [`durable-waits-and-background-jobs.md`](durable-waits-and-background-jobs.md) |
...
| [`durable-waits-and-background-jobs.md`](durable-waits-and-background-jobs.md) | Existing-session ask/timer/event waits and one-shot asynchronous command architecture, surfaces, and invariants. |
