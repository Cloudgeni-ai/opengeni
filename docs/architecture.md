...
- **Typecheck/check/prep.** `typecheck` discovers every app/package `tsconfig.json` plus the typed CI scripts and runs tsgo through a cgroup-aware bounded pool (one worker by default; measured runners may raise it explicitly). `check` is the explained, memory-bounded impacted path (including selected client/app builds); `check:full`/`prep` force the full mapped unit and real integration/E2E tiers. Dedicated desktop-image, provider-live, workspace-capture, and Rust op-stream gates remain explicit rather than becoming skip-only claims. `check:workspace-billing` retains the dedicated static auth/billing aggregate.
- **Static guards (build steps).** `scripts/publish-closure-guard.ts` validates the npm graph without auto-building unrelated packages and requires current `sdk`/`react` dist only when those clients were selected; `scripts/check-workspace-billing-static.ts` rejects unscoped `/v1` routes and misplaced Stripe/Better Auth; `scripts/check-docs-refs.ts` verifies current-tier references; `source-hygiene.test.ts` rejects raw NUL bytes.
...
- **CI.** `ci.yml` explains a fail-closed change-impact plan on pull requests, then runs deterministic memory-bounded unit/real-service/browser shards plus selected typecheck, guards, package/image builds, and deployment-artifact validation. Main, scheduled, and manual CI always run the full map to detect selector drift. `scripts/ci/` owns impact, cache, sharding, resource-budget, profiling, and local fast-path mechanics; [`toolchain.md`](toolchain.md) owns their operator contract. The protected aggregate check name remains `Typecheck and unit tests`. `.bun-version` pins this local/CI path. `release.yml` remains the separate evidence-bound package/public-image release path; OPE-25 and the private ops repository remain the sole workload deployment dispatcher. `agent-ci.yml`/`agent-release.yml` own the path-filtered, independently versioned Rust agent (`agent-v*` tags).
