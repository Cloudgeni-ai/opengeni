...
flowchart LR
  ...
  API -.->|"post-commit revisioned signalWithStart"| TQ
  TQ -->|"dispatch turn"| W
  W <-->|"exec / fs / git"| SB
...
3. **Create session.** The REST route and Agent MCP tool converge on `createSessionForRequest`; the SDK forwards the same `CreateSessionRequest` JSON, including the create idempotency key and deprecated variable-set alias. Core validates payload + resources + tools + variable set + model (`assertConfiguredModel` → 422 on unknown) and checks usage limits/entitlements before `initializeSessionStartAtomically` commits one canonical initialization transaction. That transaction includes the session row; resource, variable-set, rig, MCP, parent, sandbox-group, active-sandbox, and working-directory references; an optional goal; the canonical event prefix; the first user turn or scheduled system update; queue/source/usage settlement; the stable Temporal workflow id; and, when admission is currently open, an `initial_session` wake revision. It sets `initialization_version = 1` only after the complete shape exists. A workspace-scoped create idempotency key plus normalized `v1:<sha256>` request fingerprint makes an exact response-loss retry verify and return the committed receipt, then re-deliver its same wake revision; a different fingerprint or a malformed/incomplete receipt is a 409. Retry preserves current authorization and stable shape validation but returns before mutable resource, capability, variable-set, sandbox, and quota lookups can reinterpret the already committed request. `signalWithStart` happens only after commit through the ordinary revisioned wake-outbox delivery path, so a process death cannot expose a partial session or strand committed work. The rolling migration does not backfill old rows: version-0 sessions keep their normal continuation behavior, while a keyed create retry that tries to reinterpret one fails closed rather than fabricating a goal, event, turn, or wake. A create may attach per-session third-party MCP servers with write-only encrypted headers (`CreateSessionRequest.mcpServers`); responses/events expose only metadata, and later `user.message.mcpCredentialUpdates` rotate headers for the next turn. A create may target a **Connected Machine** with `CreateSessionRequest.targetSandboxId` (+ optional `workingDir`, which is a **422 without a `targetSandboxId`**): the active-sandbox pointer is seeded in that same transaction — race-free, committed before the worker turn can read it — so the **first** turn routes to the chosen machine and lands in its working directory; an invalid/unowned/offline target **422s** (never a silent fall-back to the default box). Canonical: `apps/api/src/routes/sessions.ts`, `apps/api/src/mcp/server.ts`, `packages/sdk/src/client.ts`, `packages/core/src/domain/sessions.ts`, `packages/core/src/domain/session-create-fingerprint.ts`, `packages/db/src/index.ts`, `packages/contracts/src/index.ts` (`CreateSessionRequest`), [`run-lifecycle.md`](run-lifecycle.md), [`session-mcp-servers.md`](session-mcp-servers.md).
