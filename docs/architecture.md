...
- **Missed events are backfilled from Postgres by `sequence`.** The SSE stream replays durable events in bounded pages, subscribes live, and on a sequence gap **backfills from the DB**, deduping by sequence. During replay it retains only the newest live notification and then gap-fills, so a hot producer cannot create an unbounded API buffer. The streaming core *refuses to deliver with a gap or without monotonic cursor progress* (it throws rather than skip or spin).
...
- **Event/output representations are deliberately distinct.** They must not be described as interchangeable views of one retained blob:
  ...
  2. `session_events` is a redacted, append-only human/audit **preview** with a 64 KiB payload envelope. The application preserves useful deterministic head/tail/identity facts; a rolling-safe database trigger supplies a content-free fallback for old writers, and a `NOT VALID` check enforces new writes without rewriting historical rows. Inline media becomes a `media_preview`; its source bytes are not materialized as base64 in the event and are not retained by this generic path.
  3. NATS/SSE/REST are bounded transport projections over durable audit previews: NATS chunks exact encoded messages, each SSE frame is bounded, and REST returns byte-bounded prefix/suffix pages with truthful continuation headers. Historical oversized rows are normalized defensively at each outbound legacy guard.
  4. The browser keeps only the newest 10,000 events within an 8 MiB serialized window (plus a per-event legacy guard). Resume sequence and latest session-status projection survive eviction; `hasOlder`/`windowTruncated` make the loss visible.
