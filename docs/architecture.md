...
- **All workspace-scoped tables use forced row-level security** (RLS). In standalone `force` topology, API and worker pools connect as the exact non-owner, `NOINHERIT`, non-superuser, non-`BYPASSRLS` `opengeni_app` login and **must** run reads/writes inside an RLS context wrapper (`withWorkspaceRls`/`withAccountRls`) or rows are invisible. API/worker startup and readiness catalog-check that identity, its zero-membership/zero-ownership posture, exact grants, and active FORCE-RLS policies before admitting work.
...
| `db` | `@opengeni/db` | Postgres data layer: Drizzle schema (68 `pgTable` declarations), forward-only SQL migrations, runtime role/RLS posture, role provisioning, the typed repository API. | `createDb`, `with*Rls` wrappers, `assertRuntimeDatabasePosture`, hundreds of CRUD/transactional helpers; the **tables themselves are a cross-service contract**. | `packages/db/src/schema.ts`, `packages/db/src/runtime-posture.ts`, `packages/db/src/provision-roles.ts`, `packages/db/drizzle/` |
...
- **RLS posture.** Every workspace-scoped table has FORCE RLS + a `workspace_isolation` policy. Standalone API/worker traffic runs as exact role `opengeni_app`: `LOGIN`, `NOSUPERUSER`, `NOBYPASSRLS`, `NOCREATEROLE`, `NOCREATEDB`, `NOREPLICATION`, `NOINHERIT`, zero memberships, zero ownership, no database/schema CREATE, and DML only on `RUNTIME_DML_TABLES`. `withRlsContext` sets and reads back both `opengeni.account_id` and `opengeni.workspace_id` on the same transaction backend before any scoped query. A query that skips the wrapper silently returns **zero rows** (not an error). `assertRuntimeDatabasePosture` checks catalogs only, in a repeatable-read/read-only transaction, and gates API/worker startup plus readiness. `rlsStrategy=scoped` is the explicit embedded-host exception: it checks coherent connection identity while the host owns its separate isolation/ownership contract. Workspace env-var values live only as AES-256-GCM ciphertext and are never returned by any API.
...
- **Dev stack.** `bun run dev` (= `scripts/dev-stack.sh`): copy `.env`, pick free ports, `docker compose up` Postgres/NATS/Temporal/MinIO, migrate through `OPENGENI_MIGRATIONS_DATABASE_URL`, provision the exact `opengeni_app` role/grants, build the sandbox image, then launch api+worker+web through the restricted `OPENGENI_DATABASE_URL`. It auto-selects alternate ports if defaults are taken.
