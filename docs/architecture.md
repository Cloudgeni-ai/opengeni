...
The experimental voice-dictation broker in `routes/transcription.ts` is a deliberately narrow provider boundary: `sessions:control` authorization and no-retention/no-training payload validation happen before the API uses its direct OpenAI key to mint a 60-second transcription client secret. The key never enters the browser. The browser then sends SDP/audio directly to OpenAI over the official Realtime WebRTC path through the lazy `@opengeni/react/transcription/openai-realtime` adapter. Partial text remains browser-ephemeral; only accepted finals enter the ordinary editable composer draft, so this route never writes `session_events` or bypasses existing send/queue/steer semantics. The provider-selection, retention/residency, metering, fallback, and production-acceptance gaps are recorded in [`design/transcription-and-realtime-voice-2026-07-19.md`](design/transcription-and-realtime-voice-2026-07-19.md).
