...
- **Turn capacity uses the Temporal activity queue, never the visible prompt queue.** A prompt behind a paused session is durable human input but is not runnable and never schedules `runAgentTurn`. `DescribeTaskQueue` backlog count/age is therefore the eligible-runnable backlog signal. Dedicated turn workers reserve Temporal activity slots through a cgroup-aware supplier before polling: density remains capped at 16, each reserved turn is budgeted at a hard 100 MiB, and 512 MiB of native/runtime headroom is retained. A finite pod that cannot safely admit one turn fails startup rather than using an OOM as admission control. Canonical: `apps/worker/src/concurrency.ts`, [`design/turn-worker-density-2026-07-16.md`](design/turn-worker-density-2026-07-16.md).
