...
- Recoverable conditions **idle or explicitly resume** the session (keep context) rather than fail it: retryable provider failures, escaped MCP request timeouts (checkpointed without replaying completed tool calls), narrowly identified Responses-stream invalid-content failures (successful final checkpoint → a new paced goal/user turn; failed checkpoint → user revival, never replay), provider context overflow after strict-shrink compaction recovery, the max-turns segment cap, and budget/credit exhaustion. Repeated productive compactions are allowed; no-shrink recovery idles instead of churning. Resolved model metadata controls raw/effective/auto-compact limits end-to-end (Codex subscription: 272,000 / 258,400 / 244,800), and local checkpoint rebuilds retain one cumulative 20,000-token newest-user-message budget. A failed session would reject the very retry it asks for, except where final checkpoint failure deliberately makes user revival the safe boundary.
...
- **The active-sandbox pointer is establishment-safe.** The per-session `(active_sandbox_id, active_epoch)` pointer is *admitted* and *established* through one shared capability predicate (`swapTargetEstablishability`), so a swap can never commit a pointer no production resolver can resume. API and worker construction roots both require the named-Modal establisher before admission treats Modal siblings as supported. A first-class named Modal target owns a lifecycle keyed by its own `sandboxes.id`; that UUID is its lease/envelope identity, an absent lease is its clean cold start, and it never reads or falls back to the calling session's home `sandbox_session_envelopes` row. A swap/seed to an unknown kind is rejected **before** the epoch-fenced CAS with typed `unsupported_backend_context`, leaving the pointer untouched. At turn start a persisted pointer whose target is *structurally* unestablishable (a deleted row, unknown kind, or enrollment-less selfhosted row) is reset to the session **home** under the epoch fence with a visible `session.route.reconciled` event — never a silent downgrade, and a lost CAS honors a concurrent higher-epoch user swap rather than clobbering it. A *null* pointer always means the session **home** backend (the Modal group box for a Modal-home turn, the machine for a machine-home session); the routing proxy caches by the full `(activeEpoch, activeSandboxId)` tuple so a clear-to-null — epoch-bumped or a same-epoch FK `SET NULL` — re-lands the next op on home, never a stale swapped-to backend. A transiently *offline* enrolled machine is left in place (it may recover mid-turn; its ops surface `agent_offline` lazily). Canonical: `packages/runtime/src/sandbox/routing/backend-resolver.ts`, `packages/core/src/sandbox/modal-lifecycle.ts`, `packages/core/src/sandbox/fleet.ts` (`resolveTarget`), `apps/worker/src/activities/agent-turn.ts` (`reconcileActiveSandboxPointer`).
- **Provider-gone recovery is an elected lifecycle transition, never a generic leaf recreate.** On warm/rearmed/attached resume, structural provider NotFound returns a typed `gone` outcome. The exact `(lease_epoch, instance_id)` elects one `warm → warming` reclaim owner; only that owner may create, and it records the replacement id before readiness/setup. Concurrent turn/viewer/Channel-A callers wait and re-read instead of creating rivals. The `warming → warm` commit bumps `lease_epoch` and, when this is the same logical target being rematerialized, invalidates routing caches by advancing the relevant sessions' `active_epoch` in the same transaction. Transient DNS/gRPC `UNAVAILABLE` is not NotFound. A false epoch heartbeat aborts the stale turn and resumes from durable conversation truth rather than continuing on its old handle. Canonical: `packages/core/src/sandbox/modal-lifecycle.ts`, `packages/db/src/index.ts` (`claimGoneLeaseReclaim`, `commitWarmingToWarm`), `packages/runtime/src/sandbox/provider-errors.ts`.
- **Mutating routing failures are not whole-operation retry tickets.** Exec, stdin, file/editor writes, materialization, and desktop input are invoked once when acceptance is unknown. A transport/fence failure without typed pre-acceptance proof surfaces `SandboxMutationAcceptanceUnknownError` with any preserved provider execution id; only read-only operations or a mutation proven never sent/rejected before acceptance may re-resolve and retry. Canonical: `packages/runtime/src/sandbox/routing/routing-session.ts`.
