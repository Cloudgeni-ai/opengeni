...
- **Ungraceful death (heartbeat timeout):** surfaces as a typed `ActivityFailure` carrying the exact Temporal activity id. `recoverSessionDispatch` recovers the same turn from durable conversation truth only when no terminal no-replay fence exists, bounded by the per-turn death ceiling; exceeding it atomically fails the exact turn/session. An exact accepted/acceptance-unknown `failed_idle` checkpoint is instead terminally consumed without a replacement attempt.
