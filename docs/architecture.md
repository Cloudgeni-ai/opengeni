...
- **A session API** — create/list/get/rename sessions, Send/Steer human prompts, Pause/Resume recursive workstreams, approve tools, and read durable history.
...
- Recovery is _explicit_: graceful shutdown checkpoints the same turn as `recovering`; ungraceful death is detected via typed heartbeat/schedule-to-start timeout failures and re-dispatched through `recoverTurnAfterWorkerDeath`, **bounded by a per-turn redispatch ceiling of 3** before the session fails for real.
...
| Store | Job | Rule |
| --- | --- | --- |
| `session_history_items` | **Conversation truth** fed to the model (default read path). | Unredacted, replay-ready, dual-written as the agent streams. Private memory tool calls/results remain here for model continuity and are protected by session access. Compaction **supersedes, never deletes** (`active=false`); summary inserted at fractional position `boundary - 0.5`. |
...
| `session_events` | **Redacted human/audit timeline** — drives replay/SSE/UI. | Secret-redacted and lossy. Private `memory_search`/`memory_save`/`memory_correct` arguments and correlated outputs are projected to ids/names plus `redacted: true`; memory lifecycle events retain ids/kind/action only. **Must never be fed back to the model.** Per-session monotonic `sequence`. |
...
Workspace knowledge is a separate store again: `knowledge_memories` holds curated and agent-written records with one typed scope (`workspace`, `user`, `role`, `session`, `ephemeral`, or fail-closed `legacy`) plus bounded relevance labels. Retrieval derives user/role/session/task context only from the trusted persisted session creator and normalized session metadata, filters applicability before ranking, and returns explainable text/scope/label/freshness/confidence/provenance/conflict components. Labels and roles are relevance inputs, never authorization. This composes continuity from existing users, workspaces, roles, and sessions; there is no canonical durable Agent/profile.
...
REST and the deprecated documents-MCP adapter resolve a signed session claim once
against the requested workspace. The effective private-memory subject is the
persisted `sessions.created_by_subject_id`, never the worker transport subject;
only that validated session id may be stored as creator/actor provenance.
`knowledge_memories(workspace_id, created_by_session_id)` references
`sessions(workspace_id, id)`, so provenance cannot cross a tenant boundary.

The agent/product lane is **active by default** through one deterministic write gate (`saveWorkspaceMemory`): sanitize/redact, exact/near dedup, active-record cap, optional supersession, then persist. The older `proposed → approved/rejected` lifecycle remains for the curated docs-MCP lane (`memory_propose` and human review); direct reviewed-status creation is rejected and approved/rejected transitions lock the row and require its current status to be `proposed`. Agent-visible memory is `active ∪ approved`. Corrections atomically retire records as `superseded` or `archived`; `knowledge_memory_relationships` records supersession, derivation, contradiction, relation, applicability, and dependency provenance. Explicit hard deletion and private export write text-free audit rows. Retention/reconciliation use audited preview → exact hash/row-version-fenced apply → preconditioned revert operations rather than opaque background rewriting.

Per-workspace `settings.memoryEnabled` gates only agent surfaces: working-set injection and first-party `memory_search`/`memory_save`/`memory_correct`. Human REST/UI audit and seed paths stay available regardless of the setting. When enabled, `resolveWorkspaceMemoryBlock` considers at most 50 applicable candidates and injects only whole records within an estimated 2,500-token block before per-session instructions; episodic records remain search-only, and labeled workspace records require a matching persisted task label for standing context. Search is hybrid semantic + keyword with keyword fallback. Public writes never accept subject/session/role actor selectors: adapters bind them from signed grants and persisted session context.
...
- **All workspace-scoped tables use forced row-level security** (RLS). The app connects as the non-superuser `opengeni_app` role and **must** run reads/writes inside an RLS context wrapper (`withWorkspaceRls`/`withAccountRls`) or rows are invisible. User-scoped memory adds a subject GUC and fails closed when no trusted subject exists; the five memory/relationship/operation/audit tables all use FORCE RLS, and private-admin export is a separately permission-checked, audited context.
...
- **Never terminate a box whose `/workspace` snapshot could not be captured** — a `persistWorkspace` failure re-throws _before_ any terminate.
...
5. **Execute.** `runAgentTurn` registers a first-class exact attempt, builds + runs the OpenAI Agents SDK stream inside a sandbox, publishes realtime events, writes attempt-fenced conversation truth to `session_history_items`, meters/bills usage per model call, and atomically settles the turn (`idle` / `requires_action` / `recovering` / `failed` / `cancelled`).
...
- **Turn**: one logical unit of agent work, triggered by a waiting human/API prompt, an approval decision, or one coalesced internal-update batch (goals, schedules, child results, capacity recovery, and similar machine updates). Each execution attempt is one **non-retryable activity**. Inside it, the SDK loop makes as many model/tool calls as the work needs.
...
- **Interrupted replacement admission is receipt-gated, not activity-gated.** After logical settlement, `peekSessionWork` reports `cancellation-wait` until the exact predecessor's durable quiescence receipt exists. The workflow waits at most five seconds for a wake, then may close cleanly; a proof accepted at that boundary is persisted first. The activity's exact proof signal can restart the stable `session-<sessionId>` workflow after direct DB exhaustion, and the committed receipt's same-transaction wake outbox supplies the ordinary coalesced restart. No quiescence scanner, inferred timeout, polling loop, synthetic user prompt, prompt/history/effect replay, replacement row, or duplicate visible work is created. Claiming remains blocked even if the original Steer row is withdrawn.
...
`github`: gate every privileged call on full configuration; HMAC CSRF state is bounded to 3600s and the installation/repository chooser tickets to 600s. `GET .../github/connect` has two explicit intents: install on GitHub or authorize/link an existing installation. The OAuth discovery reads `/user/installations` and each installation's user-visible repositories, but only repositories whose returned permission bits include `admin` can enter a workspace allowlist. The OAuth user token is ephemeral. The final workspace bind still requires `github:manage`; configured-token deployments carry that authority across the headerless browser redirect in a signed state capability bounded to 600s, while managed/local modes keep using their ordinary access resolver. Unlink deletes only that workspace's row. Repository listing, session admission, MCP token minting, and every GitHub installation-token mint in the worker enforce the current binding so scope changes take effect before another token is issued; Connected Machines are exempt because the platform never mints them a token. Generated manifests must include `/v1/github/oauth/callback` in `callback_urls`. **Footgun:** if `OPENGENI_GITHUB_APP_MANIFEST_STATE_SECRET` is unset, the secret is random per process — set it explicitly for multi-instance deploys. GitHub Enterprise hosts are hardcoded (not supported). `observability`: prom-client sits behind the `Observability` facade; metrics remain per-process (reset on restart), so API `/metrics` reflects the API process and worker `/metrics` is served by the worker HTTP listener (`OPENGENI_WORKER_HTTP_PORT`, default 8001).
...
- **Connection credentials.** `ConnectionCredentialsPort` can provide per-run GitHub, GitLab, and Azure DevOps git provider tokens plus sandbox secrets. workspace-scope cross-check requires each provider result to echo `workspaceId`, and the worker asserts the echo before injection. GitHub requests remain legacy-shaped when possible (`installationId` + `repositoryIds`), while other providers use `provider` + `repositoryRefs`. The first-party MCP `ogd_` delegated token is still self-minted from `settings.delegationSecret`, not supplied by this port.
...
- **Event/output representations are deliberately distinct.** They must not be described as interchangeable views of one retained blob:
  1. `session_history_items` is model conversation truth. Tool output is normalized/truncated at the canonical model-history boundary before it can make a later provider request unbounded; this store is never reconstructed from audit events.
  2. `session_events` is a redacted, append-only human/audit **preview** with a 64 KiB payload envelope. The application preserves useful deterministic head/tail/identity facts; a rolling-safe database trigger supplies a content-free fallback for old writers, and the follow-up rolling backfill rewrites only pre-guard violations through that trigger before validating the storage constraints. Inline media becomes a `media_preview`; its source bytes are not materialized as base64 in the event and are not retained by this generic path.
  3. NATS/SSE/REST/MCP are separate bounded delivery projections over durable audit previews. NATS chunks exact encoded messages and each SSE frame is bounded; neither inherits an MCP monitoring payload. REST/SDK no-cursor monitoring performs indexed tail selection, excludes raw message/reasoning/command/PTY deltas by default, supports shared type/semantic-class filters plus `none`/`summary`/`full` payload modes, and returns byte-bounded pages with exact covered ranges and cursors. Explicit forensic `full` is exact only for the retained durable audit projection. The manager-facing MCP result has a second 64 KiB exact pretty-JSON cap, removes rows only from the omitted pagination edge, and never manufactures an event or advances past an unreturned row. Historical oversized rows are normalized defensively at each outbound legacy guard.
  4. The browser keeps one contiguous, direction-aware window of at most 10,000 events and 8 MiB serialized bytes (plus a per-event legacy guard). Live/default accumulation retains the newest suffix. Backward paging may retain an older prefix; when that evicts the live tail, the hook aborts the current iterator and reconnects from the retained high-water mark so the missing tail is replayed before new live rows. The highest-ever-observed sequence and latest session-status projection remain separate from that rewindable resume cursor; `hasOlder`/`windowTruncated` make local eviction visible.
...
- **Three memory stores + envelope.** See §3.5: `session_history_items` (model truth), `agent_run_states` (approval resume), `session_events` (audit), and `sandbox_session_envelopes` (sandbox recovery). Typed workspace knowledge memory (`knowledge_memories`, agent-written `active` plus curated `approved`) is separate again — see §3.5.
...
- **GitHub installation bindings.** A GitHub App installation is not globally owned by one workspace. Each workspace has an independent binding and selected-repository allowlist; user OAuth plus repository-level `admin` proves authority at bind time, and every token-mint/run boundary rechecks the current binding. Workspace unlink never uninstalls the App or deletes another workspace's binding.
...
- **CI.** `ci.yml` (typecheck + unit + guards + package/image builds + deployment-artifact validation; asserts no floating `latest` tags), `release.yml` (push-triggered changesets Version PR plus a separate evidence-bound manual publication/GHCR release, dormant until `RELEASE_ENABLED=true`), `agent-ci.yml`/`agent-release.yml` (Rust agent, path-filtered, independently versioned via `agent-v*` tags).
