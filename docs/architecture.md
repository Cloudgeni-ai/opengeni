...
- Claim and every lifecycle settlement use one lock order (session → turn). Start, requires-action, ordinary terminal, preemption, and worker-death events commit in the same transaction as turn/session state, active pointer, and sequence. Missing/terminal targets, superseded attempts, and durable interrupts newer than the current attempt are event-free stale no-ops, so a cancelled zombie cannot publish contradictory terminal/preemption truth or fail the session.
