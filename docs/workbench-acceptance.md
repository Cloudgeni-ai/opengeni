...
| Steer/Pause cancellation | A committed control is never lost: the old attempt is durably fenced, every unresolved attempt-owned side-effecting tool call is closed as `interrupted / outcome unknown`, and physical activity shutdown completes before the replacement starts. Exercise model streaming; credential/environment loading; context compaction; MCP connection; an INT/TERM-resistant terminal process; repository clone, rig setup, file materialization, token seeding, and in-flight credential refresh; turn-end capture; warm snapshot; eager and lazy sandbox provisioning; recording teardown; and MCP teardown. Test cloud PTY, local PTY, and connected-machine OpCancel paths. Separately prove that a durably adopted session background command survives normal completion and Steer, remains visible as running while the turn is idle, and is moved to stopping only by Pause/Cancel or explicit command cancellation. A Steer waiting on the attempt fence renders `Stopping previous attempt…` with its durable queued prompt; it never looks like ordinary capacity queueing. No late attempt-owned capture, event, output, file mutation, lease, or provider result from the predecessor becomes authoritative. |
...
| Steer/Pause physical cancellation | worst ≤ 4,000 ms from committed control to replacement `turn.started` (Steer) or physically stopped attempt-owned activity (Pause), with zero zombie output; Steer deliberately excludes durably adopted session background commands |
...
replacement for merge provenance or acceptance.

1. Merge reviewed source to `main`.
2. Wait for trusted Version-PR CI to finish against one unchanged head, then
   merge the generated Version PR so the release source has exact package
   versions and no pending changesets. GitHub branch protection may still
   require a review to merge; ordinary candidate/operator admission binds the
   merged associated PR, not a later GitHub `APPROVE` or structured PASS body
   on that head.
