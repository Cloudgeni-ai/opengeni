...
Catalog discovery describes two connectors: **Slack (personal account)** reads
and sends as the connected user; **OpenGeni Slack bot** reads accessible channels
and sends as the installed bot. Each description states its identity, operations,
and scheduling authorization requirements. Agents choose from these descriptions
and the requested outcome; general prompts and product-help skills contain no
Slack-specific selection rules.

Installation metadata, selected tools, and executable authorization remain separate.
Discovery does not grant access, and personal authority cannot transfer to another
workspace member's message.
...
5. **A sole candidate**, because one workspace is not a choice. This is what keeps an ordinary installation quiet.

   Counting candidates in a channel deliberately ignores personal workspaces. A channel thread routed into one member's private space would be invisible to everyone else in that channel, and since every member of a managed organization is provisioned a personal workspace, counting them would mean nobody ever had exactly one candidate - an organization with a single shared workspace would be asked to choose in every channel despite having no choice to make. A bot DM is the opposite case and still resolves the sender's own personal workspace at step 4. One consequence follows: a person whose only workspace is their own is refused in a channel rather than quietly working somewhere the channel cannot see.
...
| | no candidate workspace at all, and nothing else to route on - no mapped thread, no prefix, no channel route, and no DM route. In a channel this also covers someone whose only workspace is their own personal one, since a channel does not count that as a candidate | the generic "ask an OpenGeni administrator" DM. No link, same reason |
...
Slack requires action IDs to be unique within each actions block. At the provider boundary, OpenGeni splits blocks containing repeated action IDs into deterministic individual blocks; this covers workspace choices and single-select human questions. Stored action handles and logical post/update digests stay unchanged, so an old picker rejected with `invalid_blocks` can retry the original post operation when the person invokes the bot again. Completed and ambiguous provider receipts retain their normal replay/reconciliation fences.

### Personal account access in a web chat

The workspace bot and a person's Slack OAuth connection are separate authorities. Connecting or reconnecting the personal account saves its credentials; selecting the Slack integration chooses its tools. The owning human must also authorize that personal account for the exact chat. The composer shows **Review personal account access** when a selected personal integration has no valid session grant, even if the agent has not requested a connection card. That control opens the existing **Use in this conversation** flow; shared chats require acknowledgement that results are visible to workspace members. After consent, the next message includes the exact grant. Revoked, expired, wrong-session, or stale-epoch grants do not attach. Reconnecting alone cannot create that consent.

### Saying where the work went

Every acknowledgement and delivery for a routed task carries a quiet `-> Workspace` line, so work never lands somewhere surprising. It appears only when routing made a genuine choice - a prefix, a channel route, a DM route, or a personal workspace. A sole candidate, an installation whose people have one workspace, and a reaction that fell back to the installation all made no choice, and a constant footer would be noise. A mapped thread inherits the label frozen on the interaction it reuses. For new interactions, the workspace name links to that workspace when the configured web URL fits the existing 128-byte snapshot limit. The complete Slack-formatted destination is frozen when the interaction binds, after authorization, because a rename or base-URL change between the original post and reconciliation would make the post ledger's byte comparison raise. Existing interactions keep their exact original plain-text label.
...
The Plugins Slack panel keeps the team name and connection status visible. **Where work starts** explains channel and DM routing when deployment routing is enabled and opens the existing channel picker. **More options** contains channel access, workspace settings, reconnect/disconnect, and expandable technical connection details. Disconnect confirmation names its effect on the whole installation, including channels routed to other workspaces; historical tasks remain in place.

When the current workspace has no bot connection or installation binding, the web client checks installation metadata in other same-organization workspaces for which the viewer has `connections:read`. Each read uses that workspace's normal protected API; an active binding is shown as connected only when the matching connection's verified identity and version also agree. The result links to the original workspace's Slack settings rather than copying credentials or offering another setup. Discovery does not expose inaccessible workspaces, grant installation management, or create organization-wide bot-tool authority. A failed discovery offers Retry instead of claiming no installation exists. Full organization-level visibility without home-workspace access remains separate work.
