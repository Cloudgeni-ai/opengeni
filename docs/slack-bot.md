...
Slack AI huddle notes are canvases. Sharing that canvas into a channel where the bot is a member lets OpenGeni read the notes and exposes the associated `huddleTranscriptFileId`. OpenGeni first reads any `huddle_transcription` object Slack includes in the transcript or parent-canvas `files.info` response. Slack can still return only transcript metadata to a bot while redirecting the content download to an interactive user page. OpenGeni reports `huddle_transcript_requires_participant_access` in that case and never treats the Slack web-client HTML as transcript content.
