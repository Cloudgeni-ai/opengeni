...
|---|---|---|
| **a Slack message** (mention, command, DM, shortcut, thread reply) | a route routing actually chose - `prefix`, `channel`, `dm_route`, `dm_personal`, `sole_candidate` - whose `resolveSlackTargetAuthority` refused | a bot DM plus the Slack access-request link, minted for the workspace they actually need |
...
| | no candidate workspace at all, and no prefix to answer | the generic "ask an OpenGeni administrator" DM. No link, same reason |
| | a mapped `thread` route they have lost, or the installation's own workspace - which is every route while the flag is off | nothing. The pre-routing permanent `identity_access_revoked` failure |
