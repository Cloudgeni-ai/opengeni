...
> **Historical dossier boundary.** The early owner/queue sketches retained later
> in this file are provenance, not implementation authority; do not translate
> or revive them. Current recording finalize is
> `stopRecording` → sandbox-side stat/size gate → sandbox-side curl PUT to one
> short-lived scoped storage URL → `updateRecording(available)` → artifact
> cleanup. Recording bytes enter neither worker memory nor a Temporal payload.
> The current memory and verification contract is
> `docs/design/turn-worker-density-2026-07-16.md`; source under
> `packages/runtime/src/sandbox/recording.ts` is authoritative.
...
### F10 — Resolved: recording bytes bypass Temporal and worker memory

The live sandbox stats and size-gates the finalized artifact, then curl PUTs it
directly to a short-lived, write-only object-storage URL. The turn activity
receives only content type, size, and duration metadata. Artifact deletion still
occurs only after the storage PUT and `available` row commit.
