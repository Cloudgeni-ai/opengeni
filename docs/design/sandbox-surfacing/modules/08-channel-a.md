...
| `packages/db/src/index.ts` | Add `insertPtySession`/`closePtySession`/`listOpenPtySessions`/`reapIdlePtySessions` queries (mirror `claimNextSessionExecution`'s `withWorkspaceRls`+txn pattern). |
...
- Existing per-session envelope/lease precedent: `packages/db/src/schema.ts:360` (`sandboxSessionEnvelopes`), `claimNextSessionExecution` FOR-UPDATE txn pattern in `packages/db/src/index.ts`.
