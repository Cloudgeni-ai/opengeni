...
RLS: both tables get the workspace RLS policy every workspace-scoped table has (mirror `sandbox_session_envelopes`); all lease ops go through `withWorkspaceRls`/`withRlsContext` like `claimNextSessionExecution` (`db/index.ts:3078`).
...
PHASE 1 — Lease + stateless resume-by-id + ownership inversion (the load-bearing foundation; NO user-visible change yet)
  [migration 0017 sandbox_leases/_holders] → [db lease ops: acquireLease/releaseLease/reap, the
   FOR UPDATE + cold→warming CAS + epoch fence, modeled on claimNextSessionExecution:3077]
...
- **GitHub Releases REMAINS the public archive behind the edge routes and immutable update URLs.** The `agent-v<semver>` tags from `.github/workflows/agent-release.yml` are the durable archive. `install.sh` itself is served only at `<base>/install.sh`, while its asset base must implement `/agent/latest/*` and `/agent/v<ver>/*`; a raw GitHub release-download directory is not route-compatible with `OPENGENI_INSTALL_BASE_URL`.
