...
- Handshake is **API-direct**, NOT a Temporal signal: the API resumes the box by id in-process via the shared `@opengeni/runtime/sandbox` module (mirrors the Channel-A A2 seam, `modules/08-channel-a.md` §5.1), the same shape as the API's existing outbound HTTPS (Stripe/OpenAI/GitHub) — `apps/api/src/dependencies.ts`. `ModalSandboxClient.resume()` is per-call/no-pool; `packages/runtime` has no `@temporalio` dep. (The existing `signalSessionControl`/`signalWithStart` pattern is for the agent turn, not this handshake.)
