...
|---|---|---|---|
| **(a) cold start** | a turn dispatches, or the first sandbox op when `OPENGENI_SANDBOX_LAZY_PROVISION=true` with ownership enabled | eager path: turn activity (any worker, existing dispatch) → CAS `cold→warming` → `createSandboxClient` (6080 merged into exposed ports) → `ensureDisplayStack` → CAS `→warm` + `epoch++`; lazy path: the turn computes the stable manifest env at start, injects a routing proxy seeded with a synthetic `unprovisioned` manifest, and the first default-sandbox op runs the same establish/setup sequence through a single-flight provisioner. Chat-only lazy turns never create a lease row or box. | exactly-one spawner under the held row lock; lazy first-op calls are single-flight |
