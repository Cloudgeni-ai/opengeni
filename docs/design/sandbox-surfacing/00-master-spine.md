...
│   viewer op). Optional desktop setup is lazy: │  │   CAS as a Postgres txn it owns, and       │
│   first computer action OR viewer attach.     │  │   returns inline JSON. client → API → box. │
└──────▲────────────────────────────────────────┘  └──────────────▲────────────────────────────┘
...
|---|---|---|---|
| **(a) cold start** | a turn dispatches, or the first sandbox op when `OPENGENI_SANDBOX_LAZY_PROVISION=true` with ownership enabled | eager path: turn activity (any worker, existing dispatch) → CAS `cold→warming` → `createSandboxClient` → CAS `→warm` + `epoch++`; lazy path: the turn computes the stable manifest env at start, injects a routing proxy seeded with a synthetic `unprovisioned` manifest, and the first default-sandbox op runs the same establish/setup sequence through a single-flight provisioner. Neither path starts the desktop or recording. Chat-only lazy turns never create a lease row or box. | exactly-one spawner under the held row lock; lazy first-op calls are single-flight |
| **(b) viewer connects to idle box** | browser POST `/stream-capabilities` | **client → API → box, all in-process**: API acknowledges → opens a Postgres txn → acquires the viewer holder → re-arms `draining→warm` (or runs the `cold→warming` CAS + resumes/spawns the box by id itself) → lazily ensures the display stack → `resolveExposedPort(6080)` mints the tunnel URL, records it under the epoch fence → returns it inline → browser noVNC **direct**. No worker, no Temporal, no NATS req/reply. | viewer holder is TTL-reapable; warm-meter starts; API owns the CAS txn |
| **(c) first computer action / viewer mid-turn** | the model invokes a computer tool, or a browser POST arrives during a live turn | The first actual computer action lazily and idempotently ensures the display stack, then optionally starts proof recording before the action. A viewer uses the API-direct path to resume the same box, ensure the same `:0`, acquire a viewer holder, and return its tunnel URL with no epoch bump. Ordinary shell/filesystem turns never start Xvfb, XFCE, ffmpeg, or a stream tunnel. | agent and viewer share one `:0`; tool registration is side-effect free; lock-free reattach |
| **(d) Modal 24h rollover** | lifetime-ceiling timer | whoever next touches the dead box (a turn on a worker, OR the API on a viewer op) re-establishes: snapshot → new box from snapshot → `epoch++`. Desktop state is re-created only when the next computer action or viewer actually needs it; a viewer re-mints its URL and the client hot-swaps the socket. | the ONE recovery primitive; epoch fences the dead URL |
...
**Acquire critical section (the precedent + the deviation):** the existing `claimNextSessionExecution` (`packages/db/src/index.ts:3083`) uses `for update skip locked` — that is correct for a *work queue* (skip a busy row, grab the next). The lease is the **opposite**: it uses plain **`for update`** (block, do not skip) so concurrent acquirers serialize on the one session row and exactly one wins the `cold→warming` CAS. Both the lease and owner specs note this; it is the single most important query-level distinction in the system.
