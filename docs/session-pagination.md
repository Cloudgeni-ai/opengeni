...
Archived root list responses also preserve the exact PostgreSQL microseconds in `archivedAt`, selected in the same statement as the page boundary. Clients must not reduce that value to millisecond precision when combining archive pages.
