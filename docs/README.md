...
| Durable learning confirmed-write router | `docs/durable-learning-router.md` | Router callers, authority adapters, future evaluator/controller work, and agent-facing integration must preserve exact-attempt admission, immutable input-hashed receipts, and destination lifecycle ownership. |
| Workspace learning policy | `docs/workspace-learning-policy.md` | A future evaluator/controller, source settings, and Agent Brain UI should consume this version/effective-mode contract before invoking the canonical durable-learning router instead of creating source-specific policy paths. |
