...
over every discovered workspace `tsconfig.json` plus the typed CI scripts. It uses a
...
this repo). `bun run lint` runs `oxlint --deny-warnings .`, so CI requires zero errors and zero
warnings. Config is `.oxlintrc.json` at the repo root: a lean plugin set (`react`, `react-hooks`,
`typescript`, `import`) to stay clear of the multi-plugin perf cliff. Notable rule choices:
...
Warning-level rules still distinguish severity in editor output, but `--deny-warnings` makes every
reported warning blocking in local and CI lint gates.
...
The current 25-case real Temporal workflow suite remains one authoritative
`temporal-workflow.integration.ts` entry point because its evolved session-control
semantics share one current workflow fixture. It owns one service lifecycle and
one immutable workflow bundle. The hosted-runner planner isolates that measured
file in a deterministic real-service shard while distributing the other current
integration files across a hard six-shard ceiling; obsolete pre-cutover Temporal
splits are not accepted as coverage.
