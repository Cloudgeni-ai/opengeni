...
`authority_scope = 'user'`, which the lifecycle functions always populate. Do
not backfill it as if it were a classification, and never resurrect a NULL
origin on a user-scoped row - the `ON DELETE SET NULL` foreign key erased that
origin deliberately.
