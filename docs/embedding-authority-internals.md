...
Migration 0448 freezes the credential-free external continuation privately on
...
Migration 0437 adds the credential-free `host_mcp_bindings` registry with
...
Migration 0438 adds owner-scoped `host_mcp_delegations` alongside that registry.
...
Migration 0439 adds `host_mcp_turn_authorities`, an owner-scoped FORCE-RLS
...
revisions (0441), carries them through reusable-session promotion and rollback,
...
materializer roles.
Child initialization (0442) copies only selected `always` grants from the exact
...
Optional native links (0443–0446) retain distinct external/native identities.
...
ownership (0447) resolves the effective member's own revision, not the external
...
`inheritCausalHostMcpTurnAuthorities` and the 0440 insert guard prove the
