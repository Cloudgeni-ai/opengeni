...
revisions (0433), carries them through reusable-session promotion and rollback,
...
Child initialization (0434) copies only selected `always` grants from the exact
...
Optional native links (0435–0438) retain distinct external/native identities.
...
ownership (0439) resolves the effective member's own revision, not the external
...
`inheritCausalHostMcpTurnAuthorities` and the 0432 insert guard prove the
