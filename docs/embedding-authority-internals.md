...
Migration 0443 freezes the credential-free external continuation privately on
...
Migration 0432 adds the credential-free `host_mcp_bindings` registry with
...
Migration 0433 adds owner-scoped `host_mcp_delegations` alongside that registry.
...
Migration 0434 adds `host_mcp_turn_authorities`, an owner-scoped FORCE-RLS
...
revisions (0436), carries them through reusable-session promotion and rollback,
...
materializer roles.
Child initialization (0437) copies only selected `always` grants from the exact
...
Optional native links (0438–0441) retain distinct external/native identities.
...
ownership (0442) resolves the effective member's own revision, not the external
...
`inheritCausalHostMcpTurnAuthorities` and the 0435 insert guard prove the
