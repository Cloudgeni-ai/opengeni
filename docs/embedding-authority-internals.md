...
Migration 0442 freezes the credential-free external continuation privately on
...
Migration 0431 adds the credential-free `host_mcp_bindings` registry with
...
Migration 0432 adds owner-scoped `host_mcp_delegations` alongside that registry.
...
Migration 0433 adds `host_mcp_turn_authorities`, an owner-scoped FORCE-RLS
...
revisions (0435), carries them through reusable-session promotion and rollback,
...
Optional native links (0437–0440) retain distinct external/native identities.
...
ownership (0441) resolves the effective member's own revision, not the external
...
`inheritCausalHostMcpTurnAuthorities` and the 0434 insert guard prove the
