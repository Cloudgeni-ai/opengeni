...
The built-in `opengeni-lens` Pack installs the ordinary `pr-review` Skill, then
...
resource authorities. See [`opengeni-lens.md`](opengeni-lens.md).
...
Lens declares an inline Skill and therefore uses the reviewed Pack
...
- Lens event automation: `apps/api/src/routes/lens.ts`, `packages/db/src/lens.ts`, `apps/worker/src/lens-credentials.ts`
