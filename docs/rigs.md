...
4. **Verification runs in an exactly leased, throwaway, secret-free sandbox.** Rig CI uses the change/version UUID as a canonical `sandbox_leases.sandbox_group_id`, accepts only the `cold -> warming` spawner role, records the exact provider instance before setup, and commits it warm before running the rig-setup hook, proposed command, or checks. Provider tags are diagnostic only. After successful checks, a backend may snapshot that clean filesystem once as a version-bound immutable provider image; repositories, resources, credentials, variable-set values, archives, session state, and retained processes are never injected into that build box. Cleanup quiesces commands before termination and never erases an unconfirmed provider pointer; failed direct cleanup is handed to the ordinary lease reaper.
...
6. After checks pass, compute the exact backend/source/definition content identity and ask the provider registry to build one immutable image from the same clean verifier sandbox. Direct version verification claims/finalizes the backend slot under a row lock and deterministic provider request UUID; retries converge on the same provider operation, finalized ready records are never overwritten, and stale-content conflicts fail closed. Change verification carries only a finalized hash-matching record into the newly promoted version. Provider-image failure or unsupported capability is recorded truthfully but does not turn a correctly verified rig definition into a failed verification—the runtime setup fallback remains authoritative.
