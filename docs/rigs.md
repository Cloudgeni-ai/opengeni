...
6. After checks pass, compute the exact backend/source/definition content identity and ask the provider registry to build one immutable image from the same clean verifier sandbox. Register its provider object immediately, then cold-boot that exact image in a second independently owned sandbox and re-run the exact content marker plus every declared check. Only then finalize it ready with `coldBootValidation`. Direct version verification claims/finalizes the backend slot under a row lock and deterministic provider request UUID; retries converge on the same byte-identical provider operation, older unproven records rebuild under the current protocol id, and stale-content conflicts fail closed. Change verification carries only a finalized hash-matching record into the newly promoted version. Provider-image failure or unsupported capability is recorded truthfully but does not turn a correctly verified rig definition into a failed verification—the runtime setup fallback remains authoritative.
