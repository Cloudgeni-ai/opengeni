...
After that logical image precedence resolves, a genuinely fresh Modal creation may select the version's `providerImages.modal.imageId` only when its status is `ready` and its backend, source image, setup hash, and full definition/content hash still match the frozen version. Unchanged versions therefore reuse one immutable provider image. Missing, non-ready, unsupported, malformed, or content-mismatched records leave the logical image untouched and retain the ordinary runtime-setup path. The provider image ID does not replace the logical image reference on the lease, so publishing this optimization never rotates an already-warm box and never changes archive/wake semantics.

`RigVersion.providerImages` is observable build truth keyed by backend: `building | ready | failed | unsupported`, immutable image ID/digest, source/setup/content hashes, deterministic provider request UUID, hashed provider binding, verification provenance, timestamps, and bounded failure detail. Modal currently implements the build operation through its immutable filesystem snapshot API; every other backend truthfully records or behaves as unsupported until it implements the optional provider-registry method.
