...
settlement events remain awaited. Both paths use one activity-owned ordering lane:
one active plus the oldest pending live-only batch, closed-label
`succeeded|failed|timed_out|dropped` accounting, and a bounded close on activity
success, failure, cancellation, or graceful worker shutdown. Close drops pending
live-only work and releases late settlement through attached promise handlers, so
nothing leaks into worker reuse. The lane is a live-delivery optimization, not a
second source of truth: reconnect and sequence gap-fill use the committed event
rows.
