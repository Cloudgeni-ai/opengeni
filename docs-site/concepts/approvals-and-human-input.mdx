...
The `@opengeni/react/chat` component restores unresolved approvals and questions when you reload a conversation through `createChatHandler`. Its question form supports multiple selections, selection limits, and Other answers. Completed or expired requests are not shown as actionable after reload.
