...
Personal integrations such as Gmail can be used in your Personal workspace or
in a shared workspace. Connect your own account, then authorize its use for the
conversation. In a shared conversation, review the acknowledgement that results
may be visible to workspace members. Connecting an account alone does not grant
a session permission to use it. This consent is separate from whether your
organization enables private conversations in shared workspaces.
...
Published images are retained in workspace storage and can display directly in
chat. Opening them does not require starting the compute sandbox. A file merely
...
### Slack identities in scheduled tasks

Choose **OpenGeni bot** when a scheduled task should post as OpenGeni. Choose
**My Slack account** when it should read your personal messages or send as you.
A task can use both when needed. Connecting the bot does not connect your account.

In the schedule editor, select the connected bot or authorize your personal
account. A schedule that creates new chats needs ongoing personal authorization;
a schedule in an existing chat can use authorization for that chat. The schedule
retains the account you authorized and checks that access is still valid when it
runs. Another member sending a message does not gain access to your account.

Results follow the destination chat's sharing settings. Use your Personal
workspace for private work. Removing personal authorization stops future use of
that account; the task does not switch to another member's account.
