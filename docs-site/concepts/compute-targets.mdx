...
A managed sandbox is a fresh box OpenGeni provisions for the session. Backends are pluggable: Docker (the local default), Modal, an in-process local provider, and several cloud sandbox providers. Both stock images include Terraform, Checkov, AnyDoc, GitHub CLI, git, and common shell tools, plus Python with uv, requests, pandas, numpy, matplotlib, and pytest preinstalled. `pip install` and `uv pip install --system` work without creating a virtual environment. The desktop image also includes the graphical desktop, Chrome/Chromium, and FFmpeg; it does not install Azure CLI or its Azure DevOps extension. The headless image still includes Azure CLI.

Opt-in preparation profiles authenticate existing tools; they do not select or install an alternative image. The `azure` preparation profile requires an installed Azure CLI and must not be enabled against the stock desktop image. Azure-backed deployment, storage, and model-provider support are independent of these in-sandbox CLI packages.
...
Modal commands use provider-owned execution IDs, with their acknowledged output cursors retained in OpenGeni's database. Another worker can read a yielded command through its original handle without relying on a previous SDK client's in-memory process map. Output is saved before cursors advance, and exit status comes from the provider, never a file writable by the command. Recovering workspace files into a new sandbox does not resume an old process. Historical handles without a retained provider execution ID remain unknown rather than being assigned an invented exit status. This compatibility adapter is pinned to the verified Modal SDK control-plane contract; SDK upgrades require its regression and live-provider checks.
