...
Modal commands keep their output and exit status in a journal inside the running sandbox, so another worker can read a yielded command through its original handle. Custom Modal images must include `python3`; the stock images already do. The journal is not a process checkpoint: recovering workspace files into a new sandbox does not resume an old process, and an unavailable journal is not evidence that a command finished successfully.
