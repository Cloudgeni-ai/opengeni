...
<Note>
  Check your installed package exports, types, and README for exact signatures and version
  compatibility. The documentation may describe features newer than your deployment.
</Note>
...
Use a compatible SDK release and server deployment for these examples. Follow the [package and deployment setup](/guides/integrate-your-product#connect-your-backend) before using the backend chat handler. Repository access is optional.
...
In an authenticated product, resolve `tenant` and `user` on your server and complete [external-user onboarding](/reference/authentication#external-user-onboarding). The session is created on first send. Conversation identity is independent of the acting user, whose authority is checked separately; different user IDs do not isolate a shared conversation ID. `agentAccess` defaults to `"session"`, with omitted `memory` defaulting to `false`. For user or workspace agent access, omitted `memory` follows that scope. This compatibility option selects personal or workspace Knowledge; `false` initializes Knowledge authoring to Off and leaves authorized retrieval available. Both settings apply at creation, not when reopening an existing session.
...
For embedded agents, use `create: { bundledSkillIds: [] }` in chat options to
exclude bundled product guidance, or list the individual capability guides the
product supports. Workspace and inline Skills and tool permissions remain
separate. See [Product guidance in embedded agents](/guides/integrate-your-product#product-guidance-in-embedded-agents).

Chat replies and the Vercel/OpenAI formatters currently have no first-class
monetary cost field. The separate permissioned `getBillingUsage` method returns
bounded accounting data, not an AI SDK response extension. See
[Request cost and verification](/guides/integrate-your-product#request-cost-and-verification).
