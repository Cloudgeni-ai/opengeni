...
Shared console links unfurl in Slack, X, and other chat tools with a preview card. Keep `OPENGENI_PUBLIC_BASE_URL` (or `OPENGENI_WEB_BASE_URL` when the console has its own origin) set to the address people use, so the preview image URL is absolute.
