...
- **Three memory stores, three jobs.** `session_history_items` is protocol-preserving, secret-redacted conversation truth fed to the model (default read path); known runtime credential provenance and recognized credential-bearing shapes are redacted before persistence and replay. Private memory tool arguments/results remain here for truthful model continuity and are never copied into the workspace-readable event projection. `agent_run_states` is the serialized RunState blob, used *only* to resume a turn paused for a human approval — never as conversation memory. `session_events` is the independently redacted, lossy human/audit timeline and must never be fed back to the model. Sandbox recovery state is separate again in `sandbox_session_envelopes`. Workspace knowledge memory (`knowledge_memories`) is bounded retrieval context, never conversation memory: agent-visible `active ∪ approved` records compose workspace/user/role/session/ephemeral scopes from trusted initiating-human/session context, labels affect relevance rather than authorization, and maintenance/export/deletion are explicit audited operations. See `docs/run-lifecycle.md` and the memory-design design record.
