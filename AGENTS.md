...
- **Existing-session waits are durable and inference-free.** `ask_user` resumes one saved SDK `RunState`; `wait_until`, `wait_for_event`, and one-shot background-job terminals reuse the closed `scheduled_occurrence` internal-update path. PostgreSQL owns state, Temporal timers/signals/controllers are replaceable wake machinery, and NATS is best-effort fanout. Reminders never run inference; provider work that disappears after its one start fence is `lost`, never rerun. Recurring scheduled-task definitions remain separate. See [`docs/durable-waits-and-background-jobs.md`](docs/durable-waits-and-background-jobs.md).
