...
- **Temporal cancellation is not physical quiescence.** Receipt-gated v2 workflow histories first close and fence the exact attempt in Postgres, then request nonblocking Temporal cancellation. The exact `runAgentTurn` activity owns the hard sandbox/tool fence and writes `quiesced_at` immediately after it drains; that receipt, its queue event/session revision, and the exact workflow wake commit atomically. Provider/batcher/lease/cache/recording cleanup after the fence is detachable and attempt-fenced. Until the receipt exists, every claimer stays blocked even if the Temporal activity completed or failed; the workflow may close after a bounded `cancellation-wait` and is restarted by the wake outbox without a quiescence scanner, synthetic prompt, or duplicate visible work. Queue stopping telemetry is scoped to the latest attempt and may be true with an empty visible queue. Keep heartbeat cancellation delivery throttled to five seconds. Histories without the `session-attempt-quiescence-v2` patch retain the v1 WAIT/fallback command order only for deterministic replay; new histories must never take that path, and it may be removed only after those runs drain.
