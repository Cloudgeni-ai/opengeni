...
- **Realtime is a temporary admission mode of the same ordinary session.** Realtime begin and every ordinary/recovery/maintenance workflow claim serialize on the canonical PostgreSQL session lock. Ordinary work that claims first makes begin reject; an active realtime owner makes later work remain pending until end or lease expiry. The sole claim exception is the exact ordinary turn transactionally linked to an accepted provider delegation call; its terminal settlement atomically projects one durable outbound result/error, replayed until exact provider-delivery ACK. Human composer/queue/Send/Steer commands are fenced only for that session, cross-session agent updates remain durable without waking normal inference, and the workflow must not synthesize goal work while waiting. End/expiry commits the lifecycle event plus a normal-work wake. The first eligible ordinary queued human/API text claim after end atomically selects every ended unprojected mode in `(ended_at,id)` order, renders ledger rows in `(mode,sequence,id)` order into one exact-turn model-only projection bounded to 65,536 UTF-8 bytes (newest whole rows win; counts expose omissions), and marks every source mode consumed; even an empty ended mode gets a durable `NULL`-context marker. Recovery reads that same projection strictly by turn id, while later turns cannot replay it. Active or delayed provider-delegated turns consume neither this continuity nor pending cross-session updates. WebRTC negotiation and every live delivery/ACK must prove the exact active authenticated browser owner, session, connection, and epoch before provider data moves; owner proof never crosses the broker boundary.
