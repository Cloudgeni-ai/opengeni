...
- **Accepted or acceptance-unknown provider calls never authorize same-turn replay.** A status-less Responses-stream invalid-content `APIError`, provider 5xx/network loss, or rate limit may arrive after the provider accepted work. The worker emits only typed, secret-safe diagnostics and writes one exact-attempt/generation `failed_idle` no-replay disposition atomically with the final conversation checkpoint before ordinary lifecycle settlement. If the worker dies in that gap, heartbeat recovery consumes the fence into failed-turn/idle-session truth instead of redispatching. A successful checkpoint may pace a **new** continuation turn only when a goal is active; otherwise a later user message may continue. A failed checkpoint suppresses automatic continuation. Do not infer non-acceptance from missing usage, a transient status, or provider prose, and do not replay the full turn or completed tools.
