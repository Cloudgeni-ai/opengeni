...
- **Every persistable `/workspace` writer is mutation-admitted.** Before a provider operation that may change the durable workspace, one transaction validates the exact attempt, session group, canonical turn-attempt holder, warm lease epoch, and provider identity, then increments `workspace_generation` and inserts its admission row. The exact provider promise is physically settled whether it resolves or rejects; resolved output must separately pass the current attempt/holder/lease/provider acceptance fences. Capture and archive fold remain blocked by any unsettled admission unless that exact attempt has authoritative `session_turn_attempts.quiesced_at`. Never replay an admitted operation whose provider outcome may be ambiguous.
