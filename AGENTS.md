...
- **Every turn attempt is fenced; lifecycle settlement is atomic.** Before model/tool work, a claimed turn registers the exact Temporal activity id plus a monotonic dispatch generation and trigger. Start, approval, ordinary terminal, preemption, and worker-death settlement must match that attempt. The canonical lock order is session → turn, shared by claim and every settlement. Events, turn state, session state/pointer, and sequence commit together. A missing/already-terminal turn, superseded dispatch, or durable interrupt newer than the attempt is an event-free stale outcome. A typed schedule-to-start timeout is the only no-registration recovery case because its activity never ran. Never restore append-then-CAS or a dying activity's independent cancellation write.
