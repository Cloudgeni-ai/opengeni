...
- **Active history and approval state are bounded without trimming truth.** Before a worker decodes model-facing history JSONB, PostgreSQL measures the complete active transcript under one repeatable-read snapshot. More than 32 MiB UTF-8 JSON, 4,096 rows, 200,000 decoded JSON nodes, or 100,000 object properties fails non-retryably with `active_history_too_large`; never replace those guards with compressed storage size, a partial read, silent trimming, or an OOM. Approval-only RunState separately fails above 32 MiB/200,000 nodes/100,000 properties, pending approvals fail above 256 KiB/256 items, and ordinary turns read only RunState ownership metadata so both histories are not double-held. Normal proactive compaction should keep long sessions below the active-history envelope.
