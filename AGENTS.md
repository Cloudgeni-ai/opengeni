...
- **Context compaction is one durable, portable transition.** OpenGeni uses Codex's local plaintext checkpoint algorithm for every provider: the summarizer sees the active structured history plus the fixed checkpoint prompt; the replacement is the newest real user messages in one cumulative 20,000-token budget plus one summary. Provider failures propagate, and only a summarizer context-window rejection may retry after removing exactly one oldest compaction-input record. An empty successful response is a typed failure, while Codex terminal `response.failed`/`response.error` events are bounded provider failures rather than empty summaries. A failed same-turn recovery preserves active history and ends that workflow run without synthesizing another goal continuation from unchanged state; only a later durable work-producing wake may retry. There is no deterministic summary, hard-trim ladder, request-local history deletion, SDK `context_management`, or new queue turn. The exact current turn-attempt UUID fences both the replacement-history transaction and its token signal. This intentionally differs from Codex remote-v2: OpenGeni rotates sessions across independent subscription identities, so an opaque provider-encrypted compaction blob is not portable conversation memory. Codex subscription model limits are raw/effective/auto-compact 272,000/258,400/244,800.
