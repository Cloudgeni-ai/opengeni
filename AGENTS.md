...
- **Three memory stores, three jobs.** `session_history_items` is protocol-preserving, secret-redacted conversation truth fed to the model (default read path); known runtime credential provenance and recognized credential-bearing shapes are redacted before persistence and replay. `agent_run_states` is the serialized RunState blob, used *only* to resume a turn paused for a human approval — never as conversation memory. `session_events` is the independently redacted, lossy human/audit timeline and must never be fed back to the model. Sandbox recovery state is separate again in `sandbox_session_envelopes`. Workspace knowledge memory (`knowledge_memories`; the curated docs-MCP lane remains human-reviewed: agents search approved records and propose through its tools) is retrieval context, never conversation memory. Its typed workspace/user/role/session/ephemeral selectors are enforced by FORCE RLS; missing context denies, relationship reads require both endpoints, and lifecycle evidence is immutable, exact-actor, and secret-safe. Scoped knowledge provenance (`knowledge_providers` through immutable claims/evidence/reviews) is separate again: it is RAG evidence rather than prompt state or active memory, uses exactly organization/workspace/initiating-user authority, and proposal rows never activate policy or preferences. Do not infer human authority from session creation or provenance metadata, expose legacy scopes to the runtime role, or mutate typed selectors/edges outside the lifecycle functions. See `docs/hierarchical-memory.md` and `docs/scoped-knowledge.md`.
...
- **Codex opaque-artifact rejection preserves durable truth.** Only the recognized HTTP-400 encrypted-content rejection family may invalidate an artifact. Under the canonical attempt locks, mark matching active same-credential reasoning/compaction rows and the current turn's latest frozen RunState as provider-invalid, preserve their rows/content/provenance, and recover the same logical turn with only the rejected opaque identity omitted or neutralized. If no matching artifact is invalidated, fail instead of retrying an equivalent request. Never delete history, hot-swap credentials, or classify a generic 400 into this path.
...
- **No platform git/model credential crosses to the machine; it uses its own git auth.** The env mint skips platform git-provider tokens for a `selfhosted`-effective turn (`sandboxEnvironmentForRun({ skipGitHubToken })` in `apps/worker/src/activities/environment.ts`), and selfhosted `exec` puts ambient `env: {}` on the wire. The machine authenticates git with its **own** credentials. The deliberate exception is Toolspace parity: when Toolspace is enabled, runtime writes one narrow `ogd_` bearer to the configured token file over the exec channel. That bearer carries only `toolspace:call`, is bound to the exact workspace/session/turn/attempt/execution generation, expires after one hour, and is rejected as soon as that attempt is no longer the active running attempt. Never generalize this exception into ambient secret delivery.
