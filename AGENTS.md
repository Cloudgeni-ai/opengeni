...
- **Accepted or acceptance-unknown provider calls never authorize same-turn replay.** A status-less Responses-stream invalid-content `APIError`, provider 5xx/network loss, or rate limit may arrive after the provider accepted work. The worker emits only typed, secret-safe diagnostics and first writes one exact-attempt/generation `failed_idle` no-replay disposition with `conversationCheckpoint: incomplete`, before potentially failing final SDK-history access. The final-history transaction may only upgrade that same marker to `complete` atomically with the conversation rows; the marker is monotonic and cannot be downgraded. If the worker dies after either boundary, heartbeat recovery consumes the fence into failed-turn/idle-session truth instead of redispatching. Only `complete` may pace a **new** goal continuation. An `incomplete` checkpoint suppresses goal/internal continuation but cannot strand a separately queued human/API turn: the locked settlement returns that canonical queue fact so the workflow performs its next FIFO peek/claim even when the prompt's signal predates the workflow baseline. Do not infer non-acceptance from missing usage, a transient status, or provider prose, and do not replay the failed turn or completed tools.
- **Escaped MCP transport timeouts are recoverable without replay.** A connect/tools-list/next-loop MCP request timeout can escape after a successful tool output even though thrown tool-call failures are normally converted to `isError` outputs. The worker establishes the same monotonic no-replay fence, completes the conversation checkpoint when final history remains available, truthfully settles that turn as retryable/idle, and lets an active goal continue after pacing only from the complete checkpoint; it never blindly replays the completed tool call or full turn.
...
- **Worker deaths recover only replay-safe logical turns, never by manufacturing queue work.** Before provider acceptance/progress (or when the same provider execution identity is preserved), graceful shutdown or ungraceful worker loss checkpoints durable truth and moves the exact turn to `recovering`; the workflow claims that same turn with a new attempt UUID and generation. An exact accepted/acceptance-unknown no-replay marker instead settles the failed turn and cannot reclaim it whether its conversation checkpoint is `complete` or `incomplete`; incomplete history refuses automatic goal/internal continuation but still admits any independent queued human/API turn reported by the locked settlement. That exact result remains idempotently recognizable if either ordinary settlement or worker-death settlement commits but its Temporal activity response is lost. Neither path creates a prompt, resume notice, or machine-generated queue row. A dying activity never performs a competing terminal settlement, and late callbacks remain visible as rejected evidence without becoming current truth. Do not add automatic Temporal retries around the side-effectful agent activity.
...
- **Mutating sandbox transport errors have an acceptance boundary, not a retry heuristic.** `exec`, PTY input, writes, materialization, desktop input, and editor mutations are invoked once when a typed transport/fence error leaves acceptance unknown; the routing proxy returns `sandbox_mutation_acceptance_unknown` with the exact active epoch and no raw cause. A bounded retry is allowed only when a trusted backend error proves the invocation was never admitted (currently typed selfhosted `neverSent`/`fenced`); no execution identity is inferred from arbitrary exec/process fields. DNS/`UNAVAILABLE` is transport uncertainty, not authoritative provider `NotFound`. Read-only fence retries remain separate. Canonical: `packages/runtime/src/sandbox/routing/routing-session.ts`.
