...
- **Worker deaths recover the same logical turn only when replay safety is proved.** A graceful shutdown or ungraceful worker loss checkpoints durable conversation truth and moves the exact turn to `recovering` only when no provider/model boundary was admitted or when a completed boundary has a linked receipt that the persistence-only lane can settle first. An admitted but unlinked model/compaction call has unknown effect and atomically fails the exact attempt with `ambiguous_model_call`; it is never redispatched. Safe recovery claims the same turn with a new attempt UUID/generation and creates no prompt, resume notice, or machine queue row. A dying activity never performs a competing terminal settlement, and late callbacks remain rejected evidence. Do not add automatic Temporal retries around the side-effectful agent activity.
- **Provider admission and post-effect recovery are PostgreSQL-owned, not heartbeat-owned.** Immediately before every model or compaction provider invocation, the worker durably inserts exact provenance in `session_turn_model_call_admissions`; only one admission may be unlinked per attempt. Provider completion prepares the full obligation and atomically links it to a new `session_turn_persistence_receipts` row before ordinary persistence. The contract is `admission → provider invocation → complete obligation → atomic admission/receipt link → persistence-only phases → receipt settlement`. Temporal carries only a strict version-2 receipt/identity/digest reference of at most 1 KiB. Recovery discovers exact rows even when death precedes a heartbeat: linked pending receipts run persistence code only, while admitted+unlinked calls quarantine with unknown-effect truth and never replay provider/runtime/tool execution. Malformed or mismatched evidence is also atomic non-retryable quarantine, never generic redispatch.
