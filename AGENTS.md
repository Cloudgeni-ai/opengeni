...
- **Context compaction is one durable, portable transition.** OpenGeni uses Codex's local plaintext checkpoint algorithm for every provider: the summarizer sees the active structured history plus the fixed checkpoint prompt; the replacement is the newest real user messages in one cumulative 20,000-token budget plus one summary. Provider failures propagate, and only a summarizer context-window rejection may retry after removing exactly one oldest compaction-input record. There is no deterministic summary, hard-trim ladder, request-local history deletion, SDK `context_management`, or new queue turn. The exact current turn-attempt UUID fences both the replacement-history transaction and its token signal. This intentionally differs from Codex remote-v2: OpenGeni rotates sessions across independent subscription identities, so an opaque provider-encrypted compaction blob is not portable conversation memory. Codex subscription model limits are raw/effective/auto-compact 272,000/258,400/244,800.
...
- **Responses invalid-content errors settle without replay.** The narrowly identified, status-less Responses-stream `APIError` for provider-produced invalid content records only bounded allow-listed diagnostics and requires a final conversation-truth checkpoint. A successful checkpoint permits only a paced new goal turn or later user message; a failed checkpoint recoverably fails the session until a user revival. Never replay the original trigger, full turn, model/tool request, history, or raw provider payload.
...
- **Sandbox warming and gone-box recovery are bounded, tracked, and elected.** A provider instance id must be persisted on the warming lease immediately after create returns, before readiness/setup. A warm/attached resume that proves the exact provider instance is gone returns a typed outcome and never creates in the runtime leaf; one `(lease_epoch, instance_id)`-fenced `warm → warming` winner creates/checkpoints the replacement while all other callers wait and re-read. Re-materializing the same logical target bumps the lease epoch and its sessions' active routing epochs atomically. A turn waiting on another worker's warming lease is bounded by `OPENGENI_SANDBOX_WARMING_TIMEOUT_MS` (default 600000) and fails clearly on backend capacity/create timeout instead of heartbeating forever.
