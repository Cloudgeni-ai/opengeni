...
- **Goal mutations and their timeline facts are one session-sequenced commit.** Agent/API set, update, complete, pause, resume, and clear must acquire the session sequence owner before the goal mutation and append the event in that transaction. Agent `goal_update` additionally requires one target-scoped operation receipt that atomically stores its exact result and remains replayable across replacement attempts without overwriting a newer revision. Never mutate a goal and append/publish its event in separate commits, and never pre-lock the workspace row solely around a goal event: both patterns can recreate the active-turn/goal-tool self-lock.
