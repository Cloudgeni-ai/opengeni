...
- **Preemption settlement is atomic and control wins.** Context-compaction, graceful-shutdown, and sandbox-supersession recovery lock the session and exact active turn, then append `turn.preempted`, requeue the turn, and clear the session pointer in one transaction. A missing/already-terminal turn, a newer active attempt, or a durable interrupt newer than the current attempt is an idempotent stale outcome and appends no contradictory preemption event. Never restore the old append-then-CAS sequence.
