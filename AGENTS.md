...
- **Every turn attempt is fenced; lifecycle settlement is atomic.** Before model/tool work, a claimed turn registers a first-class UUID attempt plus its execution generation and exact Temporal dispatch. Every event writer uses the canonical order `workspace_inference_controls FOR SHARE` (or a caller-owned stronger control mutation lock) → the actual `workspaces` row `FOR KEY SHARE` → UUID-ordered sessions `FOR UPDATE` → UUID-ordered exact turns `FOR UPDATE` → UUID-ordered exact attempts `FOR UPDATE`. The shared control and key-share workspace locks preserve unrelated-session concurrency without letting generic audit/title appends cross a workspace control boundary. Events, conversation history, run state, context replacement, input-token signals, queue/control state, system-update bundles, turn state, session pointer, and sequence commit must all reject a replaced attempt. A rejected callback is retained as `turn.event.rejected_late` evidence. A normal Send appends a prompt but never mutates an unsettled Pause/Steer fence. Never restore append-then-CAS or a dying activity's independent cancellation write.
