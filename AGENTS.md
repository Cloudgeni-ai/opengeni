...
- **Active history materialization is bounded without trimming truth.** Before a worker decodes model-facing history JSONB, PostgreSQL measures the complete active transcript's UTF-8 JSON representation under one repeatable-read snapshot. More than 32 MiB fails non-retryably with `active_history_too_large`; never replace that guard with compressed storage size, a partial read, silent trimming, or an OOM. Normal proactive compaction should keep long sessions below this final envelope.
