...
- **Post-effect recovery is PostgreSQL-owned, not heartbeat-owned.** Before a completed tool/model/compaction boundary can hand off, the worker stores the full pending obligation and digest in `session_turn_persistence_receipts`; Temporal carries only a strict version-2 receipt/identity/digest reference of at most 1 KiB. Recovery must discover the row by exact attempt when death precedes the heartbeat, run only persistence code, and never replay provider/runtime/tool execution. Malformed or mismatched evidence is an atomic non-retryable quarantine with unknown-effect audit truth, never generic redispatch.
