...
the `conversation` query parameter to one session, and the reply streams back.
Open `/?conversation=c_2` for a second, isolated chat. The demo user is the
`x-demo-user` header the page sends; the backend treats it as the signed-in
user, so the agent remembers that user across their chats (`memory: "user"`)
while each chat stays isolated from every other chat (`agentAccess: "session"`).
