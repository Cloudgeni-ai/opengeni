...
The imported composer, picker and stylesheet files were verified identical between
the candidate checkout and `main` at `380bba5`. The baseline authorization form is
loaded verbatim by the build from that commit. The proposed access control is local
to this fixture: no production route imports it and the design is not approved.
...
source JSON bundle. The baseline component and checkout instructions are included
in that source bundle. No runtime workspace tools are requested by the Site.
...
For visual review, serve the generated HTML and switch between **Current source**
and **Proposed change**. Exercise the real model/tools menus, duration selection,
Send, the empty-draft guard, narrow widths, and light/dark themes. This component
