...
import { parseIdentityLinkContinuation, readIdentityLinkContinuation, retainIdentityLinkContinuation } from "./identity-link-continuation";

test("identity confirmation challenge is tab-local and scrubbed before application mount", () => {
  const linkId = crypto.randomUUID();
  const organizationId = crypto.randomUUID();
  const challenge = "a".repeat(43);
  const url = new URL(`https://app.example/identity-links/${linkId}?organization=${organizationId}#challenge=${challenge}`);
  ...
  retainIdentityLinkContinuation({
    ...
    history: { state: { kept: true }, replaceState: (state: unknown, _unused: string, next: string | URL | null | undefined) => {
      expect(state).toEqual({ kept: true }); replaced = String(next);
    } } as History,
  });
