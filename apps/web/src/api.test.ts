...
describe("web API auth helpers", () => {
  ...
  test("owns the native browser batch terminal before exposing its detached bytes", async () => {
    jest.useFakeTimers();
    const originalFetch = globalThis.fetch;
    const windowDescriptor = Object.getOwnPropertyDescriptor(globalThis, "window");
    const entriesDescriptor = Object.getOwnPropertyDescriptor(performance, "getEntriesByType");
    ...
    let aborts = 0;
    let completeRequests = true;
    class BrowserBatchRequest {
      ...
      response: ArrayBuffer = new TextEncoder().encode(": connected\n\n").buffer;
      ...
      getAllResponseHeaders(): string {
        return [
          "content-length: 13",
          "content-type: application/vnd.opengeni.sse-batch; charset=utf-8",
          "x-opengeni-actor-epoch: xhr-actor",
        ].join("\r\n");
      }
      ...
      send(): void {
        ...
        if (completeRequests) queueMicrotask(() => this.onload?.({} as ProgressEvent<EventTarget>));
      }
    ...
    Object.defineProperty(globalThis, "XMLHttpRequest", {
      configurable: true,
      value: BrowserBatchRequest as unknown as typeof XMLHttpRequest,
    });
    globalThis.fetch = (async () => {
      throw new Error("bounded browser batches must not use fetch");
    }) as unknown as typeof fetch;

    try {
      configureManagedActorEpoch("xhr-actor");
      const response = await managedActorFetch(
        "https://api.example.test/v1/workspaces/current/live-events/stream",
        {
          credentials: "include",
          headers: { accept: "text/event-stream" },
      ...
      expect(new URL(requests[0]!.url).searchParams.get("transport")).toBe("http1-bounded");
      expect(requests[0]!.headers.get("accept")).toBe("application/vnd.opengeni.sse-batch");
      expect(requests[0]!.headers.get("x-opengeni-actor-epoch")).toBe("xhr-actor");
      ...
      const pending = managedActorFetch(
        "https://api.example.test/v1/workspaces/current/live-events/stream",
        {
          credentials: "include",
          headers: { accept: "text/event-stream" },
      ...
      await Promise.resolve();
      configureManagedActorEpoch("next-xhr-actor");
