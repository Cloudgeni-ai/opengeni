...
const workspaceNavSource = await Bun.file(
  `${import.meta.dir}/workspace-nav.tsx`,
).text();
...
describe("workspace rail destinations", () => {
  ...
  test("keeps primary product destinations out of workspace administration", () => {
    ...
    const settingsTargets = WORKSPACE_CONFIG_GROUPS.flatMap(
      (group) => group.items,
    ).map((item) => item.to);
    ...
    expect(
      settingsTargets.filter((target) => primaryTargets.includes(target)),
    ).toEqual([]);
    expect(isWorkspaceConfigPath("/workspaces/ws-1/schedules", "ws-1")).toBe(
      false,
    );
    expect(isWorkspaceConfigPath("/workspaces/ws-1/artifacts", "ws-1")).toBe(
      false,
    );
    expect(isWorkspaceConfigPath("/workspaces/ws-1/documents", "ws-1")).toBe(
      false,
    );
    expect(isWorkspaceConfigPath("/workspaces/ws-1/state", "ws-1")).toBe(false);
    expect(isWorkspaceConfigPath("/workspaces/ws-1/settings", "ws-1")).toBe(
      true,
    );
    expect(
      isConfigItemActive(
