...
export function LensSetupCard(props: {
  ...
  const [provider, setProvider] = useState<LensProvider>("github");
  const [name, setName] = useState("OpenGeni Lens");
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 rounded-xl border border-border bg-surface/50 p-4 text-xs text-fg-muted">
        <Loader2Icon className="size-4 animate-spin" /> Loading OpenGeni Lens setup…
      </div>
    );
  }
  ...
  return (
    <section className="grid gap-4 rounded-xl border border-brand/30 bg-brand/5 p-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-sm font-semibold">Configure OpenGeni Lens</h3>
          <p className="mt-1 text-xs leading-5 text-fg-muted">
            Register a dedicated provider identity, append its authenticated webhook path to your
            OpenGeni API URL, then enable the repositories Lens may review. Every event starts an
            ordinary exact-head agent session.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-3 rounded-lg border border-border bg-surface/70 p-3">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          <Select
            value={provider}
            onChange={(event) => setProvider(event.target.value as LensProvider)}
          >
...
function messageForError(reason: unknown): string {
  return reason instanceof Error ? reason.message : "OpenGeni Lens setup failed";
}
