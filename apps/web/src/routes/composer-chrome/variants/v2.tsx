// 2 · Filament — Morph-family compact dock: icon chips, underline morph, dense expand.
...
type SignalTone = "neutral" | "brand" | "waiting" | "running";
...
const TONE_FILAMENT: Record<SignalTone, string> = {
  neutral: "bg-fg/70",
  brand: "bg-brand",
  waiting: "bg-status-waiting",
  running: "bg-status-running",
    ...
    <div className="flex flex-col gap-6">
      <header className="space-y-3">
        <p className="text-sm leading-relaxed text-fg-muted">
          Morph-family, slimmed: icon chips only, a tone-colored filament slides under the active
          signal, and the panel expands compactly with hover actions. No SDK Morph — same shared-
          element idea as tab 7, denser. ChatComposer stays production.
        </p>
      ...
      <div className="flex flex-col gap-10">
        {visible.map((scenario, index) => (
          <section
            key={scenario.id}
            aria-labelledby={`v2-scenario-${scenario.id}`}
          ...
          >
            <header className="space-y-1 border-b border-border bg-surface-2/50 px-4 py-3 sm:px-5">
              ...
              <h2 id={`v2-scenario-${scenario.id}`} className="text-sm font-semibold text-fg">
                {scenario.title}
              </h2>
              <p className="text-xs text-fg-muted">{scenario.description}</p>
            </header>
            <div className="p-3 sm:p-4">
              <FilamentStack scenario={scenario} composer={composer} />
            </div>
          </section>
        ))}
      </div>
...
function FilamentStack({
  ...
  return (
    <div
      ...
      data-variant="filament"
    >
      <FilamentDock scenario={scenario} composer={composer} />
...
function FilamentDock({
  ...
  const [filament, setFilament] = useState({ left: 0, width: 0, opacity: 0 });
  ...
  useEffect(() => {
    ...
    const measure = () => {
      if (!rail || !active) {
        setFilament((prev) => ({ ...prev, opacity: 0 }));
        return;
      }
      ...
      setFilament({
        left: chipBox.left - railBox.left + 6,
        width: Math.max(16, chipBox.width - 12),
        opacity: 1,
      });
    };
    measure();
  ...
  const activeTone = signals.find((signal) => signal.id === active)?.tone ?? "neutral";
  const panelStyle = {
    ...
    <div className="mx-auto mb-1.5 w-full max-w-3xl shrink-0 px-4 sm:px-6">
      <div
        className={cn(
          ...
          "bg-surface-2/70 shadow-[0_4px_20px_-12px_rgba(0,0,0,0.4)] backdrop-blur-md",
          "supports-[backdrop-filter]:bg-surface-2/50",
          "transition-[box-shadow,border-color] duration-250 ease-out",
          open && "border-border-strong/40 shadow-[0_10px_28px_-16px_rgba(0,0,0,0.5)]",
        )}
      >
        <div ref={railRef} className="relative flex flex-wrap items-stretch gap-0.5 px-1 pt-1 pb-0.5">
          <div
            aria-hidden
            className={cn(
              "pointer-events-none absolute bottom-0 h-0.5 rounded-full",
              "transition-[transform,width,opacity,background-color] duration-300",
              "ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none",
              TONE_FILAMENT[activeTone],
            )}
            style={{
              width: filament.width,
              opacity: filament.opacity,
              transform: `translate3d(${filament.left}px, 0, 0)`,
            }}
          />
          {signals.map((signal) => {
            ...
            return (
              <button
                ...
                aria-controls={`v2-filament-panel-${scenario.id}`}
                onClick={() => setActive((prev) => (prev === signal.id ? null : signal.id))}
                className={cn(
                  "group relative z-[1] inline-flex min-h-7 min-w-0 items-center gap-1 rounded-lg px-2 py-1 text-left text-xs outline-none",
                  "transition-[color,background-color] duration-150 ease-out",
                  "hover:bg-bg/40 hover:text-fg focus-visible:ring-2 focus-visible:ring-ring/40",
              ...
              >
                ...
                {signal.count ? (
                  <span
                    className={cn(
                      "shrink-0 rounded px-1 py-px font-mono text-2xs tabular-nums",
                      selected ? "bg-bg/70 text-fg-muted" : "bg-surface-3/80 text-fg-subtle",
                    )}
                  >
                    {signal.count}
                  </span>
          ...
          {open ? (
            <button
              ...
              className="relative z-[1] ml-auto inline-flex size-7 shrink-0 items-center justify-center rounded-lg text-fg-subtle outline-none transition-colors hover:bg-bg/50 hover:text-fg focus-visible:ring-2 focus-visible:ring-ring/40"
            >
        ...
        <div
          id={`v2-filament-panel-${scenario.id}`}
        ...
        >
          <div className="min-h-0 overflow-hidden">
            <div
              className={cn(
                "relative border-t border-border/50 pl-3 pr-2.5 pb-2.5 pt-2",
  ...
  return (
    <ul className="flex flex-col gap-0.5" aria-label="Incoming updates">
      {inputs.map((input) => (
        <li
          key={input.id}
          className="group flex items-start gap-2 rounded-md px-1.5 py-1 transition-colors hover:bg-surface-3/45"
        >
          <span
            className={cn(
              "mt-0.5 shrink-0 rounded px-1 py-px text-2xs font-medium",
          ...
          <p className="min-w-0 flex-1 text-xs leading-4.5 text-fg">{input.summary}</p>
        </li>
      ))}
    </ul>
  );
    ...
    <ol className="flex flex-col gap-0.5" aria-label="Queued prompts">
      {turns.map((turn, index) => (
        ...
        >
          ...
          <p className="min-w-0 flex-1 truncate text-xs leading-4.5 text-fg" title={turn.prompt}>
            {turn.prompt}
          </p>
...
function GoalPanel({
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-1.5 text-2xs font-medium uppercase tracking-wider text-fg-subtle">
      ...
      <p className="text-sm leading-5 text-fg">{record.text}</p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5">
        <div className="flex flex-wrap gap-1 text-2xs text-fg-muted">
          <span className="rounded bg-surface-3/70 px-1.5 py-px">
...
function AgentsPanel({ workspaceId, nodes }: { workspaceId: string; nodes: LineageNode[] }) {
  return (
    <div>
      <p className="mb-1.5 text-2xs font-medium uppercase tracking-wider text-fg-subtle">
