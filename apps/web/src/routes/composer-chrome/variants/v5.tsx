// 5 · Shard — Morph-family compact dock: denser chips, snappy expand, hover actions.
    ...
    <div className="flex flex-col gap-6">
      <header className="space-y-3">
        <p className="text-sm leading-relaxed text-fg-muted">
          Morph-family, cut thinner: icon-led chips, a shared highlight that slides under the active
          signal, and a snappy expand with hover-revealed actions. Full scenario matrix; ChatComposer
          stays production.
        </p>
      ...
      <div className="flex flex-col gap-10">
        {visible.map((scenario, index) => (
          <section
            key={scenario.id}
            aria-labelledby={`v5-scenario-${scenario.id}`}
          ...
          >
            <header className="space-y-1 border-b border-border bg-surface-2/50 px-4 py-3 sm:px-5">
              ...
              <h2 id={`v5-scenario-${scenario.id}`} className="text-sm font-semibold text-fg">
                {scenario.title}
              </h2>
              <p className="text-xs text-fg-muted">{scenario.description}</p>
            </header>
            <div className="p-3 sm:p-4">
              <ShardStack scenario={scenario} composer={composer} />
            </div>
          </section>
        ))}
      </div>
...
function ShardStack({
  ...
  return (
    <div
      ...
      data-variant="shard"
    >
      <ShardDock scenario={scenario} composer={composer} />
...
function ShardDock({
  ...
  const [hovered, setHovered] = useState<SignalId | null>(null);
    ...
    <div className="mx-auto mb-1.5 w-full max-w-3xl shrink-0 px-4 sm:px-6">
      <div
        className={cn(
          ...
          "bg-surface-2/50 shadow-[0_6px_24px_-14px_rgba(0,0,0,0.4)] backdrop-blur-xl",
          "supports-[backdrop-filter]:bg-surface-2/35",
          "ring-1 ring-inset ring-white/5",
          "transition-[box-shadow,border-color] duration-200 ease-out",
          open && "border-border-strong/45 shadow-[0_12px_32px_-18px_rgba(0,0,0,0.5)]",
        )}
      >
        <div
          aria-hidden
          ...
          {signals.map((signal) => {
            ...
            return (
              <button
                ...
                aria-controls={`v5-shard-panel-${scenario.id}`}
                title={signal.detail ? `${signal.label}: ${signal.detail}` : signal.label}
                onClick={() => setActive((prev) => (prev === signal.id ? null : signal.id))}
                onMouseEnter={() => setHovered(signal.id)}
                onMouseLeave={() => setHovered((prev) => (prev === signal.id ? null : prev))}
                className={cn(
              ...
              >
                ...
                <span className="shrink-0 font-semibold tabular-nums">{signal.count}</span>
                <span
                  className={cn(
                    "min-w-0 max-w-0 overflow-hidden truncate font-medium opacity-0 transition-[max-width,opacity] duration-200 ease-out",
                    "group-hover:max-w-[7rem] group-hover:opacity-100",
                    "group-focus-visible:max-w-[7rem] group-focus-visible:opacity-100",
                    selected && "max-w-[7rem] opacity-100",
                    "sm:group-hover:max-w-[10rem] sm:group-focus-visible:max-w-[10rem]",
                    selected && "sm:max-w-[10rem]",
                  )}
                >
                  {signal.label}
                </span>
        ...
        >
          <div className="min-h-0 overflow-hidden">
            ...
            >
              {active === "incoming" ? <IncomingPanel inputs={queue.pendingInputs} /> : null}
              {active === "queue" ? (
                <QueuePanel
                  turns={queue.queue}
  ...
  return (
    <ul className="flex flex-col gap-0.5" aria-label="Incoming updates">
      {inputs.map((input) => (
        <li
          key={input.id}
          className="group flex items-start gap-2 rounded-md px-1.5 py-1 transition-colors hover:bg-surface-3/50"
        >
          <span
            className={cn(
    ...
    <ol className="flex flex-col gap-0.5" aria-label="Queued prompts">
      {turns.map((turn, index) => (
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100 focus-within:opacity-100 pointer-coarse:opacity-100">
            <button
              type="button"
              aria-label={`Edit queued prompt ${index + 1}`}
              onClick={() => onEdit(turn)}
              className="inline-flex size-6 items-center justify-center rounded-md text-fg-subtle outline-none hover:bg-surface-2 hover:text-fg focus-visible:ring-2 focus-visible:ring-ring/40"
            >
            ...
            <button
              ...
              className="inline-flex size-6 items-center justify-center rounded-md text-fg-subtle outline-none hover:bg-surface-2 hover:text-destructive focus-visible:ring-2 focus-visible:ring-ring/40"
            >
