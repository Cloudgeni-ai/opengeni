...
/**
 * The manual editor is gated exactly like the API route: the account grant
 * for the workspace's organization must carry `account:admin` (organization
 * owners). Organization admins and members hold no such permission.
 */
function accessContext(role: "owner" | "admin" | "member"): AccessContext {
  return {
    mode: "managed",
    subjectId,
    accountGrants: [
      {
        ...
        subjectId,
        role,
        permissions: role === "owner" ? ["account:read", "account:admin"] : ["account:read"],
      },
    ],
    workspaceGrants: [
      {
...
describe("organization company-profile manual editor", () => {
  test("lets an organization owner holding account:admin edit and activate", async () => {
  ...
  test("keeps organization admins and members without account:admin read-only", async () => {
  ...
  test("shows a server-side authorization error and re-enables the save control", async () => {
    ...
    try {
      ...
      expect(container.textContent).toContain("missing permission: account:admin");
  ...
  test("lets an organization owner activate an agent-admin proposal manually", async () => {
  ...
  test("lets an organization owner roll back a previously active revision", async () => {
