...
async function probeRfbBanner(port: number, timeoutMs = 8_000): Promise<{ ok: boolean; banner: string }> {
...
beforeAll(async () => {
  ...
  if (!docker) {
    console.warn("[p41-image] docker unavailable — skipping the local image build + stack assertions");
    return;
  }
...
describe("P4.1 desktop image — LOCAL build + stack-up assertions", () => {
  test("build docker/desktop.Dockerfile, run it, assert noVNC/WS/real-Chrome/stack-up", async () => {
    if (!docker) return;
    ...
    await exec(
      ...
      { cwd: REPO_ROOT, timeout: 30 * 60 * 1000, maxBuffer: 64 * 1024 * 1024 },
    );
    built = true;

    // 2) RUN it (entrypoint is `sleep infinity`; the stack is exec-launched).
    await exec("docker", [
      "run", "-d", "-p", `${HOST_PORT}:6080`, "--name", CTR, IMAGE,
    ]);
    ...
    const socks = await sh("(nc -z localhost 5900 && echo VNC_OK); (nc -z localhost 6080 && echo WS_OK)");
...
describe.if(LIVE)("P4.1 desktop image — GATED LIVE Modal/gVisor (XTEST read-back regression)", () => {
  test("build a Modal image from docker/desktop.Dockerfile, boot, ensureDisplayStack, XTEST + scrot", async () => {
    ...
    try {
      ...
      const socks = await run(`(nc -z localhost 5900 && echo VNC_OK); (nc -z localhost ${STREAM_PORT} && echo WS_OK)`);
