...
function registerFleetTools(
  ...
  server.registerTool(
    ...
    {
      description:
        "Provision a new sandbox for the fleet. kind=selfhosted returns device-flow enrollment instructions to share with a HUMAN (install the agent + enroll their machine with loud whole-machine consent — the agent cannot self-consent). kind=modal creates a named Modal target that materializes lazily under its own sandbox UUID lifecycle when selected; it never reuses a calling session's home envelope.",
      inputSchema: {
