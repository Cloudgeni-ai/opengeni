...
function registerWorkspaceOrchestrationTools(
  ...
  if (can("sessions:read")) {
    ...
    server.registerTool(
      ...
      {
        description:
          "Get one session: status, goal-bearing metadata, resources, tools, and environment attachment (names/ids only, never variable values). Unbounded agent-set fields (metadata, initial message) are clamped so monitoring another session cannot flood this context.",
...
function registerEnvironmentTools(
  ...
  if (can("environments:use")) {
    server.registerTool(
      ...
      {
        description:
          "List workspace environments with variable names and metadata (versions, timestamps). Values are write-only and never returned.",
        inputSchema: {},
      },
      async () =>
        json({ environments: await listWorkspaceEnvironments(deps.db, grant.workspaceId) }),
    );
  }
  ...
  if (can("environments:manage")) {
    server.registerTool(
      ...
      {
        description:
          "Set or rotate one variable in a workspace environment. Target by environmentId, or by environmentName (created if it does not exist). The value is encrypted at rest and injected into sandboxes of sessions the environment is attached to; it is never readable back through any API.",
        inputSchema: {
          environmentId: z4.string().uuid().optional(),
      ...
      async ({ environmentId, environmentName, name, value }) => {
        ...
        let environment =
          environmentId !== undefined
            ...
            : await getWorkspaceEnvironmentByName(
                deps.db,
                grant.workspaceId,
        ...
        if (!environment) {
          if (
            (await countWorkspaceEnvironments(deps.db, grant.workspaceId)) >=
            MAX_ENVIRONMENTS_PER_WORKSPACE
          ) {
            throw new Error(
              `a workspace supports at most ${MAX_ENVIRONMENTS_PER_WORKSPACE} environments`,
            );
          }
          ...
          created = true;
          await recordEnvironmentAuditEvent(deps.db, {
            grant,
        ...
        if (!exists && environment.variables.length >= MAX_VARIABLES_PER_ENVIRONMENT) {
          throw new Error(
            `an environment supports at most ${MAX_VARIABLES_PER_ENVIRONMENT} variables`,
          );
        }
...
function requireEnvironmentsUseForMcpAttachment(
  grant: AccessGrant,
