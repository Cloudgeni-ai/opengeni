...
export async function buildCapabilityCatalog(input: {
  ...
  const [
    ...
    listCapabilityCatalogItems(input.db),
    listCapabilityInstallations(input.db),
    listPackInstallations(input.db),
...
export async function enableCapability(input: {
  ...
  const item = await requireCatalogItem(input.db, input.settings, input.capabilityId);
...
export async function disableCapability(input: {
  ...
  if (item.kind === "pack") {
    await updatePackInstallationStatus(input.db, packIdFromCapabilityId(item.id), "disabled").catch(() => undefined);
    if (!await getCapabilityInstallation(input.db, item.id)) {
  ...
  } else if (!await getCapabilityInstallation(input.db, item.id)) {
    throw new HTTPException(409, { message: "capability is not currently enabled" });
  }
  return await disableCapabilityInstallation(input.db, item.id);
}

export async function settingsWithEnabledCapabilityMcpServers(db: Database, settings: Settings): Promise<Settings> {
  const enabled = await listEnabledMcpCapabilityServers(db);
...
async function requireCatalogItem(db: Database, settings: Settings, capabilityId: string): Promise<CapabilityCatalogItem> {
  const catalog = await buildCapabilityCatalog({ db, settings });
  const item = catalog.items.find((candidate) => candidate.id === capabilityId) ?? await getCapabilityCatalogItem(db, capabilityId);
  if (!item) {
...
function platformApiCatalogItems(): CapabilityCatalogItem[] {
  return [
    ...
    },
    {
      ...
      endpointPath: "/v1/document-bases",
    },
    {
      id: "api:social",
      ...
      endpointPath: "/v1/social/connections",
    },
    {
      ...
      endpointPath: "/v1/scheduled-tasks",
    },
