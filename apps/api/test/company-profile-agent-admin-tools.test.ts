...
import { registerCompanyProfileAgentAdminTools } from "../src/mcp/company-profile-agent-admin";
    ...
    router: {
      ...
      async confirm(input) {
        ...
        throw new CompanyProfileAgentAdminError(
          ...
          "The bound organization-owner/admin confirmation is unavailable or no longer valid.",
        );
      },
    },
  });
...
describe("company-profile agent administration MCP tools", () => {
  ...
  test("returns a bounded denial when bound human confirmation is unavailable", async () => {
    ...
    expect(JSON.parse(result.content[0]!.text)).toEqual({
      ...
      message: "The bound organization-owner/admin confirmation is unavailable or no longer valid.",
    });
