...
describe("managed personal-resource grant HTTP lifecycle", () => {
  ...
  for (const { name, serverId, providerDomain, url } of [
    {
      name: "Gmail",
      serverId: "gmail",
    ...
    test(`${name} consent and accepted use work before private-session activation in Personal and shared workspaces`, async () => {
      if (!shared || !client) return;
      const human = await provisionManagedHuman();
      const app = buildApp(undefined, true, {
        mcpServers: [
      ...
      for (const workspaceId of [human.personalWorkspaceId, human.legacyWorkspaceId]) {
        ...
        expect(projection.connectionContext).toEqual({
          visibility: "workspace",
          authorityEpoch: 1,
        });
          ...
          body: JSON.stringify({
            ...
            payload: {
              ...
              connectionAuthorities: [
                { serverId, connectionId: connection.id, userDelegation: grant.delegation },
              ],
            },
          }),
        });
