...
describe("workspace member helpers", () => {
  ...
  test("memberCanAdminister recognizes admin and members:manage (admin is the wildcard)", () => {
    expect(
      memberCanAdminister(member({ subjectId: "user:a", permissions: ["workspace:admin"] })),
    ).toBe(true);
    expect(
      memberCanAdminister(member({ subjectId: "user:b", permissions: ["members:manage"] })),
    ).toBe(true);
    expect(
      memberCanAdminister(member({ subjectId: "user:c", permissions: ["sessions:read"] })),
    ).toBe(false);
  });
